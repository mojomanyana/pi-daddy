...
| C03 practical watch/steer | `pi-daddy/daily-dashboard-host`; `/grants host`; dashboard socket actions | Active-child suppression; priority → scope → alternative; atomic persisted/session rebind; next occurrence names the rebound obligation | None before use; the operator still chooses each displayed action. |
| C04 measured variants/accounting | `pi-daddy/measured-session` | Bounded no-tool Sol/Terra SDK runs retained observed identity and Pi Usage; abort-during-reservation race repaired and independently approved | Pi catalog cost remains an estimate, not subscription billing. Ordinary print-child Usage remains unavailable. |
