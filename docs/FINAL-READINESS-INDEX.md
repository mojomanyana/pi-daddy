...
| Requirement | Candidate entrypoint/code | Decisive evidence | Remaining decision |
|---|---|---|---|
| C00/C01 truthful declared work | `pi-daddy work add`; work-ledger and occurrence APIs | Released baseline plus candidate occurrence tests | Human acceptance is never inferred. |
| C02 retained facts and closing review | Daily host; dashboard presenter; projection-only runtime facts | Explicit two-minute closing presence; paused, reservation-free and ordinary-quiescent gates; durable five-card ceiling; unavailable policy spends 0/5 | Exact trust calibration described below. |
| C03 practical watch/steer | `pi-daddy/dashboard`; `/grants host`; dashboard socket actions | Active-child suppression; priority → scope → alternative; atomic persisted/session rebind; next occurrence names the rebound obligation | None before use; the operator still chooses each displayed action. |
| C04 measured variants/accounting | `pi-daddy/work` | Bounded no-tool Sol/Terra SDK runs retained observed identity and Pi Usage; abort-during-reservation race repaired and independently approved | Pi catalog cost remains an estimate, not subscription billing. Ordinary print-child Usage remains unavailable. |
| C05 current status | Released Principal 3.2 status projection | Released tests and prior fresh-install receipt | No candidate code gap. |
...
| C07 useful dependent order | `pi-daddy/work` and root export | Exact and free-form projection contracts; complete sealed predecessor bytes/hash; original-owner unknown recovery; inode replacement refusal; both retained NWS outputs pass the contract offline | No live order was replayed merely to prove packaging. Effectful work still needs a separately qualified profile. |
...
- **Prior real SDK comparison:** two fresh subscription-backed C08 runs used direct Pi SDK sessions, no tools, in-memory sessions/settings, empty discovery and frozen public NWS rows. Their retained outputs and Usage are real.
- **Currently repaired packaged route:** measured-session cancellation and measured-order recovery/qualification are candidate package code. Tests exercise their host boundary; the packed-install check imports public entrypoints and loads only the two candidate extensions with zero provider requests.
- **No-model retained-output replay:** the C07 validator rechecks the already-retained C08 bytes against the integrated deterministic projection contract and records their actual hashes. It is not another model occurrence or a completed measured order.

No additional live proof is indispensable for packaging: the uncertainty under test is module/extension compatibility, which the zero-model packed host directly exercises. A fresh model call would not strengthen that claim.

## Exact unresolved inputs and next actions
...
Factory-owned retained evidence includes `REVIEW-03.md`, `MILESTONE-03-CONTROLLED-ORDERS.md`, `PR-RECEIPT-03.md`, `REQUIREMENT-AUDIT-03.md`, `packed-compatibility/RECEIPT.json`, and the two rendered human artifacts above.
