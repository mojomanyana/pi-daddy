...
export function createLearningSession(host: ReturnType<typeof createDailyDashboardSession>) {
  let busy = false;
  return {
    async run(_verb: string, ctx: ExtensionCommandContext) {
      ...
      try {
        ...
        const menu = [
          "Open cases, comparisons, trust and outcomes",
          "Create learning workspace for selected work",
          "Connect existing learning workspace",
          "Adoption / rollback for next orders",
          "Retain current work for learning",
          "Link observed result from latest adopted order",
        ];
        const choice = await ctx.ui.select(
          connected ? "Learning — deliberate review (not earned automatic exposure)" : "Learning not connected",
          connected ? menu : menu.slice(1, 3),
        );
        if (!choice) return;
        if ([menu[1], menu[2]].includes(choice)) {
          if (
            ...
            !(await ctx.ui.confirm(
              "Reconnect learning after stopping the current host?",
              "Old host evidence is preserved. No child is cancelled. Start /grants host again after configuration.",
            ))
          )
            return;
          ...
          if (choice === menu[1]) {
            if (
              !(await ctx.ui.confirm(
                "Enable scoped learning retention?",
                "Case/comparison/trust decisions and explicitly selected output artifacts will be retained privately. No model is called; no quality, calibration or authority is invented.",
              ))
            )
              return;
            const root = join(homedir(), ".local", "state", "pi-daddy", "learning");
            await mkdir(root, { recursive: true, mode: 0o700 });
            await privateDirectory(root);
            const instance = join(root, randomUUID());
            await privateDirectory(instance);
            directory = join(instance, "workspace");
            h.createLearningWorkspace(directory, {
              archiveRoot: join(instance, "archive"),
              scopeDigest: learningScopeDigest(declared),
              population: `work:${learningScopeDigest(declared).slice(0, 32)}`,
              author: "local-operator",
            });
          } else {
            const path = await ctx.ui.input("Existing learning workspace directory");
            if (!path) return;
            directory = path.trim();
          }
        ...
        if (choice === menu[5]) {
          await linkLatestWorkOutcome(ctx, declared, connected.workspace, h);
          return;
        }
        if (choice === menu[3]) {
          await workPolicyMenu(ctx, declared, connected.workspace, h);
          return;
        }
        if (choice === menu[4]) {
          if (!host.running)
            throw Error(
              "Start /grants host first; it captures and binds the current work case batch through the existing archive policy",
            );
          await host.choose("refresh-current-work");
          ctx.ui.notify(
            "Current work retained. Open Learning for real case readiness; this is not a defect label.",
            "info",
          );
          return;
        }
        const ui: LearningUI = {
          ...
          input: (title, initial) => ctx.ui.input(title, initial),
          editor: (title, text) => ctx.ui.editor(title, text),
        ...
        if (host.running)
          ctx.ui.notify(
            "Learning saved. If you changed trust configuration, stop/start /grants host explicitly to bind it; attention is not reset.",
            "info",
          );
      } finally {
        busy = false;
