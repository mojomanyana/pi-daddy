...
export async function withOrdinaryChild(
  input: {
  ...
  operation: (signal?: AbortSignal) => Promise<DelegationOutcome>,
) {
  let retained: ReturnType<typeof retainSessionChild>;
  try {
    if (
      input.declaredWork &&
      dataDigest(input.declaredWork.selectedSnapshot) !==
        dataDigest(input.session.declaredWork?.selectedSnapshot ?? null)
    )
      throw Error("declared work changed before original child admission");
    retained = retainSessionChild(
      input.session,
      {
    ...
    return runWithFinalizers(async () => {
      ...
      {
        label: "ordinary admission workspace release",
        run: async () => {
  ...
  try {
    const outcome = await operation(retained?.signal ?? input.signal);
    retained?.settle(
      {
        ok: outcome.ok,
        exitCode: outcome.exitCode,
        aborted: outcome.aborted ?? false,
        outputDigest: dataDigest(outcome.text),
      },
      outcome.control ?? "not-assessed",
      outcome.control === "failed" ? "not-assessed" : (outcome.control ?? "not-assessed"),
    );
    return outcome;
  } catch (error) {
    retained?.settle({ failure: "original executor threw; inspect original caller" }, "unknown");
    throw error;
  }
}
