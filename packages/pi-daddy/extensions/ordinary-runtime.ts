...
export async function withOrdinaryChild(input:{session:GrantsSession;executionId:string;parentExecutionId:string|null;childId:string;toolCallId?:string;signal?:AbortSignal;preparedWorkspace?:PreparedWorkspace;declaredWork?:GrantsSession["declaredWork"]},operation:(signal?:AbortSignal)=>Promise<DelegationOutcome>){
 let retained:ReturnType<typeof retainSessionChild>;
 try{if(input.declaredWork&&dataDigest(input.declaredWork.selectedSnapshot)!==dataDigest(input.session.declaredWork?.selectedSnapshot??null))throw Error("declared work changed before original child admission");retained=retainSessionChild(input.session,{executionId:input.executionId,parentExecutionId:input.parentExecutionId,toolCallId:input.toolCallId??null},input.signal);}
