...
export async function nativeDelegationContext(
  ...
  if (!refused && session.nativeSessionRoot !== undefined) {
    try {
      sessionFile = await allocateNativeSessionTarget(session.nativeSessionRoot, ids.executionId);
    } catch (error) {
      refusal = `native retention target refused (${String(error)})`;
    }
  }
  return {
    extra: {
