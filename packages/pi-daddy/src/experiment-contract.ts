...
export interface ExperimentCharter {
  version: "fixed-experiment-v1"; experimentId: string; orderId: string; budgetDigest: string;
...
export function experimentCharter(input: ExperimentCharter): ExperimentCharter {
  const c = cloneExperiment(input); closed(c, ["version", "experimentId", "orderId", "budgetDigest", "profile", "common", "mode", "deadlineMs", "variants"]);
  if (c.version !== "fixed-experiment-v1" || !id(c.experimentId) || !id(c.orderId) || !hex(c.budgetDigest) || c.profile !== DIGEST_PROFILE || !["bounded-waves", "concurrent-shadow"].includes(c.mode) || !Number.isSafeInteger(c.deadlineMs) || c.deadlineMs < 1000 || c.deadlineMs > 30000 || Buffer.byteLength(intentKey(c)) > 48000) throw new TypeError("unsupported experiment charter/profile");
  ...
  if (!Array.isArray(c.variants) || c.variants.length < 2 || c.variants.length > 32) throw new TypeError("2..32 explicit attempts required");
  ...
  for (const [i, v] of (c.variants as readonly ExperimentVariant[]).entries()) {
    ...
    if (i === 0 ? v.kind !== "primary" || v.parentExecutionId !== null || v.operation !== "digest" : !["shadow", "retry"].includes(v.kind) || !ids.has(v.parentExecutionId!)) throw new TypeError("explicit ordered parent/primary required");
