...
export async function startDailyDashboardHost(input:DailyDashboardHostInput){
 ...
 const workRelative="work.jsonl",factsRelative="facts.jsonl";await writeFile(join(input.directory,workRelative),await readFile(input.declared.ledgerPath),{mode:0o600,flag:"wx"});
 ...
 const requestDigests=new Set<string>(),dispatchDigests=new Set<string>();let host:ReturnType<typeof createDashboardHost>;
 ...
 host=createDashboardHost({harness:input.harness,config,budget,ordinary:input.ordinary,authority,humanActions:async context=>{
   ...
   return [{key,label:state.paused?"Resume new governed dispatch":"Pause new governed dispatch; running children continue",request:hostRequest}];
 }});
