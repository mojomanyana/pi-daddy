...
async function directory(path: string, ownerOnly:boolean): Promise<void> {
  ...
  if (!state.isDirectory() || state.isSymbolicLink() || state.mode & (ownerOnly?0o077:0o022) || state.uid !== process.getuid?.() || await realpath(path) !== path) throw Error(`owned non-writable${ownerOnly?" private":""} directory required: ${path}; existing permissions were not changed`);
}
export async function readProductJson(path: string, limit = 128 * 1024): Promise<unknown | null> {
  try { const state = await lstat(path); if (!state.isFile() || state.isSymbolicLink() || state.mode & 0o077 || state.uid !== process.getuid?.() || state.nlink !== 1 || await realpath(path) !== path) throw Error(`private regular product file required: ${path}`); }
  ...
  const state = await lstat(path);
  if (state.mode & 0o077 || state.uid !== process.getuid?.() || state.nlink !== 1 || await realpath(path) !== path) throw Error(`private regular product file required: ${path}`);
...
export async function writeProductJson(path: string, value: unknown, replace = false): Promise<void> {
  ...
  if (!replace) { await writeFile(path, text, { mode: 0o600, flag: "wx" }); return; }
  const temporary = `${path}.${randomUUID()}.tmp`;
  await runWithFinalizers(async()=>{await writeFile(temporary, text, { mode: 0o600, flag: "wx" }); await rename(temporary, path);},[{label:"product temporary cleanup",run:async()=>{await rm(temporary,{force:true});}}]);
}
