...
export async function closeWorkHandle(handle: FileHandle, failed: boolean): Promise<void> {
  try {
    ...
    if (!failed || !workFsError(error)) throw error;
  }
}
...
export async function appendLedgerLine(
  ...
  once?: { eventId: string; digest: string },
): Promise<void> {
  await mkdir(dirname(options.path), { recursive: true });
  if (!destination) {
    // Retain even legacy path-property evaluation timing across waits; only v4 owns detached inputs.
    await withFileLock(options.path, "grant ledger", () =>
      appendFile(options.path, line, { encoding: "utf8", flag: "a" }),
    );
    return;
  }
      ...
      try {
        ...
        if (once) {
          ...
          if (sameId.length) {
            await handle.sync();
            return;
          } // Reconcile an earlier complete but unacknowledged append.
        }
        if (
          bytes.length + Buffer.byteLength(line, "utf8") > WORK_TEXT_BYTES ||
          ingestion.events.length + 1 > WORK_RECORDS
        ) {
