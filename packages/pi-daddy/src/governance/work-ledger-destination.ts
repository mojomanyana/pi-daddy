...
export function workFsError(error: unknown): error is NodeJS.ErrnoException {
  return (
    ...
    Object.hasOwn(constants.errno, error.code)
  );
}
...
const invalid = (): never => {
  throw new WorkLedgerWriteError("WORK_DESTINATION_INVALID");
};
const alias = (): never => {
  throw new WorkLedgerWriteError("WORK_DESTINATION_ALIAS");
};
const same = (a: BigIntStats | null, b: BigIntStats | null) =>
  a !== null && b !== null && a.dev === b.dev && a.ino === b.ino;
function ancestors(path: string): string[] {
  const result: string[] = [];
  for (let p = dirname(path); ; p = dirname(p)) {
    result.push(p);
    if (dirname(p) === p) return result;
  }
}
interface PathFact {
  path: string;
...
export async function workDestination(requestedPath: string, grantLedgerPath: string | null): Promise<WorkDestination> {
  ...
  const path = filesystemPath(requestedPath),
    leaves = [path, path + ".lock"];
  const protectedPaths =
    grantLedgerPath === null ? [] : [filesystemPath(grantLedgerPath), filesystemPath(grantLedgerPath) + ".lock"];
  ...
  function probe(path: string): Promise<PathFact> {
    const prior = facts.get(path);
    if (prior) return prior;
    const pending = inspect(path);
  ...
  async function inspect(path: string): Promise<PathFact> {
    ...
    try {
      fact.entry = await lstat(path, { bigint: true });
    } catch (error) {
      if (!workFsError(error)) throw error;
      if (error.code !== "ENOENT") {
        fact.invalid = ["ENOTDIR", "ELOOP"].includes(error.code!);
        fact.io = !fact.invalid;
        return fact;
      }
    }
    if (fact.entry) {
      try {
        ...
        if (!workFsError(error)) throw error;
        fact.invalid = ["ENOENT", "ENOTDIR", "ELOOP"].includes(error.code!);
        fact.io = !fact.invalid;
      }
    } else {
      ...
      fact.invalid = parent.invalid || (parent.target !== null && !parent.target.isDirectory());
      fact.io = parent.io;
  ...
  for (const fact of initial)
    if (fact.canonical) for (const p of [fact.canonical, ...ancestors(fact.canonical)]) workPaths.add(p);
  const workFacts = await Promise.all([...workPaths].map(probe));
  for (const w of workFacts)
    for (const p of protectedFacts) {
      if (w.path === p.path || (w.canonical !== null && w.canonical === p.canonical) || same(w.target, p.target))
        alias();
    }
