...
function capabilityMap(value: unknown, valid: (entry: unknown) => boolean): boolean {
  return (
    isLedgerObject(value) && Object.keys(value).every(isLedgerCapabilityIdentifier) && Object.values(value).every(valid)
  );
}
...
function validRefusal(value: unknown): boolean {
  if (!isLedgerObject(value) || Object.keys(value).some((key) => !["code", "message", "details"].includes(key)))
    return false;
  ...
  return optional(
    ...
    "details",
    (details) =>
      isLedgerObject(details) &&
      Object.values(details).every((entry) => entry === null || ["string", "number", "boolean"].includes(typeof entry)),
  );
}
...
function validApprovalUse(value: unknown): boolean {
  return (
    isLedgerObject(value) &&
    Object.keys(value).every((key) => ["max", "remaining"].includes(key)) &&
...
function validateCapabilityDecision(event: LedgerV3Object): string | null {
  ...
  if (
    ...
    !EXECUTORS.has(String(event.executor)) ||
    !SHA256_RE.test(String(event.taskDigest))
  ) {
    return "capability decision required fields are invalid";
  }
  if (
    !optional(event, "agentType", isLedgerDisplayIdentifier) ||
    ...
    !optional(event, "approvalSources", (value) =>
      capabilityMap(value, (entry) => APPROVAL_SOURCES.has(String(entry))),
    ) ||
    !optional(event, "approvalScopes", (value) =>
      capabilityMap(value, (entry) => APPROVAL_SCOPES.has(String(entry))),
    ) ||
...
function validateWorkspaceLease(event: LedgerV3Object): string | null {
  if (
    ...
    !isNonEmptyString(event.root) ||
    !["read", "write"].includes(String(event.access)) ||
    !LEASE_OUTCOMES.has(String(event.outcome))
  ) {
    return "workspace lease required fields are invalid";
  }
  if (
    !optional(event, "recovered", (value) => value === true || value === false || value === "unknown") ||
