...
import {
  WorkInputError,
  type WorkAcceptanceEvent,
  type WorkAcceptancePayload,
  type WorkDiagnostic,
  type WorkFrozen,
  type WorkIngestion,
  ...
  type WorkLedgerInspection,
  type WorkInspectionDiagnostic,
...
function build(
  args: unknown,
  kind: WorkEventKind,
  field: "revision" | "snapshot" | "payload",
): WorkFrozen<WorkLedgerEvent> {
  ...
  if (
    ...
    Reflect.ownKeys(now).length !== 0 ||
    !Number.isFinite(Date.prototype.getTime.call(now))
  )
    throw new WorkInputError("WORK_SCHEMA_INVALID");
  const content = copyWorkJson(fields[field].value);
  const candidate = copyWorkJson({
    ledgerVersion: 4,
    event: kind,
    eventId: fields.eventId.value,
    ts: Date.prototype.toISOString.call(now),
    payload: field === "payload" ? content : { [field]: content },
  });
  const event = validateWorkEvent(candidate, true);
  if (Buffer.byteLength(canonicalWorkJson(event), "utf8") > WORK_EVENT_BYTES)
    throw new WorkInputError("WORK_LIMIT_EXCEEDED");
  return freezeWork(event);
}

export function buildWorkRevisionEvent(args: {
  eventId: string;
  now: Date;
  revision: Omit<WorkRevision, "digest">;
}): WorkFrozen<WorkRevisionEvent> {
  return build(args, "work_revision", "revision") as WorkFrozen<WorkRevisionEvent>;
}
export function buildWorkSnapshotEvent(args: {
  eventId: string;
  now: Date;
  snapshot: Omit<WorkSnapshot, "digest">;
}): WorkFrozen<WorkSnapshotEvent> {
  return build(args, "work_snapshot", "snapshot") as WorkFrozen<WorkSnapshotEvent>;
}
export function buildWorkOccurrenceEvent(args: {
  eventId: string;
  now: Date;
  payload: WorkOccurrencePayload;
}): WorkFrozen<WorkOccurrenceEvent> {
  return build(args, "work_occurrence", "payload") as WorkFrozen<WorkOccurrenceEvent>;
}
export function buildWorkAcceptanceEvent(args: {
  eventId: string;
  now: Date;
  payload: WorkAcceptancePayload;
}): WorkFrozen<WorkAcceptanceEvent> {
...
export function parseWorkLedgerText(text: string): WorkFrozen<WorkIngestion> {
  const wholeError = (code: WorkDiagnostic["code"]) =>
    freezeWork({ events: [], errors: [{ line: null, code }], complete: false });
  ...
  for (const row of lines(text))
    if (!blank(row.text) && ++records > WORK_RECORDS) return wholeError("WORK_LIMIT_EXCEEDED");
  const events: WorkLedgerEvent[] = [],
    errors: WorkDiagnostic[] = [];
  for (const row of lines(text)) {
    if (blank(row.text)) continue;
    try {
      events.push(validateWorkEvent(parseWorkJson(row.text)));
    } catch (error) {
...
export function projectWorkLedger(
  text: string,
  context: WorkFrozen<WorkProjectionContext> = { selectedSnapshot: null, authority: null },
): WorkFrozen<WorkProjection> {
  ...
  errors.sort(
    (a, b) => (a.line ?? Infinity) - (b.line ?? Infinity) || (a.code < b.code ? -1 : a.code > b.code ? 1 : 0),
  );
  const conflicts = mergeWorkConflicts([
    ...index.conflictingEvents.map((ref) => ({
      kind: "event" as const,
      id: ref.eventId,
      digests: [ref.digest],
      affectedObligations: [],
  ...
  const identities = {
    selectedSnapshot: copied?.selectedSnapshot?.snapshot ?? null,
    authoritySnapshot: copied?.authority?.snapshot ?? null,
  };
  function empty(
    scopeState: WorkProjection["scopeState"],
    problems: readonly WorkFrozen<WorkProblem>[],
    retained: readonly WorkFrozen<WorkConflict>[] = conflicts,
  ) {
    return freezeWork({
      ...identities,
      scopeState,
...
export function appendWorkLedgerEvent(
  options: { path: string; grantLedgerPath: string | null },
  event: WorkFrozen<WorkLedgerEvent>,
): Promise<void> {
...
export function appendWorkLedgerEventOnce(
  options: { path: string; grantLedgerPath: string | null },
  event: WorkFrozen<WorkLedgerEvent>,
): Promise<void> {
...
async function appendWorkEvent(
  ...
  if (
    ...
    !Object.hasOwn(fields, "grantLedgerPath") ||
    typeof fields.path.value !== "string" ||
    (fields.grantLedgerPath.value !== null && typeof fields.grantLedgerPath.value !== "string")
  )
    throw new WorkInputError("WORK_SCHEMA_INVALID");
  const path: string = fields.path.value,
    grant: string | null = fields.grantLedgerPath.value;
  ...
  try {
    ...
    await appendLedgerLine(
      ...
      canonical + "\n",
      destination,
      once ? { eventId: candidate.eventId, digest: candidate.digest } : undefined,
    );
  } catch (error) {
    if (error instanceof LockTimeoutError || workFsError(error))
      throw new WorkLedgerWriteError("WORK_LEDGER_WRITE_FAILED");
    throw error;
  }
}

/** Read-only regular-file inspection; a successful read is not a content-validity or acceptance claim. */
export async function inspectWorkLedger(
  options: { version: 4; path: string },
  context: WorkFrozen<WorkProjectionContext> = { selectedSnapshot: null, authority: null },
): Promise<WorkFrozen<WorkLedgerInspection>> {
  let exists: boolean | null = null;
  const failure = (code: WorkInspectionDiagnostic["code"]): WorkFrozen<WorkLedgerInspection> =>
    freezeWork({
      version: 4,
      status: "error",
      exists,
  ...
  const missing = (): WorkFrozen<WorkLedgerInspection> =>
    freezeWork({ version: 4, status: "missing", exists: false, ingestion: null, projection: null, errors: [] });
  ...
  try {
    ...
    try {
      const value = copyWorkJson(context);
      validateWorkContext(value);
      copied = value as unknown as WorkFrozen<WorkProjectionContext>;
    } catch (error) {
      if (error instanceof WorkInputError) throw new WorkInputError("WORK_CONTEXT_INVALID");
      throw error;
    }
  } catch (error) {
  ...
  try {
    ...
    try {
      ...
      await closeWorkHandle(handle, failed);
    }
    ...
    return freezeWork({
      ...
      projection: projectWorkLedger(text, copied),
      errors: [],
    });
  } catch (error) {
    if (error instanceof WorkInputError) return failure(error.code);
