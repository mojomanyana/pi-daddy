...
import {
  ...
  freezeWork,
  workDigest,
  workResultKey as key,
  sortWorkResults as unique,
  mergeWorkProblems,
  mergeWorkConflicts,
...
import type { resolveWorkSnapshotText } from "./work-ledger-snapshot.ts";
import type { foldWorkOccurrences } from "./work-ledger-occurrences.ts";
...
const revisionIdentity = ({ kind, id, revision, digest }: WorkFrozen<RevisionRef>) =>
  key(["revision", kind, id, revision, digest]);
...
function nestedIdentity(event: IndexedEvent): string | null {
  switch (event.event) {
    ...
    case "work_snapshot":
      return key(["snapshot", event.payload.snapshot.snapshotId, event.payload.snapshot.digest]);
    case "work_occurrence":
      return key(["occurrence", event.payload]);
...
function sourceIds(index: Sources, event: IndexedEvent | WorkFrozen<EventRef>): readonly string[] {
  ...
  return [
    ...(index.conflictSources.events[key(event.eventId)] ? [event.eventId] : []),
...
export function workEventConflictSources(
  index: Sources,
  event: IndexedEvent | WorkFrozen<EventRef>,
): WorkFrozen<EventRef>[] {
...
export function workRevisionConflictSources(
  index: Sources,
  ref: WorkFrozen<RevisionRef>,
): readonly WorkFrozen<EventRef>[] {
  return unique(
    (index.conflictSources.bodies[revisionIdentity(ref)] ?? []).flatMap((id) => index.conflictSources.events[key(id)]),
  );
}
...
function sorted<T>(values: Iterable<T>): T[] {
  ...
  return [...values]
    .map((value) => ({ value, key: canonicalWorkJson(value) }))
...
export function indexWorkLedgerText(text: string): WorkReplayIndex {
  ...
  const bodies = new Map<string, Set<string>>(),
    eventGroups = new Map<string, EventRef[]>();
  ...
  const conflictSources = {
    bodies: Object.fromEntries(byKey(bodies).map(([identity, ids]) => [identity, [...ids].sort()])),
  ...
  const effectiveEvents = ingestion.complete
    ? candidates.filter((event) => !sourceIds({ conflictSources }, event).length)
    : [];
  return freezeWork({
    ingestion,
    candidates,
...
export function indexWorkReceipts(authority: WorkFrozen<WorkProjectionContext>["authority"]) {
  ...
  for (const [id, group] of receipts)
    if (group.size > 1) {
...
export function projectWorkAcceptance(
  index: WorkReplayIndex,
  structure: ReturnType<typeof resolveWorkSnapshotText>,
  authority: WorkFrozen<WorkProjectionContext>["authority"],
  runtime: ReturnType<typeof foldWorkOccurrences>,
) {
  const snapshot = structure.snapshot!;
  const claims: WorkFrozen<WorkClaimResult>[] = [],
    obligations: WorkFrozen<WorkObligationResult>[] = [];
  ...
  const receiptIndex = indexWorkReceipts(authority),
  ...
  function available(kind: "artifact" | "evidence", id: string, digest: string): WorkFrozen<WorkCoverageItem> {
    const values = unique(
      (authority?.availability ?? [])
        .filter((row) => row.kind === kind && row.id === id && row.digest === digest)
        .map((row) => row.available),
    );
    const state: WorkAvailability =
      values.length > 1 ? "conflicted" : values.length === 0 ? "unknown" : values[0] ? "available" : "unavailable";
  ...
  for (const row of structure.obligations) {
    const binding = row.binding,
      affected = [binding.obligation];
    const problem = (code: WorkProblem["code"], reference: WorkFrozen<WorkReference> | null): Problem => ({
      code,
      reference,
    ...
    const selectedClaims = allClaims.filter(
      (e) =>
        e.payload.binding.scope.id === snapshot.scope.id && e.payload.binding.obligation.id === binding.obligation.id,
    );
    const applicable = (claim: Claim) => {
      const b = claim.payload.binding;
      return (
        key(b.snapshot) === key(structure.selectedSnapshot) &&
        key(b.scope) === key(snapshot.scope) &&
        key(b.intent) === key(binding.intent) &&
        key(b.obligation) === key(binding.obligation) &&
        key(b.artifact) === key(binding.artifact) &&
        key(b.policy) === key(binding.policy)
      );
    };
    ...
    for (const e of current)
      if (eventConflicts.has(e.eventId)) {
        blockers.push(
          ...index.conflictingEvents
            .filter((ref) => ref.eventId === e.eventId)
            .map((ref) => problem("EVENT_CONFLICT", { type: "event", ref })),
        );
      }
    for (const [id, group] of receipts)
      if (group.size > 1 && [...group.values()].some((d) => currentRefs.has(key(d.claim)))) {
    ...
    const artifactCoverage: WorkFrozen<WorkCoverage> =
      binding.artifact === null ? { state: "unselected", items: [] } : coverage(artifactItems);
    const evidenceCoverage = coverage(
      unique(
        current.flatMap((e) => e.payload.binding.evidence.map((ref) => available("evidence", ref.id, ref.digest))),
      ),
    );
    ...
    for (const claim of selectedClaims) {
      ...
      if (row.artifact && !bytesMatch)
        local.push(
          problem("ARTIFACT_DIGEST_MISMATCH", {
            type: "artifact-bytes",
            ref: { id: row.artifact.id, digest: b.artifactDigest },
          }),
        );
      ...
      const matching = decisions.filter(
        (d) =>
          bytesMatch &&
          key(d.claim) === key(eventRef(claim)) &&
          d.authorityId === claim.payload.authorityId &&
          key(d.binding) === key(b),
      );
      ...
      if (!authority) local.push(problem("AUTHORITY_MISSING", null));
        ...
        local.push(
          problem(
            decisions.some((d) => d.claim.eventId === claim.eventId) ? "RECEIPT_MISMATCH" : "RECEIPT_MISSING",
            eventReference(claim),
          ),
        );
      const support = b.evidence.flatMap((ref) => (ref.event ? sourceProblems(ref.event, affected) : []));
      const availabilityProblems = coverageProblems([
        ...artifactItems,
        ...b.evidence.map((ref) => available("evidence", ref.id, ref.digest)),
      ]);
      ...
      rowClaims.push({
        ...
        applicability: rejection
          ? "unaccepted"
          : matching.some((d) => d.decision === "accept") && local.length === 0
    ...
    for (const p of blocked)
      if (p.code === "EVENT_CONFLICT" && p.reference?.type === "event") {
        ...
        conflicts.push({
          kind: "event",
          id,
          digests: index.conflictingEvents.filter((e) => e.eventId === id).map((e) => e.digest),
          affectedObligations: affected,
        });
      }
    const finalClaims = rowClaims.map((c) =>
      blocked.length && c.applicability !== "superseded"
        ? { ...c, applicability: "unresolved" as const, problems: blocked }
        : c,
    );
    ...
    const acceptance = blocked.length
      ? "unresolved"
      : accepted
        ? "accepted-under-supplied-authority"
        : rejected
          ...
          : unresolved.length
            ? "unresolved"
            : "unaccepted";
    const obligationProblems = blocked.length
      ? blocked
      : accepted
        ? []
        : rejected
          ? finalClaims.filter((c) => c.applicability === "unaccepted").flatMap((c) => c.problems)
          : unresolved.length
            ? unresolved.flatMap((c) => c.problems)
    ...
    obligations.push({
      ...
      artifactCoverage,
      evidenceCoverage,
      claims: unique(finalClaims.map((c) => c.claim)),
      problems: mergeWorkProblems(obligationProblems),
    });
  ...
  return {
    ...
    progress: {
      accepted: obligations.filter((o) => o.acceptance === "accepted-under-supplied-authority").length,
