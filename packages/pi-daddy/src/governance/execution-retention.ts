...
export interface RetentionStatus {
  ...
  status: "disabled" | "pending" | "retained" | "lost";
}
...
export function beginExecutionRetention(identity: RetentionIdentity, directory = process.env[ENV_EXECUTION_ARCHIVE]) {
    ...
    try {
      ...
      while (dirty) {
        ...
        await writeFile(
          ...
          JSON.stringify(buildExecutionRetentionManifest(checkpoint)) + "\n",
          { mode: 0o600 },
        );
  ...
  const observeSession = (reference: NonNullable<typeof nextNative>) => {
    ...
    if (nextNative && (nextNative.source !== reference.source || nextNative.value !== reference.value))
      loss("native-session-reference-coalesced");
    nextNative = {
    ...
    nativeTask = Promise.resolve()
      .then(async () => {
        while (nextNative) {
          const current = nextNative;
          nextNative = undefined;
          const captured: NativeSessionCapture =
            current.source === "herdr-id"
              ? {
                  observation: {
                    ...missingNativeSession(),
          ...
          if (pinnedSessionId && observation.sessionId && pinnedSessionId !== observation.sessionId) {
            observation = {
              ...
              source: current.source,
              status: "changed",
              reason: "native-session-id-changed",
            };
          }
      ...
      .catch(() => {
        loss("native-session-observation-failed");
      })
      .finally(() => {
  ...
  return {
    ...
    observeSessionManager(manager: NativeSessionManager) {
      try {
        ...
        if (value) observeSession({ source: "pi-session-manager", value, manager });
          ...
          loss("native-session-not-persisted");
          schedule();
        }
      } catch {
        loss("native-session-manager-unavailable");
        schedule();
      }
    },
    status: (): RetentionStatus => {
      const value = { manifestPath, status };
      drains.set(value, drain);
      return value;
    },
    native(value: Partial<Pick<ExecutionRetentionManifest["native"], "pid" | "paneId" | "agentName" | "tabId">>) {
      ...
      for (const key of ["pid", "paneId", "agentName", "tabId"] as const) {
        ...
        if (key === "pid") {
          if (typeof item === "number" && Number.isSafeInteger(item) && item > 0) manifest.native.pid = item;
        } else if (typeof item === "string" && item.length <= 4096) manifest.native[key] = item;
      }
      schedule();
    },
    capture(
      kind: "stdout" | "stderr" | "paneSnapshot" | "checkReceipt" | "result",
      bytes: Uint8Array,
      replace = false,
    ) {
      ...
      if (indivisible && bytes.length > available) {
        loss(`${kind === "checkReceipt" ? "check-receipt" : kind}-not-retained-capacity`);
        schedule();
        return;
      }
...
export function verifyRetainedBytes(
  reference: RetainedContent,
  bytes?: Uint8Array,
): "retained" | "missing" | "mismatch" {
