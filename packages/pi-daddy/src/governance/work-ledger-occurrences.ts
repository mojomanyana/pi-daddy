import { workResultKey as key, sortWorkResults as sorted, mergeWorkProblems } from "./work-ledger-json.ts";
import { workEventConflictSources, type indexWorkLedgerText } from "./work-ledger-projection.ts";
import type {
  WorkFrozen,
  WorkSnapshot,
  WorkOccurrenceEvent,
  WorkOccurrencePayload,
  WorkAttempt,
  WorkAttemptBinding,
  WorkRuntime,
  WorkProblem,
...
const emptyLabels = (): WorkOccurrencePayload["labels"] => ({
  sessionId: null,
  branchLeafId: null,
  toolCallId: null,
  taskId: null,
...
export function foldWorkOccurrences(index: ReturnType<typeof indexWorkLedgerText>, snapshot: WorkFrozen<WorkSnapshot>) {
  ...
  const effective = new Map(
    [...groups].map(([id, group]) => [id, group.filter((e) => effectiveRefs.has(key(eventRef(e))))]),
  );
  const selectedObligations = new Set(snapshot.bindings.map((b) => key(b.obligation)));
  const selectedJoin = (e: Occurrence) =>
    key(e.payload.scope) === key(snapshot.scope) && selectedObligations.has(key(e.payload.obligation));
  const attempts: WorkFrozen<WorkAttempt>[] = [],
    occurrences: WorkFrozen<WorkRuntime>["occurrences"][number][] = [];
  const allProblems: Problem[] = [],
    conflicts: WorkFrozen<WorkConflict>[] = [];
  ...
  for (const [executionId, candidates] of groups) {
    ...
    const problem = (code: WorkProblem["code"], id = executionId) =>
      problems.push({ code, reference: { type: "execution", id }, affectedObligations: affected });
    ...
    if (bindings.size)
      for (const id of sorted(badSources.map((ref) => ref.eventId))) {
        conflicts.push({
          kind: "event",
          id,
          digests: index.conflictingEvents.filter((ref) => ref.eventId === id).map((ref) => ref.digest),
    ...
    const declaredLabels = sorted(
      clean.filter((e) => e.payload.provenance === "declared").map((e) => e.payload.labels),
    );
    ...
    for (const label of Object.keys(effectiveLabels) as Array<keyof WorkOccurrencePayload["labels"]>) {
      const values = sorted(
        observed.flatMap((e) => (e.payload.labels[label] === null ? [] : [e.payload.labels[label]!])),
      );
    ...
    let state: WorkAttempt["state"] = terminalConflict
      ? "conflicted"
    ...
    attempts.push({
      executionId,
      bindings: sorted(bindings.values()),
      parentExecutionId,
      childId,
      declaredLabels,
      observedLabels,
  ...
  const runtime: WorkFrozen<WorkRuntime> = {
