 ...
 *    (`src/governance/file-lock.ts`, `underLock` below).
 ...
 *
 * The instance this closes: `taskAtApproval` stored the model-authored task string, which `src/governance/ledger.ts`
 ...
 * because the lock already existed for the ledger — `src/governance/file-lock.ts`, one implementation, two callers.
 *
