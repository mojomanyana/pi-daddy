...
import { canonicalWorkJson, workDigest, type WorkJson } from "./work-ledger-json.ts";
import { WorkInputError, type WorkLedgerEvent } from "./work-ledger-types.ts";
...
function id(value: WorkJson): void {
  require(typeof value === "string" && /^[A-Za-z0-9][A-Za-z0-9._:/@+\-]{0,127}$/.test(value));
}
function digest(value: WorkJson): void {
  require(typeof value === "string" && /^[0-9a-f]{64}$/.test(value));
}
function integer(value: WorkJson): void {
  require(typeof value === "number" && Number.isSafeInteger(value) && value >= 1);
}
function member(value: WorkJson, values: string[]): void {
  require(typeof value === "string" && values.includes(value));
}
function nullable(value: WorkJson, validate: (value: WorkJson) => void): void {
...
function identity(value: WorkJson): void {
  const ref = closed(value, ["id", "digest"]);
  id(ref.id);
  digest(ref.digest);
}
function eventRef(value: WorkJson): void {
  const ref = closed(value, ["eventId", "digest"]);
  id(ref.eventId);
  digest(ref.digest);
}
...
function set(
  value: WorkJson,
  validate: (item: WorkJson) => void,
  building: boolean,
  key: (item: WorkJson) => string = canonicalWorkJson,
): void {
...
function ownKeys(keys: string[], building: boolean): string[] {
  return building ? keys : [...keys, "digest"];
}
...
function revision(value: WorkJson, building: boolean): void {
  ...
  if (r.kind === "artifact" || r.kind === "policy") {
    require(
      r.parent === null && noDependencies && r.policy === null && (r.permittedEffects as WorkJson[]).length === 0,
    );
  }
...
function obligationBinding(value: WorkJson): void {
  ...
  nullable(b.artifact, (v) => reference(v, ["artifact"]));
...
function occurrence(value: WorkJson): void {
  ...
  nullable(p.childId, id);
  nullable(p.variantId, id);
  nullable(p.artifact, (v) => reference(v, ["artifact"]));
  member(p.provenance, ["declared", "observed"]);
  member(p.state, ["unknown", "starting", "running", "completed", "failed"]);
  const labels = closed(p.labels, [
    "sessionId",
    "branchLeafId",
    "toolCallId",
    "taskId",
...
function evidence(value: WorkJson): void {
  ...
  id(e.id);
  digest(e.digest);
  nullable(e.event, eventRef);
}
function acceptance(value: WorkJson, building: boolean): void {
  const p = closed(value, ["authorityId", "binding"]);
  id(p.authorityId);
...
function acceptanceBinding(value: WorkJson, building: boolean): void {
  ...
  reference(b.policy, ["policy"]);
  set(b.evidence, evidence, building);
  require((b.evidence as WorkJson[]).length > 0);
}
...
export function validateWorkContext(value: WorkJson): void {
  ...
  identity(a.snapshot);
  require(Array.isArray(a.decisions));
  require(Array.isArray(a.availability));
...
export function validateWorkEvent(value: WorkJson, building = false): WorkLedgerEvent {
  ...
  if (e.event === "work_revision") {
    nested = workObject(closed(e.payload, ["revision"]).revision);
    revision(nested, building);
  } else if (e.event === "work_snapshot") {
    nested = workObject(closed(e.payload, ["snapshot"]).snapshot);
