...
const closed = (properties: Record<string, object>) => ({
  type: "object",
  properties,
...
export const RETENTION_CONTENT_KINDS = [
  "stdout",
  "stderr",
  "paneSnapshot",
  "checkReceipt",
...
export const RETENTION_SCHEMA = freeze({
  ...
  ...closed({
    ...
    identity: closed({
      executionId: string(),
      parentExecutionId: nullable(string()),
      childId: string(),
    ...
    native: closed({
      ...
      paneId: nullable(string(4096)),
      agentName: nullable(string(4096)),
      tabId: nullable(string(4096)),
    ...
    nativeSession: closed({
      ...
      sessionId: nullable(uuid),
      sessionPath: nullable(string(4096)),
      parentSessionPath: nullable(string(4096)),
      branchLeafId: nullable(string(128)),
      branchState: { enum: ["unknown", "observed"] },
      lastPersistedEntryId: nullable(string(128)),
    ...
    content: closed(
      Object.fromEntries(
        RETENTION_CONTENT_KINDS.map((kind) => [
          kind,
          {
            oneOf: [
    ...
    coverage: closed({
      ...
      losses: { type: "array", items: string(), maxItems: 64, uniqueItems: true },
    }),
  ...
  allOf: [
    ...
    },
    {
      ...
      then: {
        properties: {
          nativeSession: { properties: { branchLeafId: literal(null) } },
          native: { properties: { branchLeafId: literal(null) } },
        },
      },
    },
  ],
...
export function buildExecutionRetentionManifest(value: unknown): ExecutionRetentionManifest {
  ...
  const inspect = (x: unknown, depth: number): void => {
    ...
    if (
      !x ||
      typeof x !== "object" ||
      (!Array.isArray(x) && Object.getPrototypeOf(x) !== Object.prototype && Object.getPrototypeOf(x) !== null)
    )
    ...
    if (
      ...
      (Object.getPrototypeOf(x) !== Array.prototype ||
        ...
        Array.from({ length: x.length }, (_, i) => String(i)).some((key) => !Object.hasOwn(x, key)))
    )
  ...
  if (
    ...
    m.native.sessionPath !== m.nativeSession.sessionPath ||
    m.native.branchLeafId !== m.nativeSession.branchLeafId
  ) {
  ...
  if (
    ...
    (!m.nativeSession.sessionId || !m.nativeSession.sha256 || m.content.session.sha256 !== m.nativeSession.sha256)
  )
