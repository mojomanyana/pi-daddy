import type { WorkFrozen } from "../governance/work-ledger-types.ts";
...
export function closed(value: unknown, keys: string[]): asserts value is Record<string, unknown> {
  if (!value || typeof value !== "object" || Object.keys(value).sort().join() !== keys.sort().join())
    throw new TypeError("closed experiment data required");
}
...
export interface ExperimentVariant {
  variantId: string;
  executionId: string;
  kind: "primary" | "shadow" | "retry";
  parentExecutionId: string | null;
  suffixBase64: string;
  operation: "digest" | "hold";
...
export interface ExperimentCharter {
  ...
  orderId: string;
  budgetDigest: string;
  profile: typeof DIGEST_PROFILE;
  common: { sha256: string; bytes: number; work: WorkFrozen<WorkIntentBinding> | null; workTextDigest: string | null };
  mode: "bounded-waves" | "concurrent-shadow";
  deadlineMs: number;
  variants: readonly ExperimentVariant[];
}
export function experimentCharter(input: ExperimentCharter): ExperimentCharter {
  const c = cloneExperiment(input);
  closed(c, [
    "version",
    "experimentId",
    "orderId",
    ...
    "deadlineMs",
    "variants",
    ...(c.version === "fixed-experiment-v2" ? ["order"] : []),
  ]);
  if (
    !["fixed-experiment-v1", "fixed-experiment-v2"].includes(c.version) ||
    !id(c.experimentId) ||
    !id(c.orderId) ||
    !hex(c.budgetDigest) ||
    c.profile !== DIGEST_PROFILE ||
    !["bounded-waves", "concurrent-shadow"].includes(c.mode) ||
    !Number.isSafeInteger(c.deadlineMs) ||
    c.deadlineMs < 1000 ||
    c.deadlineMs > 30000 ||
    Buffer.byteLength(intentKey(c)) > 48000
  )
    throw new TypeError("unsupported experiment charter/profile");
  closed(c.common, ["sha256", "bytes", "work", "workTextDigest"]);
  if (!hex(c.common.sha256) || !Number.isSafeInteger(c.common.bytes) || c.common.bytes < 0 || c.common.bytes > 16384)
    throw new TypeError("bounded common input required");
  if (c.common.work !== null) {
    c.common.work = workIntentBinding(JSON.parse(intentKey(c.common.work)));
    if (!hex(c.common.workTextDigest)) throw new TypeError("exact work bytes required");
  } else if (c.common.workTextDigest !== null) throw new TypeError("unexpected work digest");
  if (
    !Array.isArray(c.variants) ||
    c.variants.length < (c.version === "fixed-experiment-v2" ? 1 : 2) ||
    c.variants.length > 32
  )
    throw new TypeError("2..32 explicit attempts required");
  ...
  for (const [i, v] of (c.variants as readonly ExperimentVariant[]).entries()) {
    ...
    if (
      !id(v.variantId) ||
      !id(v.executionId) ||
      ids.has(v.executionId) ||
      variants.has(v.variantId) ||
      !["digest", "hold"].includes(v.operation) ||
      Object.values(v.configuration).some((x) => x !== null)
    )
      throw new TypeError("unknown/general configuration cannot execute");
    if (
      c.version === "fixed-experiment-v2"
        ? !["primary", "retry"].includes(v.kind) ||
          (v.kind === "primary" ? v.parentExecutionId !== null : !ids.has(v.parentExecutionId!))
        : i === 0
          ? v.kind !== "primary" || v.parentExecutionId !== null || v.operation !== "digest"
          : !["shadow", "retry"].includes(v.kind) || !ids.has(v.parentExecutionId!)
    )
      throw new TypeError("explicit ordered parent/primary required");
    if (
      typeof v.suffixBase64 !== "string" ||
      v.suffixBase64.length > 22000 ||
      Buffer.from(v.suffixBase64, "base64").toString("base64") !== v.suffixBase64 ||
      c.common.bytes + Buffer.from(v.suffixBase64, "base64").length > 16384
    )
      throw new TypeError("invalid bounded byte difference");
    ids.add(v.executionId);
    variants.add(v.variantId);
  }
...
export const parseExperimentCharter = (text: string) =>
  experimentCharter(parseWorkJson(text) as unknown as ExperimentCharter);
export const experimentCharterDigest = (c: ExperimentCharter) => experimentHash(experimentCharter(c));
export interface ExperimentAuthority {
  authorityDigest: string;
  charterDigests: readonly string[];
...
export function experimentAuthority(value: ExperimentAuthority | null): ExperimentAuthority | null {
  ...
  closed(a, ["authorityDigest", "charterDigests", "cancellationDigests"]);
  if (
    !hex(a.authorityDigest) ||
    [a.charterDigests, a.cancellationDigests].some(
      (xs) => !Array.isArray(xs) || xs.length > 128 || xs.some((x) => !hex(x)),
    )
  )
    throw new TypeError("independent bounded host approvals required");
  return freezeWork(a);
}
...
export function experimentCancellation(value: ExperimentCancellation): ExperimentCancellation {
  ...
  closed(c, ["version", "requestId", "bindingDigest", "executionId"]);
  if (c.version !== "experiment-cancel-v1" || !id(c.requestId) || !hex(c.bindingDigest) || !id(c.executionId))
    throw new TypeError("exact cancellation required");
  return freezeWork(c);
}
