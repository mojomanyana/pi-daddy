...
export interface WorkTaskSetup {
  ...
  agent: string | null;
  model: string;
  thinking: (typeof WORK_EFFORTS)[number];
  dependencies: string[];
}
...
export interface RecordedWorkSetup {
  version: "recorded-work-setup-v1";
  setup: WorkSetup;
  state: DeclaredWorkState;
  tasks: { id: string; obligation: RevisionRef }[];
  events: WorkFrozen<WorkRevisionEvent | WorkSnapshotEvent>[];
}
...
const keys = (value: object, expected: string[]) => {
  if (!value || typeof value !== "object" || Object.keys(value).sort().join() !== expected.sort().join())
    throw Error("unsupported work setup fields");
};
const id = (value: unknown) => typeof value === "string" && /^[A-Za-z0-9][A-Za-z0-9_-]{0,31}$/.test(value);
const text = (value: unknown) =>
  typeof value === "string" && value.trim().length > 0 && Buffer.byteLength(value) <= 8192;
const ref = (r: Pick<WorkRevision, "kind" | "id" | "revision" | "digest">): RevisionRef => ({
  kind: r.kind,
  id: r.id,
  revision: r.revision,
  digest: r.digest,
...
export function workSetup(input: WorkSetup): WorkSetup {
  ...
  if (
    s.version !== "work-setup-v1" ||
    !id(s.id) ||
    !text(s.outcome) ||
    !Number.isInteger(s.maxParallel) ||
    s.maxParallel < 1 ||
    s.maxParallel > 8 ||
    ...
    throw Error("work setup requires 1–8 tasks and parallel limit 1–8");
  ...
  for (const task of s.tasks) {
    ...
    if (
      ...
      !(
        task.agent === null ||
        (typeof task.agent === "string" && /^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$/.test(task.agent))
      ) ||
      typeof task.model !== "string" ||
      !/^[A-Za-z0-9._-]+\/[A-Za-z0-9._:/@+-]+$/.test(task.model) ||
      task.model.length > 128 ||
      !WORK_EFFORTS.includes(task.thinking) ||
      !Array.isArray(task.dependencies) ||
      new Set(task.dependencies).size !== task.dependencies.length ||
      task.dependencies.some((d) => !id(d) || d === task.id)
    )
      throw Error("invalid task, model/effort or dependency; no profile fallback");
    seen.add(task.id);
  }
...
const setupPath = (state: DeclaredWorkState) =>
  join(dirname(state.statePath), "work-setups", `${state.selectedSnapshot.snapshot.digest}.json`);

/** Author first, append through P01, then select separately at an explicit owner boundary. */
export async function recordWorkSetup(
  cwd: string,
  value: WorkSetup,
  mode: "new" | "revise" | "alternative" = "new",
  ...
  return withFileLock(statePath, "work setup", async () => {
    ...
    if (intentKey(current?.selectedSnapshot ?? null) !== intentKey(expected?.selectedSnapshot ?? null))
      throw Error("work selection changed; reopen setup");
    ...
    if (
      mode !== "new" &&
      (!original ||
        original.setup.tasks.length !== setup.tasks.length ||
        original.setup.tasks.some(
          (t) =>
            !setup.tasks.some(
              (n) => n.id === t.id && intentKey([...n.dependencies].sort()) === intentKey([...t.dependencies].sort()),
            ),
        ))
    )
      throw Error("topology changes require a new work setup; no live entity expansion");
    const revisions =
      current && mode !== "new"
        ? resolveWorkSnapshotText(await readFile(current.ledgerPath, "utf8"), current.selectedSnapshot)
        : null;
    const token = hash(intentKey({ setup, mode, from: mode === "new" ? null : current?.selectedSnapshot })).slice(
        0,
        24,
      ),
      prefix = mode === "new" ? `work:${setup.id}:${token.slice(0, 8)}` : current!.scope.id.replace(/:scope$/, "");
    ...
    if (prepared && (Object.keys(prepared).join() !== "now" || !Number.isFinite(Date.parse(prepared.now))))
      throw Error("invalid retained setup preparation");
    ...
    const build = (
      kind: WorkRevision["kind"],
      entityId: string,
      content: unknown,
      parent: RevisionRef | null,
      dependencies: RevisionRef[] = [],
      policy: RevisionRef | null = null,
    ): RevisionRef => {
      const prior =
        revisions && [revisions.scope!, ...revisions.revisions].find((r) => r.kind === kind && r.id === entityId);
      const event = buildWorkRevisionEvent({
        eventId: `${prefix}:${token}:${kind}:${events.length}`,
        now,
        revision: {
          kind,
          id: entityId,
          revision: prior ? prior.revision + 1 : 1,
          predecessor: prior ? ref(prior) : null,
          scopeId: `${prefix}:scope`,
          contentDigest: hash(intentKey(content)),
          parent,
          dependencies,
          ownerId: prior?.ownerId ?? "local-operator",
          permittedEffects: prior ? [...prior.permittedEffects] : [],
          policy,
        },
      });
      events.push(event);
      return ref(event.payload.revision);
    };
    const scope = mode === "alternative" ? current!.scope : build("scope", `${prefix}:scope`, setup, null);
    const policy =
      mode === "new"
        ? build("policy", `${prefix}:policy`, { acceptance: "independent-review-required" }, null)
        : current!.policy;
    ...
    while (tasks.length < setup.tasks.length) {
      for (const task of setup.tasks) {
        if (tasks.some((t) => t.id === task.id) || !task.dependencies.every((d) => tasks.some((t) => t.id === d)))
          continue;
        ...
        const obligation = build(
          ...
          task,
          intent,
          task.dependencies.map((d) => tasks.find((t) => t.id === d)!.obligation),
          policy,
        );
    ...
    const snapshot = buildWorkSnapshotEvent({
      eventId: `${prefix}:${token}:snapshot`,
      now,
      snapshot: {
        snapshotId: `${prefix}:${token}:selected`,
        scope,
        revisions: [policy, intent, ...ordered.map((t) => t.obligation)],
        bindings: ordered.map((t) => ({ intent, obligation: t.obligation, artifact: null, policy })),
      },
    });
    ...
    const state: DeclaredWorkState = {
      ...
      id: setup.id,
      outcomeDigest: hash(setup.outcome.trim()),
      ledgerPath: join(root, "work.jsonl"),
      statePath,
      grantLedgerPath: null,
      selectedSnapshot: {
        snapshot: { id: snapshot.payload.snapshot.snapshotId, digest: snapshot.payload.snapshot.digest },
        event: { eventId: snapshot.eventId, digest: snapshot.digest },
      },
      scope,
      intent,
      obligation: ordered[0].obligation,
      policy,
    };
    const result: RecordedWorkSetup = { version: "recorded-work-setup-v1", setup, state, tasks: ordered, events };
    const path = setupPath(state),
      previous = await readProductJson(path);
    ...
    for (const event of events)
      await appendWorkLedgerEventOnce({ path: state.ledgerPath, grantLedgerPath: null }, event);
    await loadWorkSetup(state);
    return result;
  });
}
...
export async function loadWorkSetup(state: DeclaredWorkState): Promise<RecordedWorkSetup | null> {
  ...
  if (
    value.version !== "recorded-work-setup-v1" ||
    intentKey(value.state.selectedSnapshot) !== intentKey(state.selectedSnapshot) ||
    value.state.ledgerPath !== state.ledgerPath ||
    value.state.statePath !== state.statePath ||
    hash(setup.outcome.trim()) !== value.state.outcomeDigest ||
    !Array.isArray(value.tasks) ||
    value.tasks.length !== setup.tasks.length
  )
    throw Error("work setup selection mismatch");
  const resolved = resolveWorkSnapshotText(await readFile(state.ledgerPath, "utf8"), state.selectedSnapshot);
  if (
    resolved.scopeState !== "valid" ||
    !resolved.snapshot ||
    hash(intentKey(setup)) !== resolved.revisions.find((r) => r.digest === value.state.intent.digest)?.contentDigest
  )
    throw Error("recorded work labels are stale or mismatched");
  if (
    intentKey(resolved.snapshot.scope) !== intentKey(value.state.scope) ||
    !resolved.snapshot.bindings.some(
      (b) =>
        intentKey(b.intent) === intentKey(value.state.intent) &&
        intentKey(b.policy) === intentKey(value.state.policy) &&
        intentKey(b.obligation) === intentKey(state.obligation),
    )
  )
    throw Error("work state is not an exact selected binding");
  for (const task of setup.tasks) {
    const binding = value.tasks.find((t) => t.id === task.id),
      revision = resolved.revisions.find((r) => r.digest === binding?.obligation.digest);
    if (
      !binding ||
      !revision ||
      revision.contentDigest !== hash(intentKey(task)) ||
      !resolved.snapshot.bindings.some((b) => intentKey(b.obligation) === intentKey(binding.obligation))
    )
      throw Error("task configuration is not bound to selected work");
  }
  return value;
}
/** Caller must own a quiescent session, with no running daily host. No active order is rebound. */
export async function selectRecordedWork(
  recorded: RecordedWorkSetup,
  expected: IntentSelection | null,
): Promise<DeclaredWorkState> {
  ...
  return withFileLock(state.statePath, "select work", async () => {
    ...
    if (intentKey(current?.selectedSnapshot ?? null) !== intentKey(expected))
      throw Error("selection changed; no work selected");
    if (!(await loadWorkSetup(state))) throw Error("recorded setup missing");
    const { statePath: _path, ...stored } = state;
    await writeProductJson(state.statePath, stored, true);
    return state;
  });
}
...
export async function workSetupForSelection(
  cwd: string,
...
export async function workPresentation(state: DeclaredWorkState): Promise<WorkPresentation | null> {
  ...
  if (setup)
    return {
      outcome: setup.setup.outcome,
      obligations: setup.tasks.map((t) => {
        const spec = setup.setup.tasks.find((s) => s.id === t.id)!;
        return {
          digest: t.obligation.digest,
          outcome: spec.outcome,
          agent: spec.agent ?? "No-tool agent",
  ...
  if (
    ...
    hash(value.outcome.trim()) !== state.outcomeDigest
  )
    throw Error("declared outcome digest mismatch");
