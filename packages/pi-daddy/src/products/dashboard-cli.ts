...
import {
  ...
  isDashboardConnection,
  ENV_DASHBOARD_HOST_SOCKET,
  type DashboardConnection,
} from "./dashboard-host-transport.ts";
...
import { projectWorkLedger, type WorkProjectionContext } from "../governance/work-ledger.ts";
...
function dailySelection(text: string | undefined): WorkProjectionContext["selectedSnapshot"] {
  ...
  if (
    projectWorkLedger("", { selectedSnapshot: selection, authority: null }).errors.some(
      (e) => e.code === "WORK_CONTEXT_INVALID",
    )
  )
    throw new TypeError("invalid P01 snapshot selection");
  return selection;
}
...
export async function dashboardHostAction(host: DashboardConnection, line: string) {
  if (!isDashboardConnection(host) || Buffer.byteLength(line) > 60000)
    throw Error("genuine bounded dashboard connection required");
  const command = line.trim();
  if (command.startsWith("{"))
    return host.action(dashboardHostRequest(parseRetentionJson(command, 60000) as DashboardHostRequest));
...
export function dashboardActionFeedback(command: string, outcome: unknown): string {
  const value =
    outcome && typeof outcome === "object"
      ? (outcome as { state?: unknown; result?: unknown; error?: unknown; reason?: unknown })
      : {};
  const state = typeof value.state === "string" ? value.state : "unrecognised host response";
  if (state === "failed-or-unknown" || state === "readback-only")
    return `NO ACTION CLAIM: ${command} returned ${state}; inspect/reconcile explicitly.`;
  if (state !== "acknowledged")
    return `NOT APPLIED: ${command} returned ${state}.${typeof (value.error ?? value.reason) === "string" ? ` ${String(value.error ?? value.reason).slice(0, 1024)}` : ""}`;
  ...
  if (nativeApplication)
    return `ACKNOWLEDGED: ${command}; native application ${nativeApplication}, so no applied effect is claimed.`;
...
export async function dashboardFrame(options: DashboardFrameOptions): Promise<string> {
  ...
  if (options.connected) {
    ...
    try {
      view = await options.connected.frame();
    } catch (error) {
      options.actionMenu?.clear();
      return panelLines(
        [
          "PI-DADDY — Disconnected",
          "The original host is unavailable. No action was retried.",
          "Return to Pi: /grants host, then /grants dashboard.",
          ...(options.details ? [String(error)] : []),
        ],
        options.width,
      );
    }
    ...
    const summary = source?.daily
      ? renderDailyPanel(source.daily, options.width, {
          details: options.details,
          connected: true,
          presentation: view.presentation as WorkPresentation | null,
          error: view.error,
      ...
      : panelLines(
          [
            "PI-DADDY — Needs you",
            "Work observation unavailable. Capture current work or open Details.",
            ...menu.map((a) => `${a.choice}  ${a.label}`),
          ],
          options.width,
        );
    return [
      summary,
      view.debrief ? renderDebrief(view.debrief as Parameters<typeof renderDebrief>[0], options.width) : "",
      options.details
        ? panelLines(
            [
              `Host control ${view.control}; acknowledgement ${view.acknowledgement}. Attention reserved ${view.attention.attentionUsed}/5. Tip ${view.tip}.`,
  ...
  if (options.debrief || options.debriefJson) {
    ...
    const view = options.debrief.view();
    return options.debriefJson ? JSON.stringify(view) : renderDebrief(view, options.width);
  }
  if (options.dailyView) {
    try {
      if (options.dailyReader !== undefined && !isDailyViewReader(options.dailyReader))
        throw new TypeError("untrusted daily reader callback");
      const view = await (options.dailyReader ?? readDailyView)(options.dailyView);
      return options.dailyJson
        ? JSON.stringify(view)
        : renderDailyPanel(view, options.width, { details: options.details });
    } catch {
  ...
  if (!options.ledgerPath) {
    try {
      ...
      if (options.activityDetail)
        try {
          const taskKey = resolveActivityTaskSelector(
            timeline,
            options.activityDetail.taskKey,
            options.activityAliases,
          );
          if (!taskKey) throw Error("TIMELINE_DETAIL_INVALID");
          content = await detailForTimeline(activityPath, taskKey, options.activityDetail.field);
        } catch (error) {
  ...
  if (text.trimStart().startsWith('{"version":1')) {
    ...
    if (options.activityDetail)
      try {
        const taskKey = resolveActivityTaskSelector(activity, options.activityDetail.taskKey, options.activityAliases);
        if (!taskKey) throw Error("TIMELINE_DETAIL_INVALID");
        content = await detailForTimeline(ledgerPath, taskKey, options.activityDetail.field);
      } catch (error) {
        return `PI-DADDY — ACTIVITY DETAIL UNAVAILABLE\n${error instanceof Error ? error.message : String(error)}`;
      }
    timeline = renderActivityTimeline(activity, {
      details: options.details,
      history: options.history,
      filter: options.filter,
      color: options.color,
...
function parseArgs(argv: string[]): CliOptions {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    else if (arg === "--host-socket") {
      hostSocket = argv[++index];
      if (!hostSocket || hostSocket.startsWith("--")) throw Error("explicit host socket required");
    } else if (["--archive-projection", "--work-ledger", "--work-snapshot"].includes(arg)) {
      const value = argv[++index];
      if (!value || value.startsWith("--")) throw new Error("missing daily view argument");
...
export async function runDashboard(
  ...
  const dailyView =
    archivePath || workPath || selectedSnapshot || cli.dailyJson
      ? {
      ...
      : undefined;
  const socket = cli.hostSocket ?? env[ENV_DASHBOARD_HOST_SOCKET];
  const connected = host.connected ?? (socket ? connectDashboardHost(socket) : undefined);
  if (connected && (host.debrief || cli.debriefFixture || env[ENV_DEBRIEF_FIXTURE] === "1"))
    throw Error("connected host and separate debrief are exclusive");
  const debrief =
    host.debrief ?? (cli.debriefFixture || env[ENV_DEBRIEF_FIXTURE] === "1" ? await createFixtureDebrief() : undefined);
  if (debrief) {
  ...
  const display = createDashboardDisplayControls(
    cli.details,
    Boolean(!connected && !debrief && !cli.debriefJson && !dailyView),
  );
  let previous = "";
  let feedback = "";
  ...
  const draw = async (clear: boolean): Promise<void> => {
    const frame = await dashboardFrame({
      ...
      dailyView,
      dailyReader,
      dailyJson: cli.dailyJson,
      debrief,
      debriefJson: cli.debriefJson,
      connected,
    });
    ...
    process.stdout.write(
      clear ? `\u001b]0;PI-DADDY\u0007\u001b[2J\u001b[H${rendered}\n\n${input ? prompt + draft : ""}` : `${rendered}\n`,
    );
  ...
  input?.on("line", (line) => {
    ...
    const act = async () => {
      if (connected && /^\d+$/.test(line.trim())) {
        const selected = actionMenu.select(line.trim());
        label = selected.label;
        return connected.humanAction(selected.key, { tip: selected.tip, requestDigest: selected.requestDigest });
      }
    ...
    void act()
      .then((outcome) => {
        feedback = connected
          ? dashboardActionFeedback(label, outcome)
          : `ACKNOWLEDGED: ${label}. Refreshed state is shown below.`;
      })
      .catch((error) => {
