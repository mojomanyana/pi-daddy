...
import { canonicalWorkJson, copyWorkJson, freezeWork, parseWorkJson } from "../governance/work-ledger-json.ts";
import {
  parseWorkLedgerText,
  type WorkFrozen,
...
import { validateWorkSelection } from "../governance/work-ledger-validation.ts";
...
export interface IntentRequest {
  version: "intent-request-v1" | "intent-request-v2";
  requestId: string;
  bindingDigest: string;
  expectedRevision: number;
  expectedSelection: IntentSelection;
  action: "revise-scope" | "reprioritize" | "select-alternative" | "revise-selection";
...
export type IntentSnapshot = WorkFrozen<
  IntentState & {
    version: "intent-controller-snapshot-v1";
    bindingDigest: string;
    admission: "blocked-pending" | "intent-ready";
...
export function intentPriorities(input: unknown): IntentPriority[] {
  ...
  for (const p of result) {
    controlShape(p, ["obligation", "rank"]);
    controlShape(p.obligation, ["kind", "id", "revision", "digest"]);
    if (
      p.obligation.kind !== "obligation" ||
      !id(p.obligation.id) ||
      !hex(p.obligation.digest) ||
      !Number.isSafeInteger(p.obligation.revision) ||
      p.obligation.revision < 1 ||
      !Number.isSafeInteger(p.rank) ||
      p.rank < 0 ||
      p.rank > 127
    )
      throw new TypeError("invalid priority");
  }
  if (new Set(result.map((p) => intentKey(p.obligation))).size !== result.length)
    throw new TypeError("duplicate priority obligation");
...
export function intentSelection(value: unknown): IntentSelection {
  const copied = copyWorkJson(value);
...
export function intentRequest(input: IntentRequest): WorkFrozen<IntentRequest> {
  ...
  if (
    (r.action === "revise-selection" ? r.version !== "intent-request-v2" : r.version !== "intent-request-v1") ||
    !id(r.requestId) ||
    !hex(r.bindingDigest) ||
    !Number.isSafeInteger(r.expectedRevision) ||
    r.expectedRevision < 0 ||
    r.expectedRevision > 32 ||
    !["revise-scope", "reprioritize", "select-alternative", "revise-selection"].includes(r.action) ||
    !Array.isArray(r.events) ||
    r.events.length > 16 ||
    Buffer.byteLength(intentKey(r)) > 48 * 1024
  )
    throw new TypeError("invalid bounded intent request");
  const parsed = parseWorkLedgerText(r.events.map((e) => intentKey(e)).join("\n"));
  if (
    !parsed.complete ||
    parsed.events.some((e) => !id(e.eventId) || !["work_revision", "work_snapshot"].includes(e.event)) ||
    new Set(parsed.events.map((e) => e.eventId)).size !== parsed.events.length ||
    (!["revise-scope", "revise-selection"].includes(r.action) && r.events.length)
  )
    throw new TypeError("only strict revision/snapshot proposals may be appended");
  return freezeWork({
    ...r,
    expectedSelection: intentSelection(r.expectedSelection),
    selection: intentSelection(r.selection),
    priorities: intentPriorities(r.priorities),
  });
}
...
export function intentReceipt(input: WorkFrozen<IntentRequest>, decision: IntentReceipt["decision"]): IntentReceipt {
  return JSON.parse(
    intentKey({
      ...
      digest: intentRequestDigest(input),
      expectedRevision: input.expectedRevision,
      expectedSelection: input.expectedSelection,
      action: input.action,
      selection: input.selection,
      priorities: input.priorities,
      events: input.events.map((e) => ({ eventId: e.eventId, digest: e.digest })),
      decision,
      application: decision === "approved" ? "pending-or-unknown" : "not-applied",
      outcome: decision === "approved" ? "pending-or-unknown" : "refused",
...
export function matchesIntentReceipt(receipt: IntentReceipt, request: WorkFrozen<IntentRequest>): boolean {
  ...
  if (receipt.application === "applied") {
    ...
    expected.resultSelection = JSON.parse(intentKey(request.selection));
...
export function intentDecision(
  state: IntentState,
  request: Pick<IntentRequest, "expectedRevision" | "expectedSelection">,
  authorized: boolean,
  dispatchBusy: boolean,
): IntentReceipt["decision"] {
  if (!authorized) return "authority-unavailable";
  if (
    request.expectedRevision !== state.revision ||
    intentKey(request.expectedSelection) !== intentKey(state.selection)
  )
    return "stale";
...
export function replayIntent(
  ...
  if (event.type === "intent-request") {
    ...
    intentSelection(r.expectedSelection);
    intentSelection(r.selection);
    intentPriorities(r.priorities);
    if (
      !id(r.requestId) ||
      !hex(r.digest) ||
      !Number.isSafeInteger(r.expectedRevision) ||
      ...
      !Array.isArray(r.events) ||
      r.events.length > 16 ||
      state.records.length >= 32 ||
      state.records.some((p) => p.requestId === r.requestId) ||
      intentDecision(state, r, r.decision !== "authority-unavailable", dispatchBusy) !== r.decision ||
      r.application !== (r.decision === "approved" ? "pending-or-unknown" : "not-applied") ||
      r.outcome !== (r.decision === "approved" ? "pending-or-unknown" : "refused") ||
      r.resultSelection !== null
    )
      throw new Error("invalid intent decision sequence");
    for (const ref of r.events) {
      controlShape(ref, ["eventId", "digest"]);
      if (!id(ref.eventId) || !hex(ref.digest)) throw new Error("invalid event reference");
    }
    state.records.push(r);
    if (r.decision === "approved") state.revision++;
  } else {
    controlShape(event, ["type", "requestId"]);
    const r = state.records.find((r) => r.requestId === event.requestId);
    if (event.type !== "intent-apply" || !r || r.application !== "pending-or-unknown" || active || dispatchBusy)
      throw new Error("invalid intent application boundary");
    state.selection = r.selection;
    state.priorities = r.priorities;
    r.application = "applied";
    r.outcome = r.action === "reprioritize" ? "priority-applied" : "selection-applied";
    r.resultSelection = r.selection;
  }
}
