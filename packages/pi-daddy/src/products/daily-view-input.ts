...
export interface ArchiveExecution {
  executionId: string;
  parentExecutionIds: (string | null)[];
  retainedSessionIds: string[];
  activeBranch: null;
  toolCallIds: (string | null)[];
  archiveIds: string[];
  runtime: "running" | "terminal" | "conflict";
  outcome: {
    code: number | null;
    signal: string | null;
    timedOut: boolean;
...
export interface ArchiveProjection {
  version: typeof ARCHIVE_PROJECTION_VERSION;
...
export function freezeDaily<T>(value: T): T {
  if (value && typeof value === "object") {
    Object.values(value).forEach(freezeDaily);
    Object.freeze(value);
  }
  return value;
}
...
export function parseArchiveProjection(text: string): ArchiveProjection {
  ...
  for (const match of text.matchAll(/"(?:[^"\\]|\\[\s\S])*"|[^\s{}\[\],:]+/g)) {
    if (/^-?\d/.test(match[0]) && !/^-?(0|[1-9]\d*)$/.test(match[0]))
      throw new TypeError("unsupported archive numeric token");
  }
  ...
  for (const e of projection.executions) {
    if (ids.has(e.executionId) || (e.runtime !== "terminal" && e.outcome !== null))
      throw new TypeError("ambiguous archive execution");
...
export type SnapshotRead =
  { status: "read"; bytes: Buffer; sha256: string } | { status: "missing" | "error" | "unconfigured"; reason: string };
  ...
  try {
    ...
    return await runWithFinalizers(async (): Promise<SnapshotRead> => {
      ...
      if (!before.isFile() || before.size > BigInt(limit))
        return { status: "error", reason: "not-regular-or-over-limit" };
      const bytes = Buffer.alloc(Number(before.size));
      let offset = 0;
      while (offset < bytes.length) {
        const n = (await file.read(bytes, offset, bytes.length - offset, offset)).bytesRead;
        if (!n) return { status: "error", reason: "changed-during-read" };
        offset += n;
      }
      const after = await file.stat({ bigint: true }),
        located = await lstat(path, { bigint: true });
      if (
        ...
        return { status: "error", reason: "changed-during-read" };
      return { status: "read", bytes, sha256: byteHash(bytes) };
    }, [{ label: "snapshot close failed", run: () => file.close() }]);
  } catch (error) {
    return {
      status: (error as { code?: string }).code === "ENOENT" ? "missing" : "error",
