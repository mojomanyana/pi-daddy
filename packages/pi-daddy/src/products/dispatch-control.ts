...
import { canonicalWorkJson, copyWorkJson, parseWorkJson } from "../governance/work-ledger-json.ts";
...
export interface DispatchRequest {
  ...
  action:
    "pause-dispatch" | "resume-dispatch" | "cancel-execution" | "revise-scope" | "reprioritize" | "select-alternative";
...
export interface DispatchRecord {
  request: Readonly<DispatchRequest>;
  digest: string;
...
export interface DispatchSnapshot {
  version: "dispatch-control-snapshot-v1";
  bindingDigest: string;
  freshness: "snapshot-unknown";
  revision: number;
  paused: boolean;
  admission: "blocked-pending" | "paused" | "enabled";
...
export function controlShape(value: unknown, keys: string[]): asserts value is Record<string, unknown> {
  if (
    ...
    throw new TypeError("closed dispatch control data required");
}
export function dispatchRequest(input: DispatchRequest): Readonly<DispatchRequest> {
  ...
  if (
    ...
    !(input.targetExecutionId === null || id(input.targetExecutionId)) ||
    (input.action === "cancel-execution" ? input.targetExecutionId === null : input.targetExecutionId !== null)
  )
...
export function dispatchRequestDigest(input: DispatchRequest): string {
  return createHash("sha256")
    .update(canonicalWorkJson(copyWorkJson(dispatchRequest(input))))
    .digest("hex");
}
export function dispatchAuthority(input: DispatchAuthority | null): Readonly<DispatchAuthority> | null {
  ...
  if (
    !hex(input.authorityDigest) ||
    !Array.isArray(copy) ||
    copy.length > 128 ||
    !copy.every(hex) ||
    new Set(copy).size !== copy.length
  )
    throw new TypeError("invalid independent dispatch authority");
  return Object.freeze({
    authorityDigest: input.authorityDigest,
    requestDigests: Object.freeze([...copy] as string[]),
  });
}
...
export function replayDispatch(
  state: DispatchState,
  ...
  if (event.type === "control-request") {
    ...
    const request = dispatchRequest(event.request as DispatchRequest),
      decision = event.decision as DispatchRecord["decision"];
    if (
      state.records.length >= 128 ||
      request.bindingDigest !== bindingDigest ||
      state.records.some((r) => r.request.requestId === request.requestId) ||
      !["approved", "authority-unavailable", "unsupported", "stale", "busy"].includes(decision) ||
      dispatchDecision(state, request, decision !== "authority-unavailable", otherPending) !== decision
    )
      throw new Error("invalid dispatch decision sequence");
    state.records.push({
      request,
      digest: dispatchRequestDigest(request),
      decision,
      application: decision === "approved" ? "pending" : "not-applied",
      outcome: decision === "approved" ? "boundary-pending" : "refused",
    });
    ...
    if (event.type !== "control-apply" || !record || record.application !== "pending" || active !== 0)
      throw new Error("unverified dispatch application boundary");
    state.paused = record.request.action === "pause-dispatch";
    record.application = "applied";
    record.outcome = state.paused ? "paused" : "enabled";
  }
}
export function freezeDispatch(
  state: DispatchState,
  bindingDigest: string,
  otherPending = false,
  ...
  return Object.freeze({
    ...
    paused: state.paused,
    admission:
      otherPending || state.records.some((r) => r.application === "pending")
        ? "blocked-pending"
        : state.paused
          ? "paused"
          : "enabled",
    acceptance: "not-assessed",
    records: Object.freeze(state.records.map((r) => Object.freeze({ ...r }))),
  });
}
