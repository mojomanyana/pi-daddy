...
export function renderDebrief(view: DebriefFrame, width = 100): string {
  const lines = [
    "PI-DADDY — MANUAL DEBRIEF / WEEKLY QUEUE",
    `State: ${view.state}; automatic presentation UNQUALIFIED`,
    `Attention: ${view.budgetSpent ?? "UNKNOWN"}/5 allocated slots; ${view.unexposed} incidents not exposed; persistence: ${view.persistence}`,
    view.fixture
      ? "FROZEN SYNTHETIC FIXTURE — public seed/artifacts, NOT a real blind study"
      : "Host identity, pause policy and durability are not authenticated by this view.",
  ];
  if (view.observation)
    lines.push(
      `Selected signal batch: ${view.total} cases; observation ${view.observation.id}`,
      `Observation issues: ${view.observation.issues.join(", ") || "none recorded"}; counts are selected-batch only.`,
    );
  if (view.remainingBudget === null)
    lines.push("Attention persistence UNKNOWN — no new exposure allowance is inferred.");
  if (view.remainingBudget === 0)
    lines.push(
      "New questions DEFERRED — budget exhausted. Reopen retains the same slots; answers/skip do not refill them.",
    );
  for (const card of view.cards) {
    if (card.kind === "case")
      lines.push(
        `${card.slot}. CASE — ${card.summary}`,
        `   ${card.disposition} / ${card.resolution}; labels ${card.actionEnabled ? "enabled" : "unavailable or already submitted"}`,
      );
    else {
      lines.push(
        `${card.slot}. BLIND QUALITY — one shared slot; ${card.choice ? card.choice.kind : "UNSELECTED / UNRESOLVED"}`,
      );
      ...
      if (card.revealed) {
        ...
        for (const arm of (
          card.revealed as {
            arms: { label: string; cost: number | null; costUnit: string; configuration: Record<string, unknown> }[];
          }
        ).arms) {
        ...
        lines.push(
          `   Configuration/cost hidden; reveal ${card.choiceConfirmed ? "available" : "locked"}. Choices: one / tie / none / insufficient.`,
        );
    }
  }
  lines.push(
    "Skip and unselected remain unresolved. Labels are not work acceptance, causal proof or routing/adoption.",
...
export async function debriefAction(presenter: DebriefPresenter, command: string): Promise<unknown> {
  if (!isDebriefPresenter(presenter) || typeof command !== "string" || command.length > 4096)
    throw new Error("invalid debrief action");
  const [action, ...args] = command.trim().split(/\s+/),
    view = presenter.view();
  if (action === "close" && !args.length) {
    return presenter.close();
  }
  if (action === "open" && !args.length) {
    return presenter.open({ mode: "manual", userPresent: true });
  }
  if (action === "reveal" && !args.length) {
    return presenter.reveal();
  }
  if (action === "choose") {
    return presenter.choose({ kind: args[0] as "one", labels: args.slice(1) });
  }
  const slot = Number(args[0]),
    card = view.cards.find((c) => c.slot === slot);
  if (card?.kind !== "case") throw new Error("exact exposed case slot required");
  if (action === "reconcile" && args.length === 1) {
    return presenter.reconcile(card.caseManifestId);
  }
