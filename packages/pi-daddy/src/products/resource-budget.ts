...
import { parseWorkJson, freezeWork } from "../governance/work-ledger-json.ts";
import { runWithFinalizers } from "../governance/finalization.ts";
import {
  workIntentBinding,
  readIntentWork,
  resolveIntent,
  validateIntentApplication,
  applyIntentApplication,
} from "./intent-application.ts";
import {
  intentKey,
  intentRequest,
  intentRequestDigest,
  intentReceipt,
  matchesIntentReceipt,
  intentDecision,
  replayIntent,
  intentSnapshot,
...
export interface BudgetBinding<V extends "1.0" | "2.0" | "3.0" | "4.0" = "1.0"> {
  ...
  intent?: V extends "3.0" ? WorkIntentBinding : never;
}
...
interface Reservation extends AttemptDemand {
  intent?: IntentAdmission;
  owner: string;
...
export interface BudgetSnapshot {
  ...
  inputBytes: number;
  active: number;
  reservations: readonly Readonly<Reservation>[];
}
export interface ResourcePermit {
  readonly attemptId: string;
  settle(outcome: "completed" | "failed" | "cancelled"): Promise<void>;
}
interface PermitControl {
  owner: object;
  bindingDigest: string;
  version: string;
  demand: AttemptDemand;
  claimed: boolean;
  settling: number;
  settled: boolean;
  verify(): Promise<void>;
  finish: ResourcePermit["settle"];
}
...
export function claimResourcePermit(
  owner: object,
  permit: ResourcePermit,
  bindingDigest: string,
  expected: AttemptDemand,
) {
  const d = demand(expected),
    c = originalPermits.get(permit);
  if (
    !c ||
    c.owner !== owner ||
    c.version !== "4.0" ||
    c.bindingDigest !== bindingDigest ||
    JSON.stringify(c.demand) !== JSON.stringify(d) ||
    c.claimed ||
    c.settling ||
    c.settled
  )
    fail("OWNERSHIP_LOST", "original unclaimed experiment permit required");
  c.claimed = true;
...
function fail(code: ResourceAdmissionError["code"], text: string): never {
  throw new ResourceAdmissionError(code, text);
}
...
function shape(x: unknown, keys: string[]): asserts x is Record<string, unknown> {
  if (
    ...
    fail("INVALID", "closed data object required");
}
function limits(x: ResourceLimits): ResourceLimits {
  shape(x, ["maxAttempts", "maxInputBytes", "maxConcurrent"]);
  if (
    !integer(x.maxAttempts, 1024) ||
    x.maxAttempts < 1 ||
    !integer(x.maxInputBytes, 16 * 1024 * 1024) ||
    !integer(x.maxConcurrent, 32) ||
    x.maxConcurrent < 1 ||
    x.maxConcurrent > x.maxAttempts
  )
    fail("INVALID", "unsupported resource limits");
...
function binding<T extends GovernedBudgetBinding>(x: T): Readonly<T> {
  shape(x, [
    ...
    "authorityDigest",
    "limits",
    ...(Object.getOwnPropertyDescriptor(x ?? {}, "version")?.value === "3.0" ? ["intent"] : []),
  ]);
  if (
    !["1.0", "2.0", "3.0", "4.0"].includes(x.version) ||
    typeof x.directory !== "string" ||
    ...
    fail("INVALID", "invalid independent budget binding");
  return Object.freeze({
    version: x.version,
    directory: x.directory,
    device: x.device,
    inode: x.inode,
    journalDevice: x.journalDevice,
    journalInode: x.journalInode,
    authorityDigest: x.authorityDigest,
    limits: limits(x.limits),
    ...(x.version === "3.0" ? { intent: freezeWork(workIntentBinding(x.intent!)) } : {}),
  }) as Readonly<T>;
}
function demand(x: AttemptDemand): AttemptDemand {
  ...
  if (
    ![x.attemptId, x.orderId, x.experimentId].every(id) ||
    !["primary", "retry", "shadow", "descendant"].includes(x.kind) ||
    !(x.parentAttemptId === null || id(x.parentAttemptId)) ||
    (x.kind !== "primary" && x.parentAttemptId === null) ||
    !integer(x.inputBytes, 16384) ||
    !digest(x.inputDigest)
  )
    fail("INVALID", "invalid attempt demand");
  return {
    attemptId: x.attemptId,
    orderId: x.orderId,
    experimentId: x.experimentId,
    kind: x.kind,
    parentAttemptId: x.parentAttemptId,
    inputBytes: x.inputBytes,
...
async function privateDirectory(path: string) {
  ...
  if (
    ...
    fail("INVALID", "a canonical owner-private directory is required");
  return st;
}
...
export function createResourceBudget(input: BudgetCreation): Promise<Readonly<BudgetBinding>> {
  return createBudget(input, "1.0");
}
/** Explicit opt-in v2 journal; old readers refuse rather than ignore control events. */
export function createDispatchBudget(input: BudgetCreation): Promise<Readonly<DispatchBudgetBinding>> {
  return createBudget(input, "2.0");
}
export async function createIntentBudget(
  input: BudgetCreation,
  intent: Parameters<typeof readIntentWork>[0],
): Promise<Readonly<IntentBudgetBinding>> {
...
export function createExperimentBudget(input: BudgetCreation): Promise<Readonly<ExperimentBudgetBinding>> {
  return createBudget(input, "4.0");
}
...
function createBudget(
  input: BudgetCreation,
  version: "3.0",
  intent: WorkIntentBinding,
): Promise<Readonly<IntentBudgetBinding>>;
async function createBudget(
  input: BudgetCreation,
  version: GovernedBudgetBinding["version"],
  intent?: WorkIntentBinding,
): Promise<Readonly<GovernedBudgetBinding>> {
  const initialIntent = version === "3.0" ? workIntentBinding(intent!) : undefined;
  shape(input, ["directory", "authorityDigest", "limits"]);
  const policy = limits(input.limits),
  ...
  if (initialIntent)
    resolveIntent(await readIntentWork(initialIntent), initialIntent.selection, initialIntent.priorities);
  ...
  const b = await runWithFinalizers(async () => {
    ...
    const result = binding({
      ...
      inode: String(st.ino),
      journalDevice: String(journal.dev),
      journalInode: String(journal.ino),
      authorityDigest,
      limits: policy,
  ...
  await runWithFinalizers(
    () => directory.sync(),
    [{ label: "budget directory close failed", run: () => directory.close() }],
  );
  return b;
}
...
export function openResourceBudget<T extends GovernedBudgetBinding>(input: T) {
  const b = binding(input),
    path = join(b.directory, "budget.jsonl"),
    owner = randomUUID();
  ...
  async function transaction<T>(
    ...
    const checkRoot = async () => {
      ...
      if (String(st.dev) !== b.device || String(st.ino) !== b.inode)
        fail("AUTHORITY_CHANGED", "budget root identity changed");
    };
    await checkRoot();
    const execute = async () => {
      await checkRoot();
      const file = await open(
        path,
        (readOnly ? constants.O_RDONLY : constants.O_RDWR | constants.O_APPEND) |
      ...
      return runWithFinalizers(async () => {
        ...
        if (
          ...
          st.uid !== BigInt(process.getuid!()) ||
          (st.mode & 0o077n) !== 0n ||
          st.size > 2n * 1024n * 1024n
        )
          fail("INVALID", "unsafe or oversized budget journal");
        if (key !== `${b.journalDevice}:${b.journalInode}` || (pinnedFile && pinnedFile !== key))
          fail("AUTHORITY_CHANGED", "budget journal replaced");
        pinnedFile = key;
        const buffer = Buffer.alloc(Number(st.size));
        let offset = 0;
        while (offset < buffer.length) {
          const n = (await file.read(buffer, offset, buffer.length - offset, offset)).bytesRead;
          if (!n) fail("INVALID", "short budget read");
          offset += n;
        }
        ...
        if (
          lines.length > (b.version === "3.0" ? 2369 : b.version !== "1.0" ? 2305 : 2049) ||
          JSON.stringify(binding(parseWorkJson(lines[0]) as unknown as GovernedBudgetBinding)) !== JSON.stringify(b)
        )
          fail("AUTHORITY_CHANGED", "budget header does not match independent authority");
        const records: Reservation[] = [],
          control: DispatchState = { revision: 0, paused: false, records: [] };
        const intent: IntentState | null = b.intent
          ? { revision: 0, selection: b.intent.selection, priorities: b.intent.priorities, records: [] }
          : null;
        for (const line of lines.slice(1)) {
          ...
          if (event.type === "intent-request" || event.type === "intent-apply") {
            ...
            replayIntent(
              intent,
              event,
              records.filter((r) => r.state === "reserved").length,
              control.records.some((r) => r.application === "pending"),
            );
            ...
            replayDispatch(
              control,
              event,
              resourceBindingDigest(b),
              records.filter((r) => r.state === "reserved").length,
              Boolean(intent?.records.some((r) => r.application === "pending-or-unknown")),
            );
            ...
            if (
              !id(event.owner) ||
              !Array.isArray(event.demands) ||
              !event.demands.length ||
              event.demands.length > 32 ||
              control.paused ||
              control.records.some((r) => r.application === "pending")
            )
              fail("INVALID", "invalid batch");
            for (const value of event.demands) {
              const d = demand(value as AttemptDemand);
              if (
                records.some((r) => r.attemptId === d.attemptId) ||
                (d.parentAttemptId !== null && !records.some((r) => r.attemptId === d.parentAttemptId))
              )
                fail("INVALID", "invalid batch sequence");
            ...
            if (
              control.paused ||
              control.records.some((r) => r.application === "pending") ||
              intent?.records.some((r) => r.application === "pending-or-unknown")
            )
              fail("INVALID", "reservation crossed dispatch barrier");
            ...
            if (
              ...
              fail("INVALID", "invalid reservation sequence");
            if (intent && (d.kind !== "primary" || d.parentAttemptId !== null))
              fail("INVALID", "v3 schedules primary dispatch only");
            const assigned = intent ? intentAdmission(intent, records, event.intent as IntentAdmission) : undefined;
            records.push({
              ...d,
              ...(assigned ? { intent: assigned } : {}),
              owner: event.owner,
              state: "reserved",
              outcome: null,
            });
            ...
            if (
              ...
              r.state !== "reserved" ||
              event.owner !== r.owner ||
              !["completed", "failed", "cancelled"].includes(String(event.outcome))
            )
              fail("INVALID", "invalid settlement sequence");
            r.state = "settled";
            r.outcome = String(event.outcome);
          }
          if (
            records.length > b.limits.maxAttempts ||
            records.reduce((n, r) => n + r.inputBytes, 0) > b.limits.maxInputBytes ||
            records.filter((r) => r.state === "reserved").length > b.limits.maxConcurrent
          )
            fail("INVALID", "journal exceeds independently fixed limits");
        }
        ...
        const unchanged = async () => {
          ...
          if (
            `${current.dev}:${current.ino}` !== key ||
            current.size !== expectedSize ||
            (await file.stat({ bigint: true })).size !== expectedSize ||
            current.nlink !== 1n ||
            (current.mode & 0o077n) !== 0n
          )
            fail("AUTHORITY_CHANGED", "budget journal changed under lock");
        };
        await unchanged();
        return await work(
          records,
          async (event) => {
            ...
            if (expectedSize + BigInt(Buffer.byteLength(line)) > 2n * 1024n * 1024n)
              fail("EXHAUSTED", "journal capacity reached");
            await unchanged();
            await file.writeFile(line);
            expectedSize += BigInt(Buffer.byteLength(line));
            await file.sync();
            await unchanged();
          },
          control,
  ...
  const permit = (d: AttemptDemand): Readonly<ResourcePermit> => {
    ...
    const c: PermitControl = {
      owner: api,
      bindingDigest: resourceBindingDigest(b),
      version: b.version,
      demand: d,
      claimed: false,
      settling: 0,
      settled: false,
      verify: () =>
        transaction(async (records, _append, control, intent) => {
          const r = records.find((r) => r.attemptId === attemptId);
          if (
            !r ||
            r.owner !== owner ||
            r.state !== "reserved" ||
            Object.keys(d).some((k) => r[k as keyof AttemptDemand] !== d[k as keyof AttemptDemand])
          )
            fail("OWNERSHIP_LOST", "original reservation is not held");
          if (
            control.paused ||
            control.records.some((r) => r.application === "pending") ||
            intent?.records.some((r) => r.application === "pending-or-unknown")
          )
            fail("DISPATCH_BLOCKED", "dispatch barrier");
        }),
      async finish(outcome) {
        ...
        try {
          await transaction(async (records, append) => {
            ...
            if (r.state === "settled") {
              if (r.outcome !== outcome) fail("INVALID", "contradictory settlement");
              return;
            }
  ...
  const api = Object.freeze({
    ...
    async reserveBatch(inputs: readonly AttemptDemand[]): Promise<readonly Readonly<ResourcePermit>[]> {
      if (
        ...
        Array.from({ length: inputs.length }, (_, i) => Object.getOwnPropertyDescriptor(inputs, String(i))).some(
          (d) => !d?.enumerable || !Object.hasOwn(d, "value"),
        )
      )
        fail("INVALID", "explicit v4 dense bounded batch required");
      const demands = Array.from(inputs, demand);
      await transaction(async (records, append, control) => {
        if (control.paused || control.records.some((r) => r.application === "pending"))
          fail("DISPATCH_BLOCKED", "dispatch barrier");
        ...
        if (
          records.length + demands.length > b.limits.maxAttempts ||
          records.reduce((n, r) => n + r.inputBytes, 0) + demands.reduce((n, d) => n + d.inputBytes, 0) >
            b.limits.maxInputBytes ||
          records.filter((r) => r.state === "reserved").length + demands.length > b.limits.maxConcurrent
        )
          fail("EXHAUSTED", "whole experiment exceeds aggregate allowance");
    ...
    intentControls(input: DispatchAuthority | null) {
      ...
      const work = b.intent,
        authority = dispatchAuthority(input),
        scope = resourceBindingDigest(b);
      const authorized = (digest: string) =>
        authority?.authorityDigest === b.authorityDigest && authority.requestDigests.includes(digest);
      const apply = async (
        request: ReturnType<typeof intentRequest>,
        records: Reservation[],
        append: (event: unknown) => Promise<void>,
        control: DispatchState,
        state: IntentState,
        ...
        if (!pending || pending.digest !== intentRequestDigest(request))
          fail("DUPLICATE", "exact original intent request required");
        if (!matchesIntentReceipt(pending, request))
          fail("INVALID", "intent receipt projection does not match original request");
        if (
          pending.application !== "pending-or-unknown" ||
          !authorized(pending.digest) ||
          records.some((r) => r.state === "reserved") ||
          control.records.some((r) => r.application === "pending")
        )
          return;
        ...
        await append(event);
        replayIntent(state, event, 0, false);
      };
      return Object.freeze({
        inspect: () =>
          transaction(async (records, _append, control, state) => {
            resolveIntent(await readIntentWork(work), state!.selection, state!.priorities);
            const blocked =
              control.paused ||
              control.records.some((r) => r.application === "pending") ||
              state!.records.some((r) => r.application === "pending-or-unknown");
            return freezeWork({
              ...intentSnapshot(state!, scope),
              dispatchPaused: control.paused,
              nextObligation: blocked ? null : nextIntent(state!, records),
            });
          }, true),
        async request(input: IntentRequest) {
          const request = intentRequest(input),
            digest = intentRequestDigest(request);
          ...
          return transaction(async (records, append, control, state) => {
            ...
            if (prior) {
              ...
              if (!matchesIntentReceipt(prior, request))
                fail("INVALID", "intent receipt projection does not match original request");
        ...
        async reconcile(input: IntentRequest) {
          ...
          if (request.bindingDigest !== scope) fail("AUTHORITY_CHANGED", "intent request is for another binding");
          return transaction(async (records, append, control, state) => {
            await apply(request, records, append, control, state!);
            return intentSnapshot(state!, scope);
          });
        },
      });
    ...
    controls(input: DispatchAuthority | null) {
      ...
      const authority = dispatchAuthority(input),
        scope = resourceBindingDigest(b);
      const authorized = (digest: string) =>
        authority?.authorityDigest === b.authorityDigest && authority.requestDigests.includes(digest);
      const apply = async (
        ...
        await append(event);
        replayDispatch(control, event, scope, 0);
      };
      return Object.freeze({
        inspect: () =>
          transaction(
            async (_records, _append, control, intent) =>
              freezeDispatch(
                control,
                scope,
                Boolean(intent?.records.some((r) => r.application === "pending-or-unknown")),
        ...
        async reconcile(requestId: string) {
          if (typeof requestId !== "string" || !/^[a-zA-Z0-9:_-]{1,128}$/.test(requestId))
            fail("INVALID", "exact request ID required for reconciliation");
          return transaction(async (records, append, control, intent) => {
            await apply(requestId, records, append, control);
            return freezeDispatch(
              control,
              scope,
              Boolean(intent?.records.some((r) => r.application === "pending-or-unknown")),
            );
        ...
        async request(input: DispatchRequest) {
          const request = dispatchRequest(input),
            digest = dispatchRequestDigest(request);
    ...
    async controlSnapshot(): Promise<Readonly<BudgetSnapshot>> {
      return transaction(async (records) =>
        Object.freeze({
          ...
          inputBytes: records.reduce((n, r) => n + r.inputBytes, 0),
          active: records.filter((r) => r.state === "reserved").length,
          reservations: Object.freeze(records.map((r) => freezeWork({ ...r }))),
        }),
      );
    },
    ...
    async reserve(input: AttemptDemand, selection?: IntentAdmission): Promise<Readonly<ResourcePermit>> {
      const d = demand(input),
        selected: IntentAdmission | null = selection === undefined ? null : JSON.parse(intentKey(selection));
      if (Boolean(b.intent) !== Boolean(selected))
        fail("INVALID", "v3 reservations require an exact scheduled intent; older bindings do not accept it");
      await transaction(async (records, append, control, intent) => {
        if (
          ...
          fail("DISPATCH_BLOCKED", "dispatch paused or awaiting verified boundary");
        if (records.some((r) => r.attemptId === d.attemptId))
          fail("DUPLICATE", "attempt identity already charged; never launch a redelivery");
        if (d.parentAttemptId !== null && !records.some((r) => r.attemptId === d.parentAttemptId))
          fail("INVALID", "parent attempt is not in this budget");
        if (
          records.length >= b.limits.maxAttempts ||
          records.reduce((n, r) => n + r.inputBytes, 0) + d.inputBytes > b.limits.maxInputBytes ||
          records.filter((r) => r.state === "reserved").length >= b.limits.maxConcurrent
        )
          fail("EXHAUSTED", "aggregate allowance exhausted");
        if (intent) {
...
export function resourceBindingDigest(b: GovernedBudgetBinding): string {
  return hash(JSON.stringify(binding(b)));
}
