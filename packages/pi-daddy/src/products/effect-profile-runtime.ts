...
export const DIGEST_WORKER = `const b=Buffer.from(process.argv[1],'base64');
if(b.length>16384)process.exit(90);
process.stdout.write(JSON.stringify({bytes:b.length,sha256:require('node:crypto').createHash('sha256').update(b).digest('hex')})+'\\n');`;
...
export async function inspectDigestPrerequisites() {
  ...
  for (const requested of [process.execPath, "/usr/bin/bwrap", "/usr/bin/prlimit"]) {
    try {
      const path = requested === process.execPath ? await realpath(requested) : requested;
      const details = executableDetails(path, await lstat(path));
      observations.push({ requested, ...details, conforms: !Object.values(details.predicates).some(Boolean) });
    } catch (error) {
      observations.push({
        requested,
        conforms: false,
        error: { code: (error as NodeJS.ErrnoException).code ?? null, message: String(error) },
      });
    }
  }
  return {
    version: "digest-prerequisites-v1",
    platform: process.platform,
    nodeVersion: process.version,
    execPath: process.execPath,
    conforms: process.platform === "linux" && observations.every((o) => o.conforms),
    observations,
...
export async function digestRuntime(): Promise<DigestRuntime> {
  ...
  for (const path of [node, "/usr/bin/bwrap", "/usr/bin/prlimit"]) {
    ...
    if (!st.isFile() || (st.mode & 0o022) !== 0 || st.size > 256 * 1024 * 1024) {
      const details = executableDetails(path, st),
        code = details.predicates.notRegular
          ? "RUNTIME_NOT_REGULAR"
          : details.predicates.writable
            ? "RUNTIME_WRITABLE"
            : "RUNTIME_TOO_LARGE";
