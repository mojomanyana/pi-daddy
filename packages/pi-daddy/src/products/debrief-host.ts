...
export interface DurableBlindBinding {
  comparisonId: string;
  author: string;
}
export type CaseSelection =
  { version: "work-case-v2"; batchId: string } | { version: "work-signals-v1"; batchId: string; observationId: string };
function location(root: string, author: string) {
  if (
    ...
    throw new Error("explicit archive and independent operator author required");
}
function binding(input: DurableBlindBinding): DurableBlindBinding {
  const value = detached(input);
  closed(value, ["comparisonId", "author"]);
  if (!sha(value.comparisonId) || typeof value.author !== "string")
    throw new Error("exact retained comparison binding required");
  return freeze(value);
}
/** Explicit host retention action only. Caller must retain returned identity independently before exposure. */
export function retainDebriefBlind(
  harness: DebriefHarness,
  root: string,
  input: { manifest: unknown; evidence: unknown[]; qualification: unknown },
  author: string,
): DurableBlindBinding {
  location(root, author);
  const comparisonId = harness.retainBlindIntervention(
    root,
    input.manifest,
    input.evidence,
    input.qualification,
...
export function createRetainedDebrief(
  ...
  input: {
    ...
    author: string;
    cases: CaseSelection;
    blind?: DurableBlindBinding;
    persistence: DebriefPersistence;
    offset?: number;
  },
) {
  const root = input.archiveRoot,
    author = input.author,
    selection = freeze(detached(input.cases));
  location(root, author);
  if (selection.version !== "work-case-v2" && selection.version !== "work-signals-v1")
    throw new Error("explicit case version required");
  closed(
    selection,
    selection.version === "work-signals-v1" ? ["version", "batchId", "observationId"] : ["version", "batchId"],
  );
  if (!sha(selection.batchId) || (selection.version === "work-signals-v1" && !sha(selection.observationId)))
    throw new Error("exact selected batch/observation required");
  const reviewer =
    selection.version === "work-signals-v1"
      ? harness.createWorkSignalReviewer(root, selection.batchId, author)
      : harness.createWorkCaseReviewer(root, selection.batchId, author);
  ...
  const blind = blindBinding
    ? {
        ...openDebriefBlindPreview(harness, root, blindBinding),
        choose: (choice: BlindChoice) => open(root, blindBinding.comparisonId, author).choose(choice),
        reveal: () => open(root, blindBinding.comparisonId, author).reveal(),
      }
    : undefined;
  return createDebriefPresenter({
    scope: input.scope,
    offset: input.offset,
    operatorIdentity: author,
    reviewer,
    blind,
    blindBinding,
    signals:
      selection.version === "work-signals-v1"
        ? { batchId: selection.batchId, observationId: selection.observationId }
        : undefined,
