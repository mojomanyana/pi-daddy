...
export interface TimelineSkill extends ActivitySkill {
  available: boolean;
  read: boolean;
  active: boolean;
  superseded: boolean;
...
export interface TimelineTask {
  ...
  agents: Array<{ id: string; parentTaskId?: string; agent: string; state: string }>;
}
...
export const activityTaskKey = (rootId: string, taskId: string) =>
  `${encodeURIComponent(rootId)}:${encodeURIComponent(taskId)}`;
function taskFromKey(value: string): { rootId: string; taskId: string } | null {
  const parts = value.split(":");
  if (parts.length !== 2 || value.length > 1400) return null;
  try {
    const [rootId, taskId] = parts.map(decodeURIComponent);
    return validId(rootId) && validId(taskId) && value === activityTaskKey(rootId, taskId) ? { rootId, taskId } : null;
  } catch {
    return null;
  }
}
...
function eventOf(value: unknown): ActivityEvent | null {
  ...
  if (
    ...
    !validId(event.id) ||
    !validId(event.kind) ||
    !validId(event.rootId) ||
    !validId(event.taskId) ||
  ...
  if (
    ...
    (event.prompt && !validRef(event.prompt)) ||
    (event.final && !validRef(event.final))
  )
    return null;
  if (
    event.skill &&
    (!validId(event.skill.name) ||
      typeof event.skill.source !== "string" ||
      event.skill.source.length > 2048 ||
      !DIGEST.test(event.skill.digest))
  )
    return null;
...
export function parseActivityTimeline(text: string): ActivityTimeline {
  ...
  for (const [index, record] of read.records.entries()) {
    ...
    if (!event) {
      refusals.push(`line ${index + 1}: invalid or unsafe timeline event`);
      continue;
    }
    ...
    if (event.kind === "task_started") {
      ...
      task.thinking = event.thinking;
      task.prompt = event.prompt;
      task.agent = event.agent ?? task.agent;
      task.parentTaskId = event.parentTaskId ?? task.parentTaskId;
    }
    if (event.kind === "task_finished") {
      task.endedAt = event.at;
      task.final = event.final;
      task.status =
        event.outcome === "cancelled"
          ? "cancelled"
          : ["failed", "error"].includes(event.outcome ?? "")
            ? "failed"
            : "finished";
    }
    if (event.kind === "needs_you") task.status = "needs-you";
    if (event.skill) {
      let skill = task.skills.find(
        (value) =>
          value.name === event!.skill!.name &&
          value.digest === event!.skill!.digest &&
          value.source === event!.skill!.source,
      );
      if (!skill) {
        skill = { ...event.skill, available: false, read: false, active: false, superseded: false };
      ...
      if (event.kind === "skill_superseded") {
        skill.superseded = true;
        skill.active = false;
      }
    }
    if (event.kind === "agent_started" && event.agentId)
      task.agents.push({
        id: event.agentId,
        parentTaskId: event.parentTaskId,
    ...
    if (event.kind === "agent_finished" && event.agentId) {
      const agent = task.agents.find((value) => value.id === event!.agentId);
  ...
  return {
    tasks: [...tasks.values()].sort(
      (a, b) =>
        ...
        activityTaskKey(a.rootId, a.id).localeCompare(activityTaskKey(b.rootId, b.id)),
    ),
    refusals,
  };
}
...
export class ActivityTimelineAliases {
  ...
  selector(task: Pick<TimelineTask, "rootId" | "id">): string {
    ...
    if (!root) {
      root = `r${this.roots.size + 1}`;
      this.roots.set(task.rootId, root);
    }
    const key = activityTaskKey(task.rootId, task.id);
    let child = this.tasks.get(key);
    if (!child) {
      const count = [...this.tasks.keys()].filter((value) => taskFromKey(value)?.rootId === task.rootId).length;
      child = `t${count + 1}`;
      this.tasks.set(key, child);
    }
    const selector = `${root}/${child}`;
    this.selectors.set(selector, key);
    return selector;
  }
  resolve(selector: string): string | undefined {
    return this.selectors.get(selector);
  }
}
export function resolveActivityTaskSelector(
  timeline: ActivityTimeline,
  selector: string,
  aliases?: ActivityTimelineAliases,
): string | undefined {
  const key = aliases?.resolve(selector) ?? selector,
    identity = taskFromKey(key);
  return identity && timeline.tasks.some((task) => task.rootId === identity.rootId && task.id === identity.taskId)
    ? key
    : undefined;
}
...
function visualizeControls(value: string, preserveNewlines = false): string {
  ...
  for (let index = 0; index < value.length;) {
    ...
    else if ((codePoint >= 0x80 && codePoint <= 0x9f) || /\p{Cf}/u.test(char))
...
function clean(value: string | undefined): string {
  return visualizeControls(value ?? "")
    .replace(/\s+/g, " ")
    .trim();
}
...
function safeDetailLines(value: string, width: number): string[] {
  return visualizeControls(value, true)
    .split("\n")
...
interface ActivityRenderOptions {
  ...
  width?: number;
  aliases?: ActivityTimelineAliases;
  content?: { taskKey: string; field: "prompt" | "final"; text: string };
}
function taskKey(task: Pick<TimelineTask, "rootId" | "id">): string {
  return `${task.rootId}\0${task.id}`;
}
function isAttention(task: TimelineTask): boolean {
...
function stateLine(task: TimelineTask, child: boolean): { text: string; color: number } {
  ...
  if (task.status === "cancelled")
    return { text: child ? "WAIT CHILD CANCELLED" : "WAIT PARENT TURN CANCELLED", color: 33 };
...
export function renderActivityTimeline(timeline: ActivityTimeline, options: ActivityRenderOptions = {}): string {
  ...
  const width = Math.max(10, options.width ?? 80),
    ...
    aliases = options.aliases ?? new ActivityTimelineAliases();
  const byKey = new Map(timeline.tasks.map((task) => [taskKey(task), task]));
  const children = new Map<string, TimelineTask[]>(),
    roots: TimelineTask[] = [];
  for (const task of timeline.tasks) {
    const parent = task.parentTaskId ? byKey.get(`${task.rootId}\0${task.parentTaskId}`) : undefined;
    if (parent) {
      const list = children.get(taskKey(parent)) ?? [];
      list.push(task);
      children.set(taskKey(parent), list);
    } else roots.push(task);
  }
  for (const list of children.values())
    list.sort((a, b) => a.startedAt.localeCompare(b.startedAt) || a.id.localeCompare(b.id));
  roots.sort(
    (a, b) =>
      a.startedAt.localeCompare(b.startedAt) ||
      activityTaskKey(a.rootId, a.id).localeCompare(activityTaskKey(b.rootId, b.id)),
  );
  const matching = (task: TimelineTask): boolean =>
    filter === "everything"
      ? true
      : filter === "agents"
        ? Boolean(task.agent || task.agents.length)
        : filter === "skills"
  ...
  for (const task of timeline.tasks)
    if (matching(task)) {
      let current: TimelineTask | undefined = task,
        seen = new Set<string>();
      while (current && !seen.has(taskKey(current))) {
        shown.add(taskKey(current));
        seen.add(taskKey(current));
        current = current.parentTaskId ? byKey.get(`${current.rootId}\0${current.parentTaskId}`) : undefined;
      }
    }
  ...
  const choose = (tasks: TimelineTask[], limit: number): { shown: TimelineTask[]; hidden: number } => {
    const important = tasks.filter((task) => hasAttention(task)),
      quiet = tasks.filter((task) => !hasAttention(task));
    const kept = options.history ? quiet : quiet.slice(-limit);
    return {
      shown: [...important, ...kept].sort((a, b) => a.startedAt.localeCompare(b.startedAt) || a.id.localeCompare(b.id)),
      hidden: quiet.length - kept.length,
    };
  };
  const lines = [paint("PI-DADDY ACTIVITY", 36, color), ""];
  const renderTask = (task: TimelineTask, prefix: string, branch: string): void => {
    const key = taskKey(task),
      directChildren = (children.get(key) ?? []).filter((child) => shown.has(taskKey(child)));
    if (!shown.has(key)) return;
    const child = Boolean(task.parentTaskId && byKey.has(`${task.rootId}\0${task.parentTaskId}`));
    const state = stateLine(task, child),
      selector = aliases.selector(task);
    const heading = clean(task.agent) || (child ? "Governed child" : "User turn");
    const meta = [task.model && `model ${clean(task.model)}`, task.thinking && `thinking ${clean(task.thinking)}`]
      .filter(Boolean)
      .join(" · ");
    lines.push(
      `${prefix}${branch}${paint(child ? "AGENT" : "USER", child ? 36 : 34, color)} ${heading} [${selector}] · ${paint(state.text, state.color, color)}${meta ? ` · ${meta}` : ""}`,
    );
    const failedChildren = directChildren.filter((value) => value.status === "failed").length;
    if (failedChildren)
      lines.push(
        `${prefix}${branch ? "   " : ""}  ${paint(`FAIL OBSERVED CHILD FAILURES: ${failedChildren} failed (not accepted)`, 31, color)}`,
      );
    const visibleSkills = task.skills.filter(
      (skill) => options.details || filter === "skills" || skill.read || skill.active || skill.superseded,
    );
    for (const skill of visibleSkills) {
      const availabilityOnly = skill.available && !skill.read && !skill.active && !skill.superseded;
      const description = availabilityOnly
        ? "available only"
        : `read ${skill.read ? "yes" : "no"} · declared active ${skill.active ? "yes" : "no"}${skill.superseded ? " · superseded" : ""}`;
      lines.push(
        `${prefix}${branch ? "   " : ""}  ${paint("SKILL", 35, color)} ${clean(skill.name) || "unnamed"} · ${description}`,
      );
    }
    if (options.details || filter === "agents")
      for (const agent of task.agents)
        lines.push(
          `${prefix}${branch ? "   " : ""}  ${paint("AGENT", 36, color)} ${clean(agent.agent) || "unknown"} · observed ${clean(agent.state) || "unknown"}`,
        );
    if (options.details && (task.prompt || task.final))
      lines.push(
        `${prefix}${branch ? "   " : ""}  DETAILS p|f ${selector} · legacy ${activityTaskKey(task.rootId, task.id)}`,
      );
    const selected = choose(directChildren, 2);
    if (selected.hidden > 0)
      lines.push(
        `${prefix}${branch ? "   " : ""}  ${paint(`… ${selected.hidden} quiet completed child task${selected.hidden === 1 ? "" : "s"} hidden`, 90, color)}`,
      );
    selected.shown.forEach((value, index) =>
      renderTask(
        value,
        `${prefix}${branch === "" ? "" : branch === "└─ " ? "   " : "│  "}`,
        index === selected.shown.length - 1 ? "└─ " : "├─ ",
      ),
    );
  };
  const selectedRoots = choose(
    roots.filter((task) => shown.has(taskKey(task))),
    3,
  );
  if (selectedRoots.hidden > 0)
    lines.push(
      paint(
        `… ${selectedRoots.hidden} quiet completed root task${selectedRoots.hidden === 1 ? "" : "s"} hidden`,
        90,
        color,
      ),
    );
  ...
  if (timeline.refusals.length)
    lines.push(...timeline.refusals.map((refusal) => paint(`FAIL timeline refusal: ${clean(refusal)}`, 31, color)));
  if (options.content)
    lines.push(
      "",
      `${paint("PRIVATE", 34, color)} ${options.content.field.toUpperCase()} · ${clean(options.content.taskKey)}`,
      ...safeDetailLines(options.content.text, width),
    );
  lines.push(
    "",
    "Keys: d details · h history · Everything|Agents|Skills|Needs-you · p|f <r#/t# or root:task>. Read and declared active are observations, not compliance or acceptance.",
  );
...
export function activityIdentity(cwd: string, env: NodeJS.ProcessEnv = process.env): ActivityIdentity {
  ...
  const path = transported
    ? resolve(transported)
    : configured && configured !== "1" && configured !== "off" && configured !== "0"
      ? resolve(cwd, configured)
      : defaultActivityTimelinePath(cwd);
  return {
    rootId: env[ENV_ACTIVITY_ROOT]?.trim() || randomUUID(),
    path,
    ...(env[ENV_ACTIVITY_TASK]?.trim() ? { taskId: env[ENV_ACTIVITY_TASK].trim() } : {}),
    ...(env[ENV_ACTIVITY_PARENT_TASK]?.trim() ? { parentTaskId: env[ENV_ACTIVITY_PARENT_TASK].trim() } : {}),
  };
}
export class ActivityTimelineRecorder {
  ...
  constructor(cwd: string, env: NodeJS.ProcessEnv = process.env) {
    const configured = env[ENV_ACTIVITY_TIMELINE]?.trim();
    this.enabled = configured !== "off" && configured !== "0";
    this.contentEnabled =
      env[ENV_ACTIVITY_CONTENT]?.trim() !== "metadata-only" && env[ENV_ACTIVITY_CONTENT]?.trim() !== "off";
  ...
  async start(prompt: string, model?: string, thinking?: string, parentTaskId?: string): Promise<string | undefined> {
    if (!this.enabled) return undefined;
    this.taskId = randomUUID();
    await this.append("task_started", { parentTaskId, model, thinking, prompt: await this.content(prompt) });
    return this.taskId;
  }
  async childStarted(
    taskId: string,
    ...
    this.taskId = taskId;
    await this.append("task_started", {
      parentTaskId,
      agentId: taskId,
      agent,
      model,
      thinking,
      prompt: await this.content(prompt),
    });
    await this.append("agent_started", { parentTaskId, agentId: taskId, agent });
  }
  async childFinished(
    taskId: string,
    parentTaskId: string | undefined,
    ...
    if (!this.enabled) return;
    this.taskId = taskId;
    await this.append("agent_finished", { parentTaskId, agentId: taskId, agent, outcome });
    await this.finish(final, outcome);
  }
  async finish(final: string | undefined, outcome: "completed" | "failed" | "cancelled" = "completed"): Promise<void> {
    if (this.enabled && this.taskId)
      await this.append("task_finished", {
        outcome,
        ...(final === undefined ? {} : { final: await this.content(final) }),
      });
  }
  async skill(
    kind: "skill_available" | "skill_read" | "skill_active" | "skill_finished" | "skill_superseded",
    skill: ActivitySkill,
  ): Promise<void> {
    if (this.taskId) await this.append(kind, { skill });
  }
  async append(
    kind: string,
    fields: Omit<Partial<ActivityEvent>, "version" | "id" | "kind" | "at" | "rootId" | "taskId">,
  ): Promise<void> {
    if (!this.enabled || !this.taskId) return;
  ...
  private async content(value: string): Promise<ContentReference> {
    ...
    if (!this.contentEnabled || bytes > MAX_CONTENT_BYTES) return { digest, bytes };
    const root = join(dirname(this.path), "content"),
      target = join(root, digest);
    ...
    return { digest, bytes, ref: `content/${digest}` };
  }
}
export async function readActivityContent(timelinePath: string, reference: ContentReference): Promise<string> {
  ...
  const root = resolve(dirname(timelinePath), "content"),
    target = resolve(dirname(timelinePath), reference.ref);
  if (!target.startsWith(`${root}${sep}`) || (await realpath(root)) !== root) throw Error("TIMELINE_REFERENCE_INVALID");
  const handle = await open(target, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK);
  try {
    const before = await handle.stat();
    if (!before.isFile() || before.nlink !== 1 || before.size > MAX_CONTENT_BYTES)
      throw Error("TIMELINE_REFERENCE_INVALID");
    const value = await handle.readFile("utf8"),
      after = await handle.stat();
    if (after.size !== before.size || Buffer.byteLength(value) !== reference.bytes || sha(value) !== reference.digest)
      throw Error("TIMELINE_REFERENCE_TAMPERED");
    return value;
  } finally {
    await handle.close();
  }
}
export async function detailForTimeline(
  timelinePath: string,
  taskKey: string,
  field: "prompt" | "final",
): Promise<{ taskKey: string; field: "prompt" | "final"; text: string }> {
  const identity = taskFromKey(taskKey);
  if (!identity) throw Error("TIMELINE_DETAIL_INVALID");
  ...
  const reference = task?.[field];
  if (!reference) throw Error("TIMELINE_DETAIL_UNAVAILABLE");
