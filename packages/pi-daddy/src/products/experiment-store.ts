...
export async function ownedDirectory(path: string) {
  ...
  if (
    ...
    throw new Error("canonical private experiment directory required");
  return s;
}
export async function readExperimentFile(
  ...
  if (
    Object.keys(options).some((key) => key !== "appendOnly") ||
    (options.appendOnly !== undefined && options.appendOnly !== true)
  )
    throw new TypeError("invalid experiment read mode");
  ...
  return runWithFinalizers(async () => {
    ...
    if (
      ...
      throw new Error("unsafe experiment file");
    const data = Buffer.alloc(Number(s.size));
    let offset = 0;
    while (offset < data.length) {
      const n = (await file.read(data, offset, data.length - offset, offset)).bytesRead;
      if (!n) throw new Error("short experiment read");
      offset += n;
    }
    const end = await file.stat({ bigint: true }),
      current = await lstat(path, { bigint: true });
    const identityChanged = current.dev !== s.dev || current.ino !== s.ino || current.nlink !== 1n;
    if (!options.appendOnly && (end.size !== s.size || end.mtimeNs !== s.mtimeNs || identityChanged))
      throw new Error("experiment source changed");
    if (options.appendOnly) {
      if (end.size < s.size || identityChanged) throw new Error("experiment source changed");
      const verify = Buffer.alloc(data.length);
      let checked = 0;
      while (checked < verify.length) {
        const n = (await file.read(verify, checked, verify.length - checked, checked)).bytesRead;
        if (!n) throw new Error("short experiment read");
        checked += n;
      }
...
export async function writeExperimentFile(path: string, bytes: Uint8Array) {
  ...
  await runWithFinalizers(async () => {
    ...
    await f.sync();
    await f.chmod(0o400);
  }, [{ label: "experiment artifact close failed", run: () => f.close() }]);
  const dir = await open(dirname(path), constants.O_RDONLY | constants.O_DIRECTORY);
  await runWithFinalizers(
    () => dir.sync(),
    [{ label: "experiment artifact directory close failed", run: () => dir.close() }],
  );
}
export function validateExperimentBinding(value: ExperimentBinding): ExperimentBinding {
  const b = cloneExperiment(value);
  closed(b, ["version", "directory", "device", "inode", "journalDevice", "journalInode", "budget", "charter"]);
  if (
    b.version !== "experiment-binding-v1" ||
    ...
    throw new TypeError("independent experiment binding required");
...
export async function createExperimentStore(
  directory: string,
  budget: ExperimentBudgetBinding,
  charter: ExperimentCharter,
  bytes: Uint8Array,
  ...
  const s = await ownedDirectory(directory);
  ...
  const b = await runWithFinalizers(async () => {
    ...
    const value = validateExperimentBinding({
      version: "experiment-binding-v1",
      directory,
      device: String(s.dev),
      inode: String(s.ino),
      journalDevice: String(st.dev),
      journalInode: String(st.ino),
      budget,
      charter,
    });
  ...
  await runWithFinalizers(
    () => dir.sync(),
    [{ label: "experiment creation directory close failed", run: () => dir.close() }],
  );
  return b;
}
export function experimentStore(value: ExperimentBinding) {
  const b = validateExperimentBinding(value),
    path = join(b.directory, "experiment.jsonl");
  const check = async () => {
    const root = await ownedDirectory(b.directory),
      file = await lstat(path, { bigint: true });
    if (
      String(root.dev) !== b.device ||
      String(root.ino) !== b.inode ||
      String(file.dev) !== b.journalDevice ||
      String(file.ino) !== b.journalInode
    )
      throw new Error("experiment identity replaced");
  };
  const read = async () => {
    await check();
    const bytes = await readExperimentFile(path, 256 * 1024, { appendOnly: true });
    await check();
    const text = new TextDecoder("utf8", { fatal: true }).decode(bytes);
    if (!text.endsWith("\n")) throw new Error("torn experiment journal");
    const lines = text.slice(0, -1).split("\n");
    if (lines.length > 260 || experimentHash(parseWorkJson(lines[0])) !== experimentHash(b))
      throw new Error("experiment header/bound mismatch");
    let previous = experimentHash(b);
    const events: Record<string, unknown>[] = [];
    for (const line of lines.slice(1)) {
      const record = parseWorkJson(line) as Record<string, unknown>;
      closed(record, ["seq", "previous", "event"]);
      if (
        record.seq !== events.length ||
        record.previous !== previous ||
        !record.event ||
        typeof record.event !== "object" ||
        Array.isArray(record.event)
      )
        throw new Error("invalid experiment sequence");
      previous = experimentHash(record);
      events.push(record.event as Record<string, unknown>);
    }
  ...
  return {
    ...
    read,
    async transaction<T>(
      fn: (events: Record<string, unknown>[], append: (event: unknown) => Promise<void>) => Promise<T>,
    ) {
      await check();
      return withFileLock(
        path,
        ...
        async () => {
          ...
          return fn(state.events, async (event) => {
            ...
            const record = { seq: state.events.length, previous: state.previous, event },
            ...
            await runWithFinalizers(async () => {
              ...
              if (
                !s.isFile() ||
                s.nlink !== 1n ||
                String(s.dev) !== b.journalDevice ||
                String(s.ino) !== b.journalInode ||
                s.size !== BigInt(size) ||
                (s.mode & 0o077n) !== 0n
              )
                throw new Error("journal changed under lock");
              await f.writeFile(data);
              await f.sync();
            }, [{ label: "experiment append close failed", run: () => f.close() }]);
            size += data.length;
            await check();
            state.previous = experimentHash(record);
            state.events.push(cloneExperiment(event) as Record<string, unknown>);
          });
        },
