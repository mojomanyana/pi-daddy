...
export interface HostJournal {
  read(): HostEvent[];
  append(prior: string, value: Record<string, unknown>): HostEvent;
}
...
export interface DashboardHarness extends DebriefHarness {
  ...
  openTrustLifecycle(path: string): {
    history(): HostEvent[];
    inspect(now: number): { policyId: string; attentionUsed: number; tip: string; grantExpansion: false };
    expose(id: string, now: number): { mode: string; reason: string; replayed: boolean };
  };
  ...
  ): { checkpointId: string; sourceStatus: string; issues?: string[]; [key: string]: unknown };
  readArchiveCheckpoint(
    root: string,
    checkpoint: string,
  ): { checkpoint: { sourceManifestId: string; issues: string[]; [key: string]: unknown } };
  readArchiveSource(
    root: string,
    id: string,
  ): { status: string; bytes?: Uint8Array; reference?: { retention: string; sha256: string; [key: string]: unknown } };
  captureArchivedWorkSignals(
    root: string,
    id: string,
    context: WorkFrozen<WorkProjectionContext>,
    facts: unknown,
  ): { caseBatchId: string; observationId: string; linkageManifestId: string; runtimeFactsManifestId: string };
  ...
  readLearningLifecycle(
    root: string,
    id: string,
  ): { version: string; state: string; predecessorManifestId: string | null; links: Record<string, unknown> };
}
export interface DashboardHostConfig {
  ...
  policyPath: string;
  policySha256: string;
  sources: { id: string; kind: "work" | "retention" | "facts" }[];
  selection: WorkProjectionContext["selectedSnapshot"];
  cases: CaseSelection | null;
  blind: DurableBlindBinding | null;
  budgetDigest: string;
  experimentDigest: string | null;
  harnessArtifactDigest: string;
...
export interface DashboardHostRequest {
  version: "1.0";
  requestId: string;
  hostDigest: string;
  expectedTip: string;
...
export function dashboardHostRequest(input: DashboardHostRequest) {
  ...
  controlShape(r, ["version", "requestId", "hostDigest", "expectedTip", "selectionDigest", "operation", "payload"]);
  if (
    r.version !== "1.0" ||
    !/^[a-zA-Z0-9:_-]{1,128}$/.test(r.requestId) ||
    ![r.hostDigest, r.expectedTip, r.selectionDigest].every(sha) ||
    ![
      "observe",
      "present",
      "presented",
    ...
    Buffer.byteLength(JSON.stringify(r)) > 60000
  )
    throw Error("bounded exact dashboard request required");
  return freeze(r);
}
