...
export function renderDailyDetails(view: DailyView, width = 100): string {
  const lines = [
    ...
    `Inputs: work ${view.sources.work}; archive ${view.sources.archive}`,
    "",
    "INTENDED OBLIGATIONS",
  ];
  if (!view.obligations.length)
    lines.push("EMPTY / UNRESOLVED: no scoped obligations available; not proof of no intended work.");
  const attempts = new Map(view.attempts.map((a) => [a.executionId, a]));
  for (const o of view.obligations) {
    lines.push(
      `${o.obligation.id}@${o.obligation.revision} | ${o.acceptance.toUpperCase()} | ${o.activity.toUpperCase()}`,
    ...
    for (const id of o.attempts) {
      ...
      lines.push(
        `  attempt ${id} (logical ${a.logicalChildId ?? "unknown"}) | P01 runtime ${a.intentRuntime} | P03 ${a.archive?.runtime ?? "unavailable"}`,
      );
    }
  ...
  for (const a of view.attempts) {
    const e = a.archive;
    lines.push(
      `${a.executionId} | logical ${a.logicalChildId ?? "unknown"} | P01 ${a.intentRuntime} | P03 ${e?.runtime ?? "unavailable"}`,
    );
    if (e) {
      lines.push(
        `  producer-reported exit ${e.outcome?.code ?? "unknown"}; failed control/runtime ${e.outcome?.failed ?? "unknown"}; aborted ${e.outcome?.aborted ?? "unknown"}`,
  ...
  lines.push(
    "",
    `Coverage gaps: ${view.issues.join("; ") || "partial snapshots; live freshness unavailable"}`,
    `Work source SHA256: ${view.sources.workSha256 ?? "missing"}`,
    `Archive projection SHA256: ${view.sources.archiveSha256 ?? "missing"}`,
  ...
  const bounded = lines.slice(0, 300);
  if (lines.length > 300)
    bounded.push(`${lines.length - 300} additional lines omitted; full rows remain in the read-model.`);
  const columns = Number.isFinite(width) ? Math.max(20, Math.min(240, Math.floor(width))) : 100;
  return bounded
    .map((line) => {
      const text = [...clean(line)];
      return text.length > columns ? text.slice(0, columns - 3).join("") + "..." : text.join("");
    })
    .join("\n");
}
