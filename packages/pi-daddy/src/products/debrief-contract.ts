...
export interface ReviewItem {
  caseManifestId: string;
  candidate: { id: string; reason: string; [key: string]: unknown };
  priorDecisionId: string | null;
...
export interface BlindChoice {
  kind: "one" | "tie" | "none" | "insufficient";
  labels: string[];
}
export interface BlindView {
  version: "blind-intervention-view-v1";
  cards: { label: string; artifactDigests: string[] }[];
  limitations: string[];
}
...
export const dataKey = (v: unknown): string =>
  Array.isArray(v)
    ? `[${v.map(dataKey).join(",")}]`
    : v !== null && typeof v === "object"
      ? `{${Object.keys(v)
          .sort()
          .map((k) => JSON.stringify(k) + ":" + dataKey((v as Record<string, unknown>)[k]))
          .join(",")}}`
      : JSON.stringify(v);
...
export function detached<T>(input: T): T {
  ...
  const inspect = (v: unknown, depth: number): void => {
    ...
    const array = Array.isArray(v),
      ...
      fields = Object.getOwnPropertyDescriptors(v);
    if (
      array
        ? prototype !== Array.prototype || v.length > 4096 || Reflect.ownKeys(fields).length !== v.length + 1
        : ![Object.prototype, null].includes(prototype)
    )
      throw new Error("plain debrief object/array required");
    active.add(v);
    for (const k of Reflect.ownKeys(fields)) {
      if (array && k === "length") continue;
      const d = fields[k as string];
      if (typeof k !== "string" || !d.enumerable || !Object.hasOwn(d, "value"))
        throw new Error("hidden/accessor debrief data refused");
      inspect(d.value, depth + 1);
    }
    if (array)
      for (let i = 0; i < v.length; i++) if (!Object.hasOwn(fields, String(i))) throw new Error("sparse debrief array");
    active.delete(v);
  };
  inspect(input, 0);
...
export function closed(v: unknown, names: string[]): asserts v is Record<string, unknown> {
  if (!v || typeof v !== "object" || Array.isArray(v) || Object.keys(v).sort().join() !== [...names].sort().join())
    throw new Error("closed debrief object required");
}
export function reviewRequest(input: ReviewRequest): ReviewRequest {
  const r = detached(input);
  if (!requestCheck.Check(r)) throw new Error("invalid label request");
  return freeze(r);
}
...
function signalCase(c: Record<string, any>): boolean {
  if (c.capture_schema === 2)
    return (
      c.reason === "coverage_gap" &&
      c.classification === "coverage_issue" &&
      caseCheck.Check(c) &&
      c.id === dataDigest({ detector: c.detector, target: c.target, reason: c.reason })
    );
  if (
    c.capture_schema !== 3 ||
    c.classification !== "candidate_defect" ||
    !["overdue_checkpoint", "reopened_acceptance", "intent_conflict"].includes(c.reason)
  )
    return false;
  // Signal facts may contribute256 references plus the two base scope/obligation references.
  if (!Array.isArray(c.evidence) || !c.evidence.length || c.evidence.length > 258 || !c.evidence.every(sha))
    return false;
  const common = {
    ...c,
    capture_schema: 2,
    ...
    metrics: {},
    evidence: c.evidence.slice(0, 256),
  };
  if (
    !caseCheck.Check(common) ||
    c.detector.id !== c.reason ||
    !c.detector.version.startsWith("work-signals-v1:") ||
    !c.metrics ||
    typeof c.metrics !== "object"
  )
    return false;
  const keys =
    c.reason === "overdue_checkpoint"
      ? ["deadlineMs", "observedAt"]
      : c.reason === "intent_conflict"
        ? ["failedChecks"]
        : [];
  ...
  if (
    (c.reason === "overdue_checkpoint" && m.observedAt <= m.deadlineMs) ||
    (c.reason === "intent_conflict" && m.failedChecks < 1)
  )
    return false;
  const detector = { id: c.detector.id, version: c.detector.version, population: c.detector.population };
  const target = {
    kind: c.target.kind,
    snapshotDigest: c.target.snapshotDigest,
    obligationId: c.target.obligationId,
    obligationDigest: c.target.obligationDigest,
  };
  const metrics = Object.fromEntries(keys.map((k) => [k, c.metrics[k]])),
    evidence = [...new Set(c.evidence)].sort();
  return (
    dataKey(evidence) === dataKey(c.evidence) &&
    c.id ===
      createHash("sha256")
        .update(JSON.stringify({ capture_schema: 3, detector, target, reason: c.reason, metrics, evidence }))
        .digest("hex")
  );
}
export function reviewPage(value: unknown, limit: number, offset = 0, signals?: SignalSelection) {
  const page = detached(value) as {
    total: number;
    offset: number;
    items: ReviewItem[];
    observationId?: string;
  ...
  if (
    signals &&
    (page.observationId !== signals.observationId ||
      page.total > 1024 ||
      !Array.isArray(page.issues) ||
      page.issues.length > 1 ||
      page.issues.some((i) => i !== "scope-unresolved"))
  )
    throw new Error("unbound work signal observation");
  if (
    ...
    page.items.length > Math.max(0, page.total - offset)
  )
    throw new Error("invalid bounded case page");
  for (const item of page.items) {
    closed(item, ["caseManifestId", "candidate", "priorDecisionId", "disposition"]);
    const c = item.candidate as Record<string, any>;
    if (
      !sha(item.caseManifestId) ||
      !(signals ? signalCase(c) : caseCheck.Check(c)) ||
      !(item.priorDecisionId === null || sha(item.priorDecisionId)) ||
      !["unresolved", "confirmed_defect", "expected_behavior", "exemplar", "uncertain", "skip"].includes(
        item.disposition,
      ) ||
      ...
      (!signals &&
        c.classification !==
          (
            {
              repeat_without_progress: "candidate_defect",
              economical_exemplar: "candidate_exemplar",
              coverage_gap: "coverage_issue",
            } as Record<string, string>
          )[c.reason]) ||
      (c.reason === "repeat_without_progress" &&
        (c.metrics.equivalentAttempts === undefined || c.metrics.equivalentAttempts < 2)) ||
      (c.reason === "economical_exemplar" &&
        (c.metrics.measuredCost === undefined ||
          c.metrics.costLimit === undefined ||
          c.metrics.costUnit === undefined ||
          c.metrics.measuredCost > c.metrics.costLimit))
    )
      throw new Error("invalid case identity/state");
  }
  if (new Set(page.items.map((i) => i.caseManifestId)).size !== page.items.length)
    throw new Error("duplicate case card");
  return freeze(page);
}
export function validatedHistory(value: unknown, item: ReviewItem) {
  ...
  for (const d of history) {
    ...
    if (
      ...
      !sha(id) ||
      dataDigest(body) !== id ||
      d.caseId !== item.candidate.id ||
      d.priorDecisionId !== prior ||
      ...
      !requestCheck.Check({
        ...
        priorDecisionId: d.priorDecisionId,
        disposition: d.disposition,
        note: d.note,
      }) ||
      !Array.isArray(d.evidence) ||
      d.evidence.length > 256 ||
      !d.evidence.every(sha) ||
      !d.evidence.includes(item.caseManifestId)
    )
      throw new Error("unbound review history");
    prior = id;
  }
...
export function matchingHistory(value: unknown, item: ReviewItem, request: ReviewRequest, author: string) {
  ...
  const expected = dataDigest({
    caseId: item.candidate.id,
    priorDecisionId: request.priorDecisionId,
    disposition: request.disposition,
    author,
    evidence: [item.caseManifestId],
    note: request.note,
  });
  return last?.id === expected ? expected : null;
}
export function blindView(value: unknown): BlindView {
  const view = detached(value) as BlindView;
  closed(view, ["version", "cards", "limitations"]);
  if (
    view.version !== "blind-intervention-view-v1" ||
    !Array.isArray(view.cards) ||
    view.cards.length < 2 ||
    view.cards.length > 4 ||
    !Array.isArray(view.limitations) ||
    view.limitations.some((v) => typeof v !== "string")
  )
    throw new Error("unsupported blind view");
  for (const c of view.cards) {
    closed(c, ["label", "artifactDigests"]);
    if (
      !/^variant-[a-f0-9]{16}$/.test(c.label) ||
      !Array.isArray(c.artifactDigests) ||
      c.artifactDigests.length > 4 ||
      !c.artifactDigests.every(sha)
    )
      throw new Error("invalid opaque artifact card");
  }
  if (new Set(view.cards.map((c) => c.label)).size !== view.cards.length) throw new Error("duplicate blind label");
  return freeze(view);
}
export function blindChoice(input: BlindChoice, view: BlindView): BlindChoice {
  const c = detached(input);
  closed(c, ["kind", "labels"]);
  if (
    !Array.isArray(c.labels) ||
    new Set(c.labels).size !== c.labels.length ||
    c.labels.some((l) => !view.cards.some((v) => v.label === l)) ||
    !["one", "tie", "none", "insufficient"].includes(c.kind) ||
    (c.kind === "one" ? c.labels.length !== 1 : c.kind === "tie" ? c.labels.length < 2 : c.labels.length !== 0)
  )
    throw new Error("invalid quality choice");
