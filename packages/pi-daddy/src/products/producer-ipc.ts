...
import {
  ...
  producerIpcDemand,
  producerIpcFrame,
  PRODUCER_IPC_LIMITS as LIMITS,
  type ProducerIpcBinding,
  type ProducerIpcReferences,
} from "./producer-ipc-contract.ts";
export {
  producerIpcBinding,
  producerIpcBindingDigest,
  producerIpcDemand,
...
export interface ProducerIpcContext {
  readonly binding: Readonly<ProducerIpcBinding>;
  readonly signal: AbortSignal;
}
export interface ProducerIpcHost {
  readonly binding: Readonly<ProducerIpcBinding>;
}
interface HostState {
  owner: Owner;
  exchange: (frames: Readable, context: ProducerIpcContext) => Promise<ProducerIpcReferences>;
  used: boolean;
}
...
export function createProducerIpcHost(input: {
  owner: Owner;
  binding: ProducerIpcBinding;
  exchange: HostState["exchange"];
}): ProducerIpcHost {
  ...
  hosts.set(port, { owner: input.owner, exchange: input.exchange, used: false });
  return port;
}
...
export async function startProducerIpc(input: {
  owner: Owner;
  permit: ResourcePermit;
  binding: ProducerIpcBinding;
  host: ProducerIpcHost;
  signal: AbortSignal;
  timeoutMs: number;
}): Promise<ProducerIpcRun> {
  ipcShape(input, ["owner", "permit", "binding", "host", "signal", "timeoutMs"]);
  const binding = producerIpcBinding(input.binding),
    host = hosts.get(input.host);
  if (
    !host ||
    host.used ||
    host.owner !== input.owner ||
    producerIpcBindingDigest(input.host.binding) !== producerIpcBindingDigest(binding)
  )
    throw Error("original unused host with exact owner/charter/invocation required");
  if (
    !(input.signal instanceof AbortSignal) ||
    !Number.isSafeInteger(input.timeoutMs) ||
    input.timeoutMs < 50 ||
    input.timeoutMs > LIMITS.maxElapsedMs
  )
    throw new TypeError("bounded original cancellation/deadline required");
  const permit = claimResourcePermit(input.owner, input.permit, binding.budgetDigest, producerIpcDemand(binding));
  host.used = true;
  const caller = input.signal,
    deadline = performance.now() + input.timeoutMs,
    abort = new AbortController(),
    signal = AbortSignal.any([caller, abort.signal]);
  const started = deferred<"spawned" | "not-spawned">(),
    readiness = deferred<"frame-ready" | "not-ready">(),
    result = deferred<ProducerIpcSnapshot>();
  let outcome: ProducerIpcSnapshot["outcome"] = "pending",
    reason: ProducerIpcSnapshot["reason"] = null,
    child: ChildRunResult | null = null;
  let childState: ProducerIpcSnapshot["childState"] = "pending",
    hostState: ProducerIpcSnapshot["hostState"] = "not-called",
    settlement: ProducerIpcSnapshot["settlement"] = "pending";
  let references: Readonly<ProducerIpcReferences> | null = null,
    frameSha256: string | null = null,
    frames: Readable | undefined,
    timer: NodeJS.Timeout | undefined;
  const inspect = (): ProducerIpcSnapshot =>
    Object.freeze({
      binding,
      outcome,
      reason,
      child: child ? Object.freeze({ ...child }) : null,
      childState,
      hostState,
      frameSha256,
      references,
  ...
  const latch = (next: Exclude<ProducerIpcSnapshot["outcome"], "pending">, why: ProducerIpcSnapshot["reason"]) => {
    ...
    result.resolve(inspect());
  };
  const cancel = () => {
    latch("cancelled", "CANCELLED");
    abort.abort();
    frames?.destroy();
  };
  const expire = () => {
    latch("timed-out", "DEADLINE");
    abort.abort();
    frames?.destroy();
  };
  const current = () => {
    ...
    else if (performance.now() >= deadline) expire();
  ...
  const childPromise = (async () => {
    try {
      await permit.admission;
    } catch {
      latch("failed", "ADMISSION_FAILED");
      return null;
    }
    ...
    try {
      ...
      return Object.freeze(
        await runChild({
          ...
          args: digestNamespaceArgs(runtime, WORKER, [producerIpcFrame(binding).toString("base64")]),
          env: {},
          cwd: "/",
          signal,
          timeoutMs: LIMITS.childMs,
          killGraceMs: LIMITS.graceMs,
          hardDeadlineAt: Date.now() + LIMITS.childMs + 500,
          maxOutputBytes: LIMITS.frameBytes,
          onSpawn: () => {
            spawned = true;
            started.resolve("spawned");
          },
          onStreamEnd: (s) => {
            if (s === "stdout") eof = true;
          },
          onObservation: (s, b) => {
            if (s !== "stdout" || overflow) return;
            if (stdout.length + b.length > LIMITS.frameBytes) {
              overflow = true;
              return;
            }
            stdout = Buffer.concat([stdout, Buffer.from(b)]);
          },
        }),
      );
    } catch {
      latch("failed", "RUNTIME_FAILED");
      return null;
    }
  })();
  const completion = (async () => {
    child = await childPromise;
    childState = spawned ? (child ? "settled" : "unknown") : "not-spawned";
    started.resolve("not-spawned");
    if (childState === "unknown") {
      readiness.resolve("not-ready");
      settlement = "failed-or-unknown";
      return inspect();
    }
    if (current()) {
      if (
        ...
        latch("failed", "CHILD_FAILED");
      else if (!eof || overflow || !stdout.equals(producerIpcFrame(binding))) latch("failed", "FRAME_INVALID");
      else {
        frameSha256 = createHash("sha256").update(stdout).digest("hex");
        readiness.resolve("frame-ready");
        ...
        try {
          if (current()) {
            hostState = "pending";
            references = ipcReferences(await host.exchange(frames, Object.freeze({ binding, signal })));
            if (!frames.readableEnded) throw Error("original host frame not consumed");
            hostState = "acknowledged";
            if (current()) latch("completed", null);
          }
        } catch {
          hostState = "failed";
          if (current()) latch("failed", "HOST_FAILED");
        } finally {
          frames.destroy();
        }
      }
    ...
    try {
      await permit.settle(terminal === "completed" ? "completed" : terminal === "cancelled" ? "cancelled" : "failed");
