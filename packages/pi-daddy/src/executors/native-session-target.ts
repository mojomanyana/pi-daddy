...
export async function allocateNativeSessionTarget(root: string, executionId: string): Promise<string> {
  if (!isAbsolute(root) || resolve(root) !== root || !/^[a-zA-Z0-9:_-]{1,128}$/.test(executionId))
    throw new Error("explicit native root/execution identity required");
  for (let p = root; ; p = dirname(p)) {
    const s = await lstat(p);
    if (!s.isDirectory() || s.isSymbolicLink()) throw new Error("native target ancestor changed");
    if (p === parse(p).root) break;
  }
  const before = await lstat(root);
  if ((await realpath(root)) !== root || before.mode & 0o077 || before.uid !== process.getuid?.())
    throw new Error("private owned native target root required");
  ...
  if (before.dev !== after.dev || before.ino !== after.ino || (await realpath(directory)) !== directory)
    throw new Error("native target root replaced");
