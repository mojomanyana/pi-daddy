...
export async function buildDailyFixture(scratchParent: string): Promise<Record<string, string>> {
  ...
  const archive = await readFile(
    new URL("../contracts/daily-view/v1/p03/fixtures/retained-executions.json", import.meta.url),
    "utf8",
  );
  await writeFile(join(scratch, "work.jsonl"), work);
  await writeFile(join(scratch, "archive.json"), archive);
  const view = await readDailyView({
    workLedgerPath: join(scratch, "work.jsonl"),
    archiveProjectionPath: join(scratch, "archive.json"),
    workContext: dailyAuthority(),
  });
  return {
    "work.jsonl": work,
    "view.json": JSON.stringify(view, null, 2) + "\n",
    "view.txt": renderDailyView(view, 120) + "\n",
  };
}
if (process.argv[1] && import.meta.url === pathToFileURL(resolve(process.argv[1])).href) {
  const [mode, target, scratch] = process.argv.slice(2);
  if (!["--write", "--check"].includes(mode) || !target || !scratch || process.argv.length !== 5)
    throw new Error("use --write|--check EXPLICIT_TARGET EXPLICIT_SCRATCH_PARENT");
  ...
  for (const [name, text] of Object.entries(files)) {
    ...
    else if ((await readFile(join(resolve(target), name), "utf8")) !== text)
      throw new Error(`daily fixture mismatch: ${name}`);
  }
}
