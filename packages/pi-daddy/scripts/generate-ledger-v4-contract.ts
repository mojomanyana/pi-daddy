...
export function buildLedgerV4ContractFixtures() {
  ...
  const revision = buildWorkRevisionEvent({
    eventId: "revision",
    now,
    revision: {
      kind: "scope",
  ...
  const occurrence = buildWorkOccurrenceEvent({
    eventId: "occurrence",
    now,
    payload: {
      scope,
  ...
  const acceptance = buildWorkAcceptanceEvent({
    eventId: "acceptance",
    now,
    payload: {
      ...
      binding: {
        ...binding,
        scope,
        snapshot: { id: "snapshot", digest: snapshot.payload.snapshot.digest },
  ...
  return {
    "work-revision.json": revision,
    "work-snapshot.json": snapshot,
    "work-occurrence.json": occurrence,
    "work-acceptance.json": acceptance,
  };
}
...
export function buildLayoutOptionsFixture() {
  ...
  const scope = revision("scope", "layout"),
    policy = revision("policy", "layout-policy"),
    artifact = revision("artifact", "layout-image");
  ...
  const snapshot = buildWorkSnapshotEvent({
    eventId: "layout:snapshot",
    now,
    snapshot: {
      snapshotId: "layout-selected",
      scope: ref(scope),
      revisions: revisions.slice(1).map(ref),
      bindings: [binding],
    },
  });
  const occurrences = ["a", "b", "c"].map((variant, i) =>
    buildWorkOccurrenceEvent({
      eventId: `layout:observation:${variant}`,
      now,
      payload: {
        scope: ref(scope),
        obligation: binding.obligation,
        executionId:
          i === 2 ? "exec:00000000-0000-4000-8000-000000000022" : "exec:00000000-0000-4000-8000-000000000021",
        parentExecutionId: null,
        childId: "layout-designer",
        variantId: `layout:${variant}`,
        artifact: binding.artifact,
        provenance: "observed",
        state: "completed",
        labels: {
          sessionId: i === 2 ? "layout-session:b" : "layout-session:a",
          branchLeafId: i === 2 ? "layout-leaf:b" : "layout-leaf:a",
          toolCallId: null,
          taskId: null,
          workspaceId: null,
          definitionDigest: null,
  ...
  const claim = buildWorkAcceptanceEvent({
    eventId: "layout:claim",
    now,
    payload: {
      authorityId: "layout-controller",
      binding: {
        ...binding,
        scope: ref(scope),
        ...
        evidence: [
          {
            id: "layout-proof",
            digest: "d".repeat(64),
            event: { eventId: occurrences[0].eventId, digest: occurrences[0].digest },
          },
        ],
      },
    },
...
export async function generateLedgerV4Contract(
  target: string,
  schemaSource = fileURLToPath(new URL("../contracts/ledger/v4/ledger-event.schema.json", import.meta.url)),
): Promise<void> {
  explicitPath(target);
  explicitPath(schemaSource);
