...
export function runUnitTests(directory = process.cwd()) {
  const batches = unitTestBatches(
    readdirSync(resolve(directory, "test"))
  ...
  for (const batch of batches) {
    ...
    if (remaining <= 0) {
      console.error("UNIT_BATCH_UNEXECUTED " + JSON.stringify(batch));
      failed = true;
      continue;
    }
    console.log("UNIT_BATCH " + JSON.stringify(batch));
    const result = spawnSync(
      process.execPath,
      ["--test", "--test-concurrency=2", `--test-timeout=${batch.timeout}`, ...batch.files],
      { cwd: directory, env, stdio: "inherit", timeout: Math.min(remaining, batch.outer), killSignal: "SIGKILL" },
    );
    if (result.status !== 0) {
      failed = true;
      console.error(
