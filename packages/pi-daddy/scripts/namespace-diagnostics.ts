...
interface FileObservation extends Binary {
  text?: string;
}
...
export async function readDiagnosticFile(path: string, limit: number, digestOnly = false): Promise<FileObservation> {
  ...
  let result: FileObservation | undefined,
    failed = false,
    primary: unknown;
  try {
    ...
    if (
      digestOnly &&
      (bytes !== before.size ||
        ["dev", "ino", "size", "mode", "mtimeMs", "ctimeMs"].some(
          (k) => before[k as keyof typeof before] !== after[k as keyof typeof after],
        ))
    )
      refusal("BINARY_CHANGED");
    result = {
      bytes,
      sha256: hash.digest("hex"),
      mode: (before.mode & 0o7777).toString(8),
      ...(!digestOnly ? { text: Buffer.concat(chunks).toString("utf8") } : {}),
    };
  } catch (error) {
    failed = true;
    primary = error;
...
const systemIO: DiagnosticIO = {
  ...
  version: async () =>
    (
      await promisify(execFile)(BWRAP, ["--version"], {
        cwd: "/",
        env: {},
        timeout: DIAGNOSTIC_LIMITS.versionTimeoutMs,
        maxBuffer: DIAGNOSTIC_LIMITS.versionBytes,
        killSignal: "SIGKILL",
        encoding: "utf8",
      })
    ).stdout,
...
export async function inspectNamespaceDiagnostics(io: DiagnosticIO = systemIO) {
  let kernel: Observation<string>,
    binary: Observation<Binary>,
    version: Observation<string> = unavailable("BINARY_UNAVAILABLE");
  try {
    kernel = boundedValue(io.kernel(), /^[a-zA-Z0-9_.+-]{1,256}$/);
  } catch (error) {
    kernel = unavailable(errorCode(error));
  }
  try {
    ...
    binary =
      ...
      b.bytes >= 0 &&
      b.bytes <= DIAGNOSTIC_LIMITS.binaryBytes &&
      /^[a-f0-9]{64}$/.test(b.sha256) &&
      /^[0-7]{3,4}$/.test(b.mode)
        ? observed({ bytes: b.bytes, sha256: b.sha256, mode: b.mode })
        : unavailable("INVALID_BINARY_METADATA");
    if (binary.state === "observed" && (parseInt(binary.value.mode, 8) & 0o6022) === 0) {
      try {
        version = boundedValue(
          await io.version(),
          /^bubblewrap [0-9][a-zA-Z0-9.+~-]{0,63}$/,
          DIAGNOSTIC_LIMITS.versionBytes,
        );
      } catch (error) {
        version = unavailable(errorCode(error));
      }
    } else version = unavailable("UNSAFE_OR_UNAVAILABLE_BINARY");
  } catch (error) {
    binary = unavailable(errorCode(error));
  }
  ...
  try {
    ...
    for (const field of publicFields) {
      ...
      const pattern = field.startsWith("Cap")
        ? /^[a-fA-F0-9]{1,32}$/
        : field === "NoNewPrivs"
          ? /^[01]$/
          : field === "Seccomp"
            ? /^[012]$/
            : /^\d{1,10}$/;
      processSecurity[field] =
        values.length === 1
          ? boundedValue(values[0], pattern)
          : unavailable(values.length ? "DUPLICATE_FIELD" : "MISSING_FIELD");
    }
  } catch (error) {
    for (const field of publicFields) processSecurity[field] = unavailable(errorCode(error));
  }
  const restrictionReadback: Record<string, Observation<string>> = {};
  for (const [path, pattern] of restrictions) {
    try {
      restrictionReadback[path] = boundedValue(await io.read(path), pattern);
    } catch (error) {
      restrictionReadback[path] = unavailable(errorCode(error));
    }
  }
  return {
    version: "namespace-diagnostics-v1",
    qualification: "not-assessed",
    cause: "unknown",
    kernel,
    bwrap: { file: binary, version },
    processSecurity,
    restrictions: restrictionReadback,
