...
const MUTATIONS = [
  ...
  { name: "grant store: legacy v1 gains retroactive ledger consent",
    ...
    find: "    stored: { grant: parsed.grant as Capability[], projectLedger: parsed.version === 2 },",
    replace: "    stored: { grant: parsed.grant as Capability[], projectLedger: true },",
    expect: "new init opts this project into its ledger" },
  { name: "grant store: a lookalike v1 field becomes ledger consent",
    file: "src/grant-store.ts", test: "test/grant-store.test.ts",
    find: "    stored: { grant: parsed.grant as Capability[], projectLedger: parsed.version === 2 },",
    replace: "    stored: { grant: parsed.grant as Capability[], projectLedger: parsed.projectLedger === true },",
    expect: "new init opts this project into its ledger" },
  { name: "grant store: v2 no longer requires explicit ledger consent",
    file: "src/grant-store.ts", test: "test/grant-store.test.ts",
    find: "  if (parsed.version === 2 && parsed.projectLedger !== true) {\n    return { state: \"refuse\", reason: \"malformed\" };\n  }",
    replace: "  if (false) return { state: \"refuse\", reason: \"malformed\" };",
    expect: "parser rejects every malformed store shape" },
  { name: "session: child environment consults the cwd project store",
    file: "extensions/stored-grant-session.ts", test: "test-integration/governance.it.ts",
    find: "  const state = grantRaw === undefined ? loadStoredGrantStateSync(cwd) : { state: \"absent\" as const };",
    replace: "  const state = loadStoredGrantStateSync(cwd);",
  ...
  { name: "session: stored ledger overrides explicit environment",
    ...
    expect: "explicit environment configuration still bypasses" },
  { name: "session: explicit empty ledger is mistaken for absence",
    file: "extensions/session.ts", test: "test/session-project-ledger.test.ts",
    find: "  const ledgerFromEnvironment = ledgerRaw !== undefined;",
    replace: "  const ledgerFromEnvironment = Boolean(ledgerRaw);",
    expect: "adopting init's project ledger is live now" },
  { name: "session: every legacy stored grant activates a ledger",
    file: "extensions/stored-grant-session.ts", test: "test-integration/governance.it.ts",
    find: "    defaultLedger: stored?.projectLedger || refusal ? projectLedgerPath(cwd) : undefined,",
    replace: "    defaultLedger: stored || refusal ? projectLedgerPath(cwd) : undefined,",
    expect: "stored grant governs the session" },
  ...
  { name: "finalization: cleanup replaces the primary failure",
    file: "src/finalization.ts", test: "test/run-herdr.test.ts",
    find: "    if (failures.length > 0) throw appendFinalizerFailures(primary, failures);",
    replace: "    if (failures.length > 0) throw failures[0].error;",
    expect: "finalizer failure cannot mask the primary" },
  { name: "finalization: attached writer-close failure releases the lease",
    file: "extensions/execute-child.ts", test: "test/execute-child-lifecycle.test.ts",
    find: "  return error instanceof HerdrWriterCloseError ||\n    hasFinalizerError(error, (value) => value instanceof HerdrWriterCloseError);",
    replace: "  return error instanceof HerdrWriterCloseError;",
    expect: "attached Herdr close failure" },
  ...
  { name: "correlation: unknown schema version accepted",
    file: "src/correlation.ts", test: "test/correlation.test.ts",
    find: "    if (key === \"schema_version\" && value !== \"1.0\") {",
    replace: "    if (false) {",
    expect: "correlation schema version and assurance scope" },
  { name: "correlation: assurance scope shape copied without validation",
    file: "src/correlation.ts", test: "test/correlation.test.ts",
    find: "      output[key] = normaliseAssuranceScope(value);",
    replace: "      output[key] = value;",
    expect: "correlation schema version and assurance scope" },
  { name: "tool schema: correlation version widened",
    file: "extensions/correlation-shape.ts", test: "test/delegate-all-wiring.test.ts",
    find: "    schema_version: Type.Optional(Type.Literal(\"1.0\")),",
    replace: "    schema_version: Type.Optional(Type.String()),",
    expect: "all three tool schemas expose" },
  { name: "tool schema: assurance scope widened",
    file: "extensions/correlation-shape.ts", test: "test/delegate-all-wiring.test.ts",
    find: "    assurance_scope: Type.Optional(assuranceScope),",
    replace: "    assurance_scope: Type.Optional(Type.Any()),",
    expect: "all three tool schemas expose" },
  { name: "tool schema: correlation accepts undeclared fields",
    file: "extensions/correlation-shape.ts", test: "test/delegate-all-wiring.test.ts",
    find: "    check_receipt_id: Type.Optional(Type.String()),\n  }, { additionalProperties: false });",
    replace: "    check_receipt_id: Type.Optional(Type.String()),\n  });",
    expect: "all three tool schemas expose" },
  { name: "tool schema: assurance scope accepts undeclared fields",
    file: "extensions/correlation-shape.ts", test: "test/delegate-all-wiring.test.ts",
    find: "      selectors: Type.Array(Type.String({ minLength: 1 }), { maxItems: 0 }),\n    }, { additionalProperties: false }),",
    replace: "      selectors: Type.Array(Type.String({ minLength: 1 }), { maxItems: 0 }),\n    }),",
    expect: "all three tool schemas expose" },
  { name: "contract v3: unknown correlation version accepted",
    file: "contracts/ledger/v3/ledger-event.schema.json", test: "test/ledger-contract.test.ts",
    find: "        \"schema_version\": {\n          \"const\": \"1.0\"\n        },",
    replace: "        \"schema_version\": { \"type\": \"string\" },",
    expect: "closed v3 schema accepts" },
  { name: "contract v3: empty selector scope accepted",
    file: "contracts/ledger/v3/ledger-event.schema.json", test: "test/ledger-contract.test.ts",
    find: "                  \"type\": \"array\",\n                  \"minItems\": 1,\n                  \"items\": {",
    replace: "                  \"type\": \"array\",\n                  \"minItems\": 0,\n                  \"items\": {",
    expect: "closed v3 schema accepts" },
  { name: "model preflight: unresolved model bypasses the catalogue",
    file: "src/model-preflight.ts", test: "test/delegate-all-wiring.test.ts",
    find: "  if (model === undefined || allowUnresolved) return undefined;",
    replace: "  if (true) return undefined;",
    expect: "unresolved model is ledgered" },
  { name: "model preflight: chain asks before resolving its models",
    file: "extensions/delegate-chain.ts", test: "test/delegate-chain-wiring.test.ts",
    find: "        (model) => preflightModel(model, ctx.modelRegistry, session.modelResolutionCache, session.allowUnresolvedModels),",
    replace: "        () => undefined,",
    expect: "unresolved chain model is ledgered" },
  { name: "grant store: unsupported version collapses into a valid absence",
    file: "src/grant-store.ts", test: "test/grant-store.test.ts",
    find: "    return { state: \"refuse\", reason: \"unsupported-version\" };",
    replace: "    return { state: \"absent\" };",
    expect: "loader distinguishes absent" },
  { name: "session: refused store regains the ungoverned wildcard",
    file: "extensions/stored-grant-session.ts", test: "test/session-project-ledger.test.ts",
    find: "    inherited: grantRaw !== undefined ? parseList(grantRaw) : stored?.grant ?? (refusal ? [] : [WILDCARD]),",
    replace: "    inherited: grantRaw !== undefined ? parseList(grantRaw) : stored?.grant ?? [WILDCARD],",
    expect: "invalid project store creates" },
  { name: "session: stored-grant refusal is not ledgered",
    file: "extensions/grant-store-refusal.ts", test: "test-integration/governance.it.ts",
    find: "  if (!session.grantStoreRefusal) return;",
    replace: "  return; if (!session.grantStoreRefusal) return;",
    expect: "unsupported project store loudly refuses" },
  { name: "approval binding: supplied tree state is not bound",
    file: "src/correlation.ts", test: "test/approval-binding.test.ts",
    find: "    ...(input.treeSha !== undefined ? { tree_sha: input.treeSha } : {}),",
    replace: "",
    expect: "planner binds supplied tree state" },
  { name: "approval binding: optional tree fields invalidate every old digest",
    file: "src/correlation.ts", test: "test/approval-binding.test.ts",
    find: "    ...(binding.tree_sha !== undefined ? { tree_sha: binding.tree_sha } : {}),\n    ...(binding.last_change_seq !== undefined ? { last_change_seq: binding.last_change_seq } : {}),",
    replace: "    tree_sha: binding.tree_sha ?? null,\n    last_change_seq: binding.last_change_seq ?? null,",
    expect: "absent tree fields preserve the v1 digest" },
