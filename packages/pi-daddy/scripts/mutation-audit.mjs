...
const MUTATIONS = [
  ...
  { name: "grant store: a lookalike v1 field becomes ledger consent",
    file: "src/grant-store.ts", test: "test/grant-store.test.ts",
    find: "      projectLedger: parsed.version === 2,",
    replace: "      projectLedger: parsed.projectLedger === true,",
    expect: "new init opts this project into its ledger" },
  ...
  { name: "session: every legacy stored grant activates a ledger",
    file: "extensions/session.ts", test: "test-integration/governance.it.ts",
    find: "  const storedLedger = stored?.projectLedger ? projectLedgerPath(storeCwd) : undefined;",
    replace: "  const storedLedger = stored ? projectLedgerPath(storeCwd) : undefined;",
