...
const request = (attemptId: string, more: Partial<AttemptDemand> = {}): AttemptDemand => ({
  attemptId,
  orderId: "order-a",
  ...
  inputBytes: 3,
  inputDigest: hash("abc"),
  ...more,
});
async function fixture(limits = { maxAttempts: 4, maxInputBytes: 12, maxConcurrent: 2 }) {
  const root = await tempDir("resource-budget-");
  await chmod(root, 0o700);
  return createResourceBudget({
    directory: join(root, "authority"),
    authorityDigest: hash("independent-policy"),
    limits,
  });
}
...
test("control snapshots preserve all frozen accounting fields and wait on the actual lock; inspect does not lock", async () => {
  ...
  const first = await budget.reserve(request("snapshot-first")),
    second = await budget.reserve(request("snapshot-second"));
  await first.settle("failed");
  const a = await budget.controlSnapshot(),
    b = await budget.inspect();
  assert.deepEqual(a, b);
  assert.equal(a.attempts, 2);
  assert.equal(a.inputBytes, 6);
  assert.equal(a.active, 1);
  assert.equal(a.reservations[0].outcome, "failed");
  assert.equal(a.reservations[1].outcome, null);
  assert.notEqual(a.reservations, b.reservations);
  assert.ok(Object.isFrozen(a));
  assert.ok(Object.isFrozen(a.reservations));
  assert.ok(Object.isFrozen(a.reservations[0]));
  ...
  assert.equal(await readFile(path + ".lock", "utf8"), "owned-test-held-lock\n");
});

test("creation detaches authority and destination before its first asynchronous boundary", async () => {
  const root = await tempDir("resource-create-");
  await chmod(root, 0o700);
  const directory = join(root, "original"),
    authorityDigest = hash("original");
  ...
  input.directory = join(root, "redirected");
  input.authorityDigest = hash("changed");
  input.limits.maxAttempts = 8;
  const binding = await pending;
  assert.equal(binding.directory, directory);
  assert.equal(binding.authorityDigest, authorityDigest);
  assert.equal(binding.limits.maxAttempts, 4);
});

test("one exact authority reserves aggregate allowance across orders, experiments, retries and shadows", async () => {
  const binding = await fixture(),
    a = openResourceBudget(binding),
    b = openResourceBudget(binding);
  const first = await a.reserve(request("a"));
  const shadow = await b.reserve(
    request("b", { kind: "shadow", parentAttemptId: "a", orderId: "order-b", experimentId: "experiment-b" }),
  );
  ...
  await retry.settle("failed");
  await descendant.settle("completed");
  await assert.rejects(a.reserve(request("e", { inputBytes: 0 })), { code: "EXHAUSTED" });
  const state = await b.inspect();
  assert.equal(state.inputBytes, 12);
  assert.equal(state.attempts, 4);
  assert.equal(state.active, 0);
  assert.ok(Object.isFrozen(state.reservations[0]));
  assert.ok(Object.isFrozen(binding.limits));
});
...
test("restart retains outstanding reservations; duplicate and late receipts do not mint allowance", async () => {
  ...
  const budget = openResourceBudget(binding),
    permit = await budget.reserve(request("lost"));
  ...
  const next = await restarted.reserve(request("next"));
  await permit.settle("cancelled");
  assert.equal((await restarted.inspect()).active, 1);
  await assert.rejects(permit.settle("completed"), { code: "INVALID" });
  await next.settle("completed");
  assert.equal((await restarted.inspect()).inputBytes, 6, "cancellation never refunds bytes");
});
...
test("real cross-process reservations serialize and exact redeliveries never launch twice", async () => {
  ...
  const run = () =>
    promisify(execFile)(
      process.execPath,
      ["--input-type=module", "-e", code, JSON.stringify(binding), JSON.stringify(request("duplicate"))],
      {
        env: { PATH: "", HOME: binding.directory, TMPDIR: binding.directory, PI_CODING_AGENT_DIR: binding.directory },
        timeout: 10000,
      },
    );
  ...
  const results2 = await Promise.all(
    Array.from({ length: 6 }, (_, i) =>
      promisify(execFile)(
        process.execPath,
        ["--input-type=module", "-e", code, JSON.stringify(fresh), JSON.stringify(request(`parallel-${i}`))],
        { env: { PATH: "", HOME: fresh.directory, TMPDIR: fresh.directory }, timeout: 10000 },
      ),
    ),
  );
...
test("byte exhaustion, malformed demands and parent references fail before a reservation", async () => {
  ...
  await assert.rejects(b.reserve(request("orphan", { inputBytes: 0, kind: "retry", parentAttemptId: "absent" })), {
    code: "INVALID",
  });
...
test("independent authority and physical root/journal pins reject mutable or alternate stores", async () => {
  const binding = await fixture(),
    budget = openResourceBudget(binding),
    permit = await budget.reserve(request("a"));
  await assert.rejects(openResourceBudget({ ...binding, limits: { ...binding.limits, maxAttempts: 5 } }).inspect(), {
    code: "AUTHORITY_CHANGED",
  });
  assert.throws(() => openResourceBudget({ ...binding, leaseStore: "elsewhere" } as typeof binding), {
    code: "INVALID",
  });
  const path = join(binding.directory, "budget.jsonl"),
  ...
  await assert.rejects(openResourceBudget({ ...binding, directory: another.directory }).inspect(), {
    code: "AUTHORITY_CHANGED",
  });
...
test("partial journals, aliases and unsafe permissions are unavailable rather than repaired", async () => {
  const binding = await fixture(),
    path = join(binding.directory, "budget.jsonl");
  ...
  const alias = binding.directory + "-alias";
  await symlink(binding.directory, alias);
  await assert.rejects(openResourceBudget({ ...binding, directory: alias }).inspect());
  const unsafe = binding.directory + "-unsafe";
  await mkdir(unsafe, { mode: 0o755 });
  await assert.rejects(
    createResourceBudget({
      directory: join(unsafe, "new"),
      authorityDigest: binding.authorityDigest,
      limits: binding.limits,
    }),
    { code: "INVALID" },
