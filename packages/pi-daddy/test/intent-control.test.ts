...
async function fixture() {
  ...
  const w = intentWorld(),
    ...
    grantPath = join(root, "grants.jsonl");
  await writeFile(path, w.text, { mode: 0o600 });
  await writeFile(grantPath, "owned permission fixture: unchanged\n", { mode: 0o600 });
  const work = await bindWorkIntent({
    path,
    grantLedgerPath: grantPath,
    selection: w.selection(w.base),
    priorities: w.priorities(w.obligations),
  });
  const binding = await createIntentBudget(
    {
      directory: join(root, "budget"),
      authorityDigest: hostDigest,
      limits: { maxAttempts: 8, maxInputBytes: 128, maxConcurrent: 2 },
    },
    work,
  );
  const budget = openResourceBudget(binding),
    controls = budget.intentControls(fixedIntentAuthority(binding));
  return { root, path, grantPath, binding, budget, controls, requests: fixedIntentRequests(binding), w };
}
const demand = (attemptId: string) => ({
  attemptId,
  orderId: "order",
...
test("actual P01 successor append, explicit priority application, and recorded alternative selection", async () => {
  ...
  assert.equal(next.records[0].application, "applied");
  assert.equal(next.records[0].outcome, "selection-applied");
  assert.deepEqual(next.records[0].resultSelection, f.w.selection(f.w.next));
  assert.deepEqual(next.selection, f.w.selection(f.w.next));
  let text = await readFile(f.path, "utf8");
  ...
  assert.equal(projected.scopeState, "valid");
  assert.equal(projected.obligations[0].binding.obligation.revision, 2);
  const view = await readDailyView({
    workLedgerPath: f.path,
    workContext: { selectedSnapshot: next.selection, authority: null },
  });
  assert.equal(view.scope?.revision, 2);
  ...
  assert.equal(priority.records[1].application, "applied");
  assert.equal(priority.records[1].outcome, "priority-applied");
  assert.equal((await f.controls.inspect()).nextObligation?.id, "obligation-1");
  assert.equal(
    await readFile(f.path, "utf8"),
    text,
    "priority is explicit controller policy, not rewritten work or array ordering",
  );
  ...
  assert.deepEqual(alternative.selection, f.w.selection(f.w.alternative));
  assert.equal(await readFile(f.path, "utf8"), text);
  assert.equal(
    projectWorkLedger(text, { selectedSnapshot: alternative.selection, authority: null }).scopeState,
    "valid",
  );
  assert.equal(await readFile(f.grantPath, "utf8"), "owned permission fixture: unchanged\n");
  for (const e of parseWorkLedgerText(text).events)
    if (e.event === "work_revision")
      assert.deepEqual(e.payload.revision.permittedEffects, e.payload.revision.kind === "policy" ? [] : ["read"]);
});
...
test("busy barrier waits for original live reservation; exact duplicate never applies and status never writes", async () => {
  ...
  const state = await f.controls.inspect(),
    permit = await f.budget.reserve(demand("busy"), {
      selection: state.selection,
      revision: state.revision,
      obligation: state.nextObligation!,
    });
  assert.equal((await f.controls.request(f.requests.priority)).records[1].application, "pending-or-unknown");
  assert.equal(
    (await f.budget.controls(null).inspect()).admission,
    "blocked-pending",
    "dispatch readback must include the v3 intent barrier",
  );
  await assert.rejects(
    f.budget.reserve(demand("blocked"), {
      selection: state.selection,
      revision: state.revision,
      obligation: state.nextObligation!,
    }),
  ...
  const files = [f.path, join(f.binding.directory, "budget.jsonl")],
    bytes = await Promise.all(files.map((p) => readFile(p))),
    names = await readdir(f.binding.directory);
  for (let i = 0; i < 3; i++) await f.controls.inspect();
  await f.controls.request(f.requests.priority);
  assert.deepEqual(await Promise.all(files.map((p) => readFile(p))), bytes);
  assert.deepEqual(await readdir(f.binding.directory), names);
  ...
  const after = await readFile(files[1]);
  await restart.reconcile(f.requests.priority);
  assert.deepEqual(await readFile(files[1]), after);
});
...
test("stale binding/selection/authority and permission expansion refuse without changing authoritative intent", async () => {
  ...
  const altered = {
    ...
    expectedSelection: f.w.selection(f.w.alternative),
  };
  ...
  await assert.rejects(f.controls.request({ ...f.requests.priority, bindingDigest: "c".repeat(64) }), {
    code: "AUTHORITY_CHANGED",
  });
  await assert.rejects(f.controls.request({ ...f.requests.revise, selection: f.w.selection(f.w.base) }), {
    code: "DUPLICATE",
  });
});
...
test("reconciliation checks the complete original request against receipt references, not digest-shaped metadata alone", async () => {
  ...
  const permit = await f.budget.reserve(demand("busy-projection"), {
    selection: state.selection,
    revision: state.revision,
    obligation: state.nextObligation!,
  });
  await f.controls.request(f.requests.revise);
  await permit.settle("completed");
  const path = join(f.binding.directory, "budget.jsonl"),
...
test("work append completed but controller receipt failed: unknown until exact request reconciliation, no duplicate intent", async (t) => {
  ...
  await assert.rejects(f.controls.request(f.requests.revise), /owned application receipt failure/);
  fault.mock.restore();
  const text = await readFile(f.path, "utf8");
  assert.equal(parseWorkLedgerText(text).events.length, 11);
  const state = await f.controls.inspect();
  assert.deepEqual(state.selection, f.w.selection(f.w.base));
  assert.equal(state.records[0].application, "pending-or-unknown");
  assert.equal(state.records[0].outcome, "pending-or-unknown");
  assert.equal(state.records[0].resultSelection, null);
...
test("interrupted multi-record application resumes a complete prefix exactly once", async (t) => {
  ...
  let workWrites = 0;
  const fault = t.mock.method(proto, "writeFile", async function (this: unknown, data: unknown, ...args: unknown[]) {
    if (String(data).includes('"event":"work_revision"') && ++workWrites === 2)
      throw new Error("owned between-record interruption");
    return original.call(this, data, ...args);
  });
  await assert.rejects(f.controls.request(f.requests.revise), /owned between-record interruption/);
  fault.mock.restore();
...
test("torn work bytes and physical substitution fail closed without reset or alternate-store recovery", async () => {
  ...
  await rename(g.path, g.path + ".history");
  await writeFile(g.path, g.w.text, { mode: 0o600 });
...
test("real cross-process duplicate applications append one exact work revision set", async () => {
  const f = await fixture(),
    module = new URL("../src/products/resource-budget.ts", import.meta.url).href,
    fixtures = new URL("./intent-control-fixture.ts", import.meta.url).href;
  ...
  await Promise.all(
    Array.from({ length: 3 }, () =>
      promisify(execFile)(process.execPath, ["--input-type=module", "-e", code, JSON.stringify(f.binding)], {
        env: { PATH: "", HOME: f.root, TMPDIR: f.root },
        timeout: 10000,
      }),
    ),
  );
...
test("actual controller crash preserves pending receipts and non-expiring locks; status cannot reclaim them", async () => {
  ...
  const lockPath = join(f.binding.directory, "budget.jsonl.lock"),
    lock = await readFile(lockPath);
...
test("wire parser is duplicate-aware, exact numeric and detached; control receipts retain references, not shadow intent", async () => {
  const f = await fixture(),
    wire = JSON.stringify(f.requests.revise);
  assert.equal(intentRequestDigest(parseIntentRequest(wire)), intentRequestDigest(f.requests.revise));
  const schema = JSON.parse(
    await readFile(new URL(import.meta.resolve("pi-daddy/contracts/intent-control/v1/request.schema.json")), "utf8"),
  );
  const ledger = JSON.parse(
    await readFile(new URL("../contracts/ledger/v4/ledger-event.schema.json", import.meta.url), "utf8"),
  );
  for (const [key, definition] of Object.entries(ledger.$defs))
    assert.deepEqual(
      schema.$defs[`work_${key}`],
      JSON.parse(JSON.stringify(definition).replaceAll("#/$defs/", "#/$defs/work_")),
    );
  ...
  assert.throws(() =>
    parseIntentRequest(wire.replace('"expectedRevision":0', '"expectedRevision":0.00000000000000000001')),
  );
  assert.throws(() =>
    parseIntentRequest(wire.replace('"requestId":"revise"', '"requestId":"revise","requestId":"revise"')),
  );
  const pending = f.controls.request(f.requests.revise);
  f.requests.revise.events.length = 0;
  ...
  assert.equal(text.includes('"contentDigest"'), false);
  assert.equal(text.includes('"permittedEffects"'), false);
});
