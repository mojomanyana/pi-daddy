...
} from "../src/governance/native-session.ts";
import {
...
function assistant() {
  return {
    ...
    content: [{ type: "text" as const, text: "native fixture; no model called" }],
    api: "openai-responses" as const,
    provider: "fixture",
    model: "fixture",
    stopReason: "stop" as const,
    timestamp: 1788888888000,
    usage: {
      input: 0,
      output: 0,
      cacheRead: 0,
...
test("installed SessionManager creates real header and parent linkage without any model runtime", async () => {
  ...
  assert.deepEqual(header, JSON.parse(JSON.stringify(f.manager.getHeader())));
  const parsed = await readNativeSession({ path: f.path, source: "pi-session-file", allowedRoot: f.root });
  assert.equal(parsed.observation.status, "verified");
  assert.equal(parsed.observation.sessionId, f.manager.getSessionId());
  assert.deepEqual(parsed.bytes, raw);
  assert.equal(parsed.observation.lastPersistedEntryId, f.last);
  assert.equal(parsed.observation.branchState, "unknown");
  assert.equal(parsed.observation.branchLeafId, null);
  f.manager.newSession({ parentSession: f.path });
  f.manager.appendMessage(assistant());
  const child = await readNativeSession({
    path: f.manager.getSessionFile()!,
    source: "pi-session-manager",
    manager: f.manager,
    allowedRoot: f.root,
  });
  assert.equal(child.observation.parentSessionPath, f.path);
  assert.notEqual(child.observation.sessionId, parsed.observation.sessionId);
});
...
test("a live native branch can differ from the identical persisted session bytes", async () => {
  ...
  const live = await readNativeSession({
    path: f.path,
    source: "pi-session-manager",
    manager: f.manager,
    allowedRoot: f.root,
  });
  assert.equal(live.observation.branchState, "observed");
  assert.equal(live.observation.branchLeafId, f.first);
  assert.equal(live.observation.lastPersistedEntryId, f.last);
  f.manager.resetLeaf();
  const reset = await readNativeSession({
    path: f.path,
    source: "pi-session-manager",
    manager: f.manager,
    allowedRoot: f.root,
  });
  assert.equal(reset.observation.branchState, "observed");
  assert.equal(reset.observation.branchLeafId, null);
});

test("missing, partial, invalid-parent and duplicate native bytes stay explicitly incomplete", async () => {
  const f = await native(),
    raw = await readFile(f.path);
  const missing = await readNativeSession({
    path: join(f.root, "absent.jsonl"),
    source: "herdr-path",
    allowedRoot: f.root,
  });
  assert.equal(missing.observation.status, "missing");
  ...
  assert.deepEqual(
    interrupted.bytes,
    interruptedUtf8,
    "a valid native header admits the interrupted raw bytes without lossy decoding",
  );
  ...
  assert.equal(partial.observation.status, "truncated");
  assert.equal(partial.observation.branchLeafId, null);
  assert.ok(partial.bytes);
  const bad = Buffer.from(
    raw.toString() +
      JSON.stringify({ type: "message", id: "new", parentId: "absent", timestamp: new Date().toISOString() }) +
      "\n",
  );
  ...
  assert.equal(
    parseNativeSessionBytes(Buffer.from(duplicate), { path: f.path, source: "herdr-path" }).bytes,
    undefined,
  );
  assert.equal(
    parseNativeSessionBytes(raw, { path: f.path, source: "herdr-path", expectedSessionId: "other" }).observation.status,
    "changed",
  );
});
...
test("private-root restrictions reject session aliases and do not harvest arbitrary files", async () => {
  const f = await native();
  const outside = await tempDir("outside-session-");
  assert.equal(
    (await readNativeSession({ path: f.path, source: "herdr-path", allowedRoot: outside })).observation.status,
    "unsupported",
  );
  const alias = join(f.root, "alias.jsonl");
  await symlink(f.path, alias);
  assert.equal(
    (await readNativeSession({ path: alias, source: "herdr-path", allowedRoot: f.root })).observation.status,
    "unsupported",
  );
  const auth = join(f.root, "not-a-session.jsonl");
  await writeFile(auth, '{"access_token":"fixture-not-a-real-credential"}\n');
  ...
  assert.equal(
    (await readNativeSession({ path: f.path, source: "herdr-path", allowedRoot: f.root })).observation.status,
    "unsupported",
  );
});
...
test("replaced native files quarantine the current reference while retaining previous blob bytes", async () => {
  const f = await native();
  const archive = await tempDir("native-archive-");
  ...
  try {
    ...
    h.observeSession({ source: "herdr-path", value: f.path });
    ...
    await rename(f.path, f.path + ".historical");
    await writeFile(f.path, bytes);
    h.observeSession({ source: "herdr-path", value: f.path });
    h.finish(outcome);
    await h.flush();
    const second = parseExecutionRetentionManifest(await readFile(firstPath, "utf8"));
    assert.equal(second.nativeSession.status, "changed");
    assert.equal(second.content.session.status, "missing");
    assert.equal(second.native.sessionId, null);
    assert.equal(first.nativeSession.status, "verified");
...
test("native observation detaches queued references and explicitly records coalesced reference loss", async () => {
  const f = await native(),
    archive = await tempDir("queued-native-archive-");
  const prior = process.env[ENV_NATIVE_SESSION_ROOT];
  process.env[ENV_NATIVE_SESSION_ROOT] = f.root;
  try {
    const h = beginExecutionRetention(identity, archive),
      reference = { source: "herdr-path" as const, value: f.path };
    h.observeSession(reference);
    reference.value = join(f.root, "absent.jsonl");
    h.finish(outcome);
    const path = (await h.flush()).manifestPath!;
    const m = parseExecutionRetentionManifest(await readFile(path, "utf8"));
    assert.equal(
      m.nativeSession.status,
      "verified",
      "later caller mutation cannot redirect an already-observed reference",
    );
    ...
    dropped.observeSession({ source: "herdr-path", value: f.path });
    dropped.finish(outcome);
...
test("the governed process seam retains actual private SessionManager bytes from an explicit host target", async () => {
  ...
  try {
    const plan = planDelegation(
      { task: "fixture", tools: [] },
      { ownGrant: [], depth: 0, maxDepth: 2, gated: [], sessionFile: f.path },
    );
    assert.ok(plan.args.includes("--session"));
    assert.ok(!plan.args.includes("--no-session"));
    const result = await executePlannedChild({
      session: { executor: { kind: "process" } } as never,
      plan,
      childId: "reused",
      executionId: "exec:fixture-process",
      parentExecutionId: "exec:parent",
    ...
    const path = drained.manifestPath!,
...
test("existing Herdr native replies bind session bytes to the exact pane and execution", async () => {
  const f = await native(),
    archive = await tempDir("herdr-native-archive-");
  const prior = process.env[ENV_NATIVE_SESSION_ROOT];
  process.env[ENV_NATIVE_SESSION_ROOT] = f.root;
  try {
    const h = beginExecutionRetention(identity, archive);
    ...
    const info = {
      pane_id: "w1:p2",
      agent: "pi",
      agent_session: { agent: "pi", source: "fixture-native-integration", kind: "path", value: f.path },
    };
    assert.equal(herdrSessionReference(info, "different-pane"), null);
    assert.equal(
      herdrSessionReference({ ...info, agent_session: { ...info.agent_session, agent: "other" } }, "w1:p2"),
      null,
    );
    const run = await runHerdrPane({
      args: ["--no-session"],
      prompt: "fixture",
      env: {},
      cwd: f.root,
      name: "same-name",
      keepPane: true,
      onSessionReference: (ref) => h.observeSession(ref),
      exec: async (args) => {
        ...
        const result =
          verb === "tab create"
            ...
            : verb === "agent start"
              ? { agent: { ...info, state_change_seq: 1 } }
              : verb === "agent get"
                ? {
                    agent: {
                      ...info,
                      state_change_seq: ++gets + 1,
                      agent_status: "idle",
    ...
    const m = parseExecutionRetentionManifest(await readFile(path, "utf8"));
    assert.equal(m.native.sessionId, f.manager.getSessionId());
    assert.equal(m.identity.toolCallId, "call:exact");
    assert.equal(m.native.branchLeafId, null);
    assert.equal(m.content.session.status, "retained");
