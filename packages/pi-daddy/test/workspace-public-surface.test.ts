...
const LEASE_NAMES = ["ENV_WORKSPACE_LEASE_DIR", "acquireWorkspaceLease", "defaultWorkspaceLeaseDir", "leaseAcquisitionOutcome", "leaseReleaseLedgerOutcome"];
...
test("pi-daddy/workspace exposes every routing export and the lease surface it had before the split", async () => {
  ...
  for (const name of Object.keys(routing)) assert.ok(exported.has(name), `routing export missing from subpath: ${name}`);
  ...
  assert.deepEqual(manifest.exports["./workspace"], { types: "./dist/governance/workspace-public.d.ts", default: "./dist/governance/workspace-public.js" });
});
