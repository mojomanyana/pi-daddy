...
export async function connectedHarness(root: string) {
  ...
  for(const pin of pins){const bytes=await readFile(new URL(pin.target,sources),"utf8");if(hash(bytes)!==pin.sha256||pin.commit!=="28b55d40a64ce7af8ed23410a137f2e3a075e522")throw Error("pinned host source drift");
  ...
  const typeboxRoot=await realpath(new URL("../node_modules/typebox",import.meta.url));const artifact:DashboardHarnessArtifact={version:"dashboard-harness-artifact-v1",sourceCommit:"28b55d40a64ce7af8ed23410a137f2e3a075e522",files,typeboxRoot,typeboxPackageSha256:hash(await readFile(join(typeboxRoot,"package.json")))};
