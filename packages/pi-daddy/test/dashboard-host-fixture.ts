...
export async function connectedHarness(root: string) {
  ...
  for(const pin of pins){const bytes=await readFile(new URL(pin.target,sources),"utf8");if(hash(bytes)!==pin.sha256||pin.commit!=="73ab11883bb1c8924dc5ebbe8a61c96e913ee437")throw Error("pinned host source drift");
  ...
  const typeboxRoot=await realpath(new URL("../node_modules/typebox",import.meta.url));const artifact:DashboardHarnessArtifact={version:"dashboard-harness-artifact-v1",sourceCommit:"73ab11883bb1c8924dc5ebbe8a61c96e913ee437",files,typeboxRoot,typeboxPackageSha256:hash(await readFile(join(typeboxRoot,"package.json")))};
