...
export async function connectedHarness(root: string) {
  ...
  for(const pin of pins){const bytes=await readFile(new URL(pin.target,sources),"utf8");if(hash(bytes)!==pin.sha256||pin.commit!=="d123257e53d48a2cad6919708976b5371dc7590e")throw Error("pinned host source drift");
  ...
  const typeboxRoot=await realpath(new URL("../node_modules/typebox",import.meta.url));const artifact:DashboardHarnessArtifact={version:"dashboard-harness-artifact-v1",sourceCommit:"d123257e53d48a2cad6919708976b5371dc7590e",files,typeboxRoot,typeboxPackageSha256:hash(await readFile(join(typeboxRoot,"package.json")))};
