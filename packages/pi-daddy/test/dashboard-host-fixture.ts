...
export async function connectedHarness(root: string) {
  ...
  for(const pin of pins){const bytes=await readFile(new URL(pin.target,sources),"utf8");if(hash(bytes)!==pin.sha256||pin.commit!=="2a36632f8780289a2cb82fa42efff440e8530f9a")throw Error("pinned host source drift");
  ...
  const typeboxRoot=await realpath(new URL("../node_modules/typebox",import.meta.url));const artifact:DashboardHarnessArtifact={version:"dashboard-harness-artifact-v1",sourceCommit:"2a36632f8780289a2cb82fa42efff440e8530f9a",files,typeboxRoot,typeboxPackageSha256:hash(await readFile(join(typeboxRoot,"package.json")))};
