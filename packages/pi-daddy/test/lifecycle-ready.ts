...
export async function liveFixtureReady(path: string, stderr: () => Promise<string> = async () => "") {
  ...
  while (Date.now() < deadline) {
    ...
    assert.equal(record.ready, true, "fixture must acknowledge actual Node event-loop readiness");
    assert.ok(Number.isSafeInteger(record.pid) && record.pid > 0);
    try {
      process.kill(record.pid, 0);
    } catch (error) {
      assert.fail(`owned fixture exited before teardown readiness: ${String(error)}; ${await stderr()}`);
    }
