...
import { createDashboardHost, openDashboardHost, dashboardHostDigest, dashboardHostRequestDigest, type DashboardHostRequest, type DashboardHostConfig, type DashboardHostAuthority } from "../src/products/dashboard-host.ts";
...
export async function hostWorld(blind=true,kind:"signals"|"zero"|"v2"|"experiment"|"retention"="signals",ordinary?:OrdinaryChildren,withLearning=false){
  ...
  return {root,api,archiveRoot,modules,pins,retention,learningLifecycleId,w,workPath,config,budget,trust,options,host,request,pause,presence,experiment,get authority(){return authority;},set authority(a){authority=a;},reopen:()=>openDashboardHost(options)};
}
