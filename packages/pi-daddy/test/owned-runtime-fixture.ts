...
export async function ownedRuntimeFixture(directory: string) {
  const source = await realpath(process.execPath),
    ...
    target = join(directory, "node");
  assert.ok(
    before.isFile() && before.size <= 268435456,
    "bounded regular retained Node required for the missing-bwrap fixture",
  );
  const hash = async (path: string) => {
    ...
    for await (const chunk of createReadStream(path)) {
      bytes += chunk.length;
      if (bytes > 268435456) throw new Error("runtime fixture source grew beyond bound");
      digest.update(chunk);
    }
  ...
  assert.equal(await hash(target), expected);
  assert.equal(await hash(source), expected);
