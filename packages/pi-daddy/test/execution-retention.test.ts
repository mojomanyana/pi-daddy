...
async function manifest(path: string): Promise<ExecutionRetentionManifest> {
...
test("governed interruption publishes retention without changing the control outcome", async () => {
  ...
  const old = process.env[ENV_EXECUTION_ARCHIVE];
  process.env[ENV_EXECUTION_ARCHIVE] = dir;
  try {
    ...
    for (let i = 0; i < 100; i++) {
      try {
        found = await manifest((outcome as any).retention.manifestPath);
      } catch {}
    ...
    old === undefined ? delete process.env[ENV_EXECUTION_ARCHIVE] : (process.env[ENV_EXECUTION_ARCHIVE] = old);
  }
});

test("public concurrent delegation calls retain distinct native attempts and exact call joins", async () => {
  const dir = await tempDir("p02-public-");
  ...
  try {
    ...
    grantsExtension({
      on: (n: string, fn: unknown) => hooks.set(n, fn),
...
test("raw structured bytes survive interruption and throwing observers cannot cancel control", async () => {
  const dir = await tempDir("p02-raw-");
  const h = beginExecutionRetention(identity, dir);
  const abort = new AbortController();
  const run = await runChild({
    command: process.execPath,
    args: ["-e", "process.stdout.write('{\\\"partial\\\":true}\\n');setTimeout(()=>{},10000)"],
    cwd: dir,
    env: {},
    signal: abort.signal,
    killGraceMs: 10,
    onObservation: (stream, bytes) => {
      h.capture(stream, bytes);
      abort.abort();
    },
  });
  assert.equal(run.aborted, true);
  assert.equal(run.spawnError, undefined);
  h.finish({
    code: run.code,
...
test("herdr observations reuse existing native replies and snapshots without extra control calls", async () => {
  const dir = await tempDir("p02-herdr-");
  const h = beginExecutionRetention({ ...identity, executor: "herdr" }, dir);
  const calls: string[] = [];
  let gets = 0;
  const run = await runHerdrPane({
    ...
    name: "reused",
    keepPane: true,
    onPane: (paneId, agentName) => h.native({ paneId, agentName }),
    onNativeTab: (tabId) => h.native({ tabId }),
      ...
      const result =
        verb === "tab create"
          ...
          : verb === "agent start"
            ...
            : verb === "agent get"
              ? { agent: { state_change_seq: ++gets + 1, agent_status: "idle", screen_detection_skipped: true } }
              : verb === "agent read"
                ? { output: "native pane bytes" }
                : { ok: true };
  ...
  assert.equal(m.native.paneId, "w1:p2");
  assert.equal(m.native.tabId, "w1:t2");
  assert.match(m.native.agentName!, /^reused/);
  assert.equal(m.native.sessionId, null);
...
test("complete receipt bytes have reserved capacity and remain distinct from a reference", async () => {
  const dir = await tempDir("p02-receipt-");
  const h = beginExecutionRetention(identity, dir);
  h.capture("stdout", Buffer.alloc(1024 * 1024));
  const bytes = Buffer.from(
    '{"schema_version":"1.0","receipt_id":"fixture","complete":"all fields supplied by caller"}\n',
  );
  h.capture("checkReceipt", bytes);
  h.finish(terminal);
  const path = (await h.flush()).manifestPath!;
  const m = await manifest(path);
  ...
  assert.equal(m.content.session.status, "missing");
...
test("retention publishes exact bytes and an independent manifest for reused names", async () => {
  ...
  a.capture("stdout", bytes);
  b.capture("stdout", Buffer.from("second"));
  a.native({ pid: 123 });
  a.finish(terminal);
  b.finish({ ...terminal, code: 7, failed: true });
  const [ar, br] = await Promise.all([a.flush(), b.flush()]);
  assert.equal(ar.status, "retained");
  assert.notEqual(ar.manifestPath, br.manifestPath);
  const m = await manifest(ar.manifestPath!);
  assert.equal(m.identity.toolCallId, "call:a");
  assert.equal(m.native.pid, 123);
  ...
  assert.equal(m.native.branchLeafId, null);
  assert.equal(m.coverage.complete, false);
  assert.equal(m.acceptance, "not-assessed");
});
...
test("observation loss and missing receipt bytes never become complete evidence", async () => {
  ...
  const failed = beginExecutionRetention(identity, file);
  failed.finish(terminal);
