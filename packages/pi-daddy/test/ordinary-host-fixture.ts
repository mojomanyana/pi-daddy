...
export async function ordinaryHostFixture(root: string) {
  ...
  await writeFile(
    join(bin, "pi"),
    `#!${process.execPath}\nimport fs from 'node:fs';const task=process.argv.at(-1).trim();fs.writeFileSync(task+'.ready','ready',{flag:'wx'});process.stdout.write('owned output');if(task==='fast')process.exit(0);setTimeout(()=>process.exit(0),8000);`,
    { mode: 0o700 },
  );
  const env = {
      PATH: bin,
  ...
  try {
    ...
    return {
      ...
      async ready(task: string) {
        const end = Date.now() + 3000;
        while (true) {
          try {
            if ((await readFile(join(root, task + ".ready"), "utf8")) === "ready") return;
          } catch (error) {
            if (Date.now() > end) throw error;
          }
          if (Date.now() > end) throw Error("owned child readiness unavailable");
