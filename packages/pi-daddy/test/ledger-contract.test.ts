...
test("the closed v3 schema accepts fixtures and rejects v2, extra fields and missing identity", async () => {
  ...
  for (const [name, event] of Object.entries(buildLedgerV3ContractFixtures())) {
    assert.equal(
      ...
      true,
      `${name}: ${[...validator.Errors(event)].map((e) => e.message).join(", ")}`,
    );
  }
  ...
  assert.equal(
    validator.Check({
      ...fixture,
      correlation: { ...fixture.correlation, policy_label: "SECRET TASK TEXT" },
    }),
    false,
  );
  assert.equal(
    validator.Check({
      ...fixture,
      correlation: { ...fixture.correlation, assurance_scope: null },
    }),
    false,
    "the schema must agree with runtime normalization, which never emits a null scope",
  );
  assert.equal(
    validator.Check({
      ...fixture,
      correlation: { ...fixture.correlation, schema_version: "1.1" },
    }),
    false,
    "unknown upstream schema versions fail at the version field",
  );
  assert.equal(
    validator.Check({
      ...fixture,
      correlation: { ...fixture.correlation, assurance_scope: { type: "selectors", selectors: [] } },
    }),
    false,
    "selector scope is non-empty",
  );
  assert.equal(
    validator.Check({
      ...fixture,
      correlation: { ...fixture.correlation, assurance_scope: { type: "entire-run", selectors: ["src/**"] } },
    }),
    false,
