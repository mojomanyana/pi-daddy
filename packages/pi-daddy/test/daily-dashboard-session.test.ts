...
import {
  createDailyDashboardSession,
  dailyDashboardPaths,
  preparePrivateHostRoot,
} from "../extensions/daily-dashboard-session.ts";
...
test("production host state avoids reserved .pi paths and keeps the private socket bounded", () => {
  ...
  assert.equal(paths.directory.includes("/.pi/"), false);
  assert.equal(paths.socketPath.includes("/.pi/"), false);
...
test("existing or partial daily host IDs refuse before bridge startup and preserve their bytes", async () => {
  const root = await tempDir("daily-host-reuse-"),
    cwd = "/workspace/project",
    id = "partial-id",
    paths = dailyDashboardPaths(cwd, id, root);
  await mkdir(paths.directory, { recursive: true, mode: 0o700 });
  const marker = paths.directory + "/partial";
  await writeFile(marker, "preserve");
  const session = createDailyDashboardSession({
    ordinary: () => ({}) as never,
    declared: () => ({}) as never,
    rebind: () => {},
    cwd: () => cwd,
...
test("host setup creates only owner-private state and refuses unsafe or linked prior state", async () => {
  const root = await tempDir("daily-host-state-"),
    fresh = join(root, "fresh"),
    unsafe = join(root, "unsafe"),
