...
test("missing namespace prerequisites remain a failure but cannot leak the fixture listener", async () => {
  ...
  try {
    await promisify(execFile)(
      ...
      [
        "--import",
        new URL("./effect-profile-missing-runtime.mjs", import.meta.url).pathname,
        "--test",
        "--test-name-pattern=^actual namespace denies",
        new URL("./effect-profile.test.ts", import.meta.url).pathname,
      ],
      { env, timeout: 8000 },
    );
  ...
  await writeFile(
    join(dir, "missing-runtime.json"),
    JSON.stringify({ code: result.code, killed: result.killed, stdout: result.stdout, stderr: result.stderr }, null, 2),
  );
...
test("ordinary inspect still refuses a real concurrent append without taking a control lock", async () => {
  const root = await tempDir("read-only-race-"),
    binding = await createResourceBudget({
      directory: join(root, "budget"),
      authorityDigest: "a".repeat(64),
      limits: { maxAttempts: 2, maxInputBytes: 4, maxConcurrent: 2 },
    });
  ...
  const file = await open(join(binding.directory, "budget.jsonl"), "r"),
  ...
  try {
    await assert.rejects(budget.inspect(), { code: "AUTHORITY_CHANGED" });
  } finally {
    proto.read = original;
  }
