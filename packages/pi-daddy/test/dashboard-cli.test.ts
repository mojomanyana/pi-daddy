...
test("activity dashboard filters and loads exact private prompt/final only on demand", async () => {
  const cwd = await tempDir("dashboard-activity-");
  const recorder = new ActivityTimelineRecorder(cwd, { PI_DADDY_ACTIVITY_ROOT: "root-a" });
  await recorder.start("exact submitted prompt", "model-a", "high");
  await recorder.skill("skill_available", { name: "review", source: "/skills/review", digest: "a".repeat(64) });
  await recorder.finish("exact final response");
  const task = parseActivityTimeline(
    await import("node:fs/promises").then(({ readFile }) => readFile(defaultActivityTimelinePath(cwd), "utf8")),
  ).tasks[0]!;
  ...
  assert.match(compact, new RegExp(`legacy ${taskKey}`));
  assert.doesNotMatch(compact, /exact submitted prompt/);
...
test("dashboard keeps displayed short aliases stable and resolves them only to their own root-qualified task", async () => {
  const cwd = await tempDir("dashboard-activity-alias-"),
    recorder = new ActivityTimelineRecorder(cwd, { PI_DADDY_ACTIVITY_ROOT: "root-a" });
  await recorder.start("first prompt");
  await recorder.finish("first final");
  const display = createDashboardDisplayControls(true, true);
  const frame = () =>
    dashboardFrame({ cwd, color: false, width: 120, ...display.state, activityAliases: display.aliases });
  ...
  assert.match(
    ...
    "the old alias keeps selecting root-a after another root appears",
  );
});

test("dashboard detail commands select same-named tasks by root key and reject shorthand", async () => {
  const cwd = await tempDir("dashboard-activity-roots-"),
    first = new ActivityTimelineRecorder(cwd, { PI_DADDY_ACTIVITY_ROOT: "root-a" }),
    second = new ActivityTimelineRecorder(cwd, { PI_DADDY_ACTIVITY_ROOT: "root-b" });
  await first.childStarted("same-task", undefined, "one", "first prompt");
  await first.childFinished("same-task", undefined, "one", "first final", "completed");
  await second.childStarted("same-task", undefined, "two", "second prompt");
  await second.childFinished("same-task", undefined, "two", "second final", "completed");
  const firstKey = activityTaskKey("root-a", "same-task"),
    secondKey = activityTaskKey("root-b", "same-task");
  for (const [taskKey, field, expected] of [
    [firstKey, "prompt", "first prompt"],
    [firstKey, "final", "first final"],
    [secondKey, "prompt", "second prompt"],
    [secondKey, "final", "second final"],
  ] as const)
    assert.match(await dashboardFrame({ cwd, activityDetail: { taskKey, field } }), new RegExp(expected));
...
test(
  "ledger CLI readline accepts h, details, and collapse without dispatching an action",
  { timeout: 15000 },
  async () => {
    ...
    const child = spawn(
      process.execPath,
      [join(import.meta.dirname, "..", "src", "products", "dashboard-cli.ts"), "--ledger", ledgerPath, "--no-color"],
