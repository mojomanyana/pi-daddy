...
const fixtures = new URL("fixtures/debrief-host/", import.meta.url),
  contract = new URL("../contracts/debrief/v1/", import.meta.url);
...
export async function harnessFixture(root: string) {
  const modules = join(root, "host-modules");
  ...
  const cases = await json("work-capture/fixtures/cases.json"),
    batch = await json("work-capture/fixtures/batch.json");
  const candidateIds = cases.map((c: unknown) => capture.retainWorkCandidate(archiveRoot, c));
  if (JSON.stringify(candidateIds) !== JSON.stringify(batch.candidateIds))
    throw new Error("actual host archive fixture IDs drift");
  const batchId = archive.retainArchiveSource(archiveRoot, {
  ...
  const manifest = await json("intervention/fixtures/manifest.json"),
    blindView = await json("intervention/fixtures/blind-view.json");
  // Same explicit synthetic evidence/roles as the immutable fixture producer; not host authentication.
  const artifacts = new Map<string, Buffer>();
  for (const c of blindView.cards)
    for (const digest of c.artifactDigests)
      artifacts.set(digest, await readFile(new URL(`intervention/fixtures/artifact-${digest}.txt`, contract)));
  const sha = (b: string | Uint8Array) => createHash("sha256").update(b).digest("hex");
  const evidence = ["a", "b"].map((armId, i) => {
    const digest = sha(`Synthetic Layout ${armId.toUpperCase()}\n`);
    return {
      armId,
  ...
  const qualification = {
    ...
    proposer: { requested: "fixture:proposer", canonical: "fixture-proposer" },
    judge: { requested: "fixture:judge", canonical: "fixture-judge" },
    subjects: {
      a: { requested: "fixture:a", canonical: "fixture-subject-a" },
      b: { requested: "fixture:b", canonical: "fixture-subject-b" },
    },
    evidenceDigests: Object.fromEntries(evidence.map((e) => [e.armId, intervention.interventionEvidenceDigest(e)])),
    artifacts,
  };
...
export function attentionFixture(root: string): DebriefPersistence {
  const path = join(root, "fixture-host-attention.json");
  let known = false;
  return {
    ...
    async load() {
      try {
        ...
        for (const line of text.trimEnd().split("\n")) {
          ...
          if (record.prior !== (value ? dataDigest(value) : null)) throw new Error("attention history changed");
          value = record.next;
        }
        known = true;
        return value;
      } catch (error) {
        if (!known && (error as { code?: string }).code === "ENOENT") return null;
        throw error;
      }
    },
    async compareAndSwap(expected, next) {
      await withFileLock(
        ...
        async () => {
          ...
          try {
            await file.writeFile(JSON.stringify({ prior: expected, next: detached(next) }) + "\n");
            await file.sync();
          } finally {
            await file.close();
          }
