...
const bodySentinel = operation.endsWith("-undefined")
  ? undefined
  : Object.freeze({ fixture: "primary callback error" });
...
if (cleanupFault) {
  fs.readFile = (async (...args: Parameters<typeof fs.readFile>) => {
    if (String(args[0]) === lockPath && cleanupReady) {
      ...
      if (cleanupHas("read")) {
        hit = true;
        throw Object.assign(new Error("fixture lock read failure"), { code: "EACCES" });
      }
    }
  ...
  fs.rm = async (...args: Parameters<typeof fs.rm>) => {
    if (String(args[0]) === lockPath && cleanupReady) {
      cleanup.removals++;
      if (cleanupHas("unlink")) {
        hit = true;
        throw Object.assign(new Error("fixture lock unlink failure"), { code: "EACCES" });
      }
    }
...
const observation = () => ({
  ...(observing ? { observation: { path, protection, mutations } } : {}),
  ...(cleanupFault ? { cleanup } : {}),
});
const injected = () =>
  Object.assign(new Error("fixture filesystem failure"), { code: fault === "preflight" ? "EACCES" : "EIO" });
const originalOpen = fs.open,
  originalLstat = fs.lstat,
  originalRealpath = fs.realpath,
  originalStat = fs.stat,
  originalMkdir = fs.mkdir;
if (["preflight", "resolution"].includes(fault))
  fs.mkdir = (async (...args: Parameters<typeof fs.mkdir>) => {
    if (String(args[0]).startsWith(resolve(root) + sep) || String(args[0]) === resolve(root))
      throw new Error("preflight attempted mkdir");
...
const timer = setTimeout(() => {
  process.stderr.write("fixture child deadline\n");
  process.exit(124);
}, 30_000);
if (fault === "preflight")
...
fs.open = (async (...args: Parameters<typeof fs.open>) => {
  if (["preflight", "resolution"].includes(fault) && String(args[0]).startsWith(resolve(root) + sep))
    throw new Error("preflight attempted open");
  if (String(args[0]) === path && fault === "gone") {
    hit = true;
    await fs.rename(path, path + ".gone");
  }
  if (String(args[0]) === path && fault === "open") {
    hit = true;
    throw injected();
  }
  const flags = args[1];
  ...
  if (cleanupFault && String(args[0]) === lockPath && flags === "wx") {
    ...
    handle.writeFile = async (...values: Parameters<typeof handle.writeFile>) => {
      await write(...values);
      cleanup.token = String(values[0]);
    };
    handle.close = async () => {
      cleanup.closes++;
      await close();
      cleanupReady = true; // close the real descriptor; do not leak it in the test
      if (cleanupHas("replacement")) await fs.writeFile(lockPath, "replacement-owner\n");
      if (cleanupHas("close")) {
        hit = true;
        throw Object.assign(new Error("fixture lock close failure"), { code: "EIO" });
      }
    };
    return handle;
  }
  ...
  if (fault === "grow") {
    hit = true;
    await fs.appendFile(path, " ".repeat(16 * 1024 * 1024));
  }
  if (fault === "swap") {
    hit = true;
  ...
  if (fault === "alias-after-open") {
    hit = true;
    await fs.link(path, protection);
  }
  if (fault === "alias-swap") {
    hit = true;
    await fs.rename(path, protection);
    await fs.writeFile(path, "replacement");
  }
...
try {
  ...
  if (operation.startsWith("lock-")) {
    ...
    const work = async () => {
      if (operation.includes("-body")) throw bodySentinel;
      return "body-result";
    };
    const value = operation.startsWith("lock-default")
      ? await withFileLock(path, "fixture", work)
      : await withFileLock(path, "fixture", work, {
          staleRecovery: operation.startsWith("lock-disabled") ? "disabled" : "age",
        });
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      server.listen(basename(path), resolve);
    });
    process.send?.("paused");
    await released;
    ...
    await api.appendWorkLedgerEvent(
      { path, grantLedgerPath: protection === "null" ? null : protection },
  ...
  if (operation.startsWith("lock-") && error === bodySentinel) {
    ...
    process.stdout.write(
      JSON.stringify({ ok: false, hit, name: error.name, code: error.code, ...observation() }) + "\n",
    );
  }
} finally {
  clearTimeout(timer);
