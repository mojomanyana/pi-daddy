...
test("daily host offers case preparation only at explicit quiescent closing pause and defers without trust policy",async()=>{
 ...
 try{const remote=connectDashboardHost(running.socketPath);let frame=await remote.frame() as any;assert.equal(frame.actions.some((x:any)=>x.key==="prepare-case-cards"),false);await remote.humanAction("pause-new-dispatch");frame=await remote.frame() as any;const prepare=frame.actions.find((x:any)=>x.key==="prepare-case-cards");assert.ok(prepare);const prepared=await running.host.humanAction(prepare.key) as any;assert.equal(prepared.result.state,"deferred");assert.equal(prepared.result.reason,"policy-unavailable");frame=await remote.frame() as any;assert.equal(frame.debrief,null);assert.equal(frame.attention.attentionUsed,0);assert.equal(frame.actions.some((x:any)=>x.key==="acknowledge-case-cards"),false);await remote.humanAction("resume-dispatch");}
 finally{await running.endSession();await child.close();}
});
...
test("daily host retains a usable current cancellation through 75 same-tip sibling lifecycles", async () => {
  ...
  try {
    const remote=connectDashboardHost(running.socketPath),initial=await remote.frame(),oldCancel=initial.actions.find(a=>a.operation==="ordinary-cancel");assert.ok(oldCancel);
    for(let index=0;index<75;index++){
      ...
      const active=await remote.frame();assert.equal(active.error,null);assert.equal(active.actions.filter(a=>a.operation==="ordinary-cancel").length,2);
      sibling.settle({state:"settled"},"not-assessed");const settled=await remote.frame();assert.equal(settled.error,null);assert.equal(settled.actions.filter(a=>a.operation==="ordinary-cancel").length,1);
    }
    const frame=await remote.frame(),cancellations=frame.actions.filter(a=>a.operation==="ordinary-cancel");assert.equal(frame.tip,initial.tip);assert.equal(frame.error,null);assert.equal(cancellations.length,1,"the original child must retain a fresh exact cancellation after cumulative sibling churn");const cancel=cancellations[0];
    assert.notEqual(cancel.key,oldCancel.key);await assert.rejects(remote.humanAction(oldCancel.key,{tip:initial.tip,requestDigest:oldCancel.requestDigest}),/dashboard refused or acknowledgement unknown/);assert.equal(first.signal.aborted,false,"the old same-tip choice must not cancel a current target");
    assert.equal(cancel.label,"Stop active agent 1");await remote.humanAction(cancel.key,{tip:frame.tip,requestDigest:cancel.requestDigest});assert.equal(first.signal.aborted,true);assert.equal((child.port.inspect() as any).children.find((row:any)=>row.target.toolCallId==="call:original").abortRequested,true);assert.equal((child.port.inspect() as any).children.filter((row:any)=>row.target.toolCallId?.startsWith("call:sibling-")).every((row:any)=>row.state==="settled"),true,"the current cancellation must not affect settled siblings");
