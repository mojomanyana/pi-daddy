...
test("supported daily host command composition publishes fresh work and pauses only new ordinary dispatch", async () => {
  ...
  try {
    ...
    assert.equal(first.source.daily.obligations.length,1);assert.equal(first.source.daily.attempts.length,1);assert.equal(first.source.observations.length,2);assert.equal(first.source.unobservedSources.length,0);
    assert.deepEqual(first.actions.map((x:any)=>x.key),["pause-new-dispatch"]);await remote.humanAction("pause-new-dispatch");
    ...
    assert.deepEqual((await remote.frame() as any).actions.map((x:any)=>x.key),["resume-dispatch"]);await remote.humanAction("resume-dispatch");
