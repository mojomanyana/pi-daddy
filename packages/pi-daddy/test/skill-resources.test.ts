...
async function fixture(local = false) {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "example-skills", version: "1.0.0", pi: { skills: ["./skills/*"] } }),
  );
...
for (const local of [false, true])
  test(`${local ? "project" : "global"} configured npm skills stay installed through init, repeat and force`, async () => {
    ...
    for (const force of [false, false, true]) {
      ...
      assert.equal(
        await stat(join(cwd, ".pi", "skills")).then(
          () => true,
          () => false,
        ),
        false,
...
test("package filters exclude disabled skills from setup, runtime and catalog; local override supplies the ceiling", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ packages: [{ source: "npm:example-skills", skills: ["!skills/other/**"] }] }),
  );
...
test("disabled packages cannot return through legacy npm scanning; missing packages do not install", async () => {
  ...
  await writeFile(
    join(base, "settings.json"),
    JSON.stringify({ packages: [{ source: "npm:example-skills", skills: [] }, "npm:missing-skills"] }),
  );
  assert.deepEqual(await discoverSkillPackages(cwd), []);
  assert.equal((await loadDefinitions(cwd)).size, 0);
  assert.equal(
    await stat(join(base, "npm", "node_modules", "missing-skills")).then(
      () => true,
      () => false,
    ),
    false,
...
test("a missing configured pin cannot fall back to a different unregistered npm version", async () => {
  ...
  await writeFile(
    ...
    JSON.stringify({ name: "example-skills", version: "1.0.0", pi: { skills: ["./advice"] } }),
  );
...
test("invalid UTF-8 is reported even when it prevents configured frontmatter parsing", async () => {
  ...
  assert.deepEqual(
    packages.flatMap((p) => p.refused),
    [{ subject: "advice", reason: "not-utf8", detail: [] }],
  );
  assert.equal((await loadDefinitions(cwd)).has("advice"), false);
