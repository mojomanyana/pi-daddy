...
test("work-v4 canonical identity matches independently fixed vectors", () => {
  ...
  assert.equal(
    createHash("sha256")
      .update(canonicalWorkJson({ b: 2, a: 1 }))
      .digest("hex"),
    "43258cff783fe7036d8a43033f830adfc60ec037382473548ac742b888292777",
  );
  assert.equal(
    createHash("sha256").update(canonicalWorkJson(vectors[1][0])).digest("hex"),
...
test("real work-v4 builders hash every own field and preserve nested digest identity", () => {
  const events = [
    rev(),
    buildWorkSnapshotEvent({ eventId: "snapshot", now, snapshot: snapshot() }),
  ...
  for (const event of events) {
    ...
    assert.notEqual(
      createHash("sha256").update(rawPretty).digest("hex"),
  ...
  const escaped = JSON.stringify(first)
    .replace('"eventId":"revision"', String.raw`"eventId":"\u0072evision"`)
    .replace('"revision":1', '"revision":10e-1');
  assert.deepEqual(parseWorkLedgerText(escaped), { events: [first], errors: [], complete: true });
  assert.notEqual(
    first.digest,
    buildWorkRevisionEvent({ eventId: "revision", now: new Date(now.getTime() + 1), revision: revision() }).digest,
  );
  ...
  for (const [field, value] of Object.entries({
    ...
    parseError(
      JSON.stringify({ ...outer, digest: createHash("sha256").update(canonicalWorkJson(outer)).digest("hex") }),
...
test("work-v4 builders reject unsupported objects without invoking getters or coercions", () => {
  ...
  for (const value of unsupported) {
    assert.throws(
      () => buildWorkRevisionEvent({ eventId: "revision", now, revision: { ...revision(), extra: value } } as any),
      inputError("WORK_SCHEMA_INVALID"),
    );
  }
  const args = Object.defineProperty({ eventId: "revision", revision: revision() }, "now", {
    get() {
      invoked++;
      return now;
    },
  ...
  for (const invalidDate of [new Date(NaN), Object.create(Date.prototype)]) {
    assert.throws(
      () => buildWorkRevisionEvent({ eventId: "revision", now: invalidDate, revision: revision() }),
      inputError("WORK_SCHEMA_INVALID"),
    );
  }
  assert.throws(
    () => buildWorkRevisionEvent({ eventId: "revision", now, revision: { ...revision(), digest } } as any),
    inputError("WORK_SCHEMA_INVALID"),
  );
  assert.throws(
    () => buildWorkSnapshotEvent({ eventId: "snapshot", now, snapshot: { ...snapshot(), digest } } as any),
    inputError("WORK_SCHEMA_INVALID"),
  );
  const input = revision();
  const event = buildWorkRevisionEvent({ eventId: "revision", now, revision: input });
...
test("work-v4 closed shapes, ID domains, timestamp calendar and canonical sets fail closed", () => {
  ...
  for (const id of ["", " space", "a b", "é", "a".repeat(129)]) {
    assert.throws(
      () => buildWorkRevisionEvent({ eventId: id, now, revision: revision() }),
      inputError("WORK_SCHEMA_INVALID"),
    );
  }
  for (const ts of [
  ...
  assert.throws(
    () =>
      buildWorkRevisionEvent({ eventId: "revision", now, revision: { ...revision(), permittedEffects: ["x", "x"] } }),
    inputError("WORK_SCHEMA_INVALID"),
  );
  ...
  assert.throws(
    () =>
      buildWorkSnapshotEvent({
        eventId: "snapshot",
        now,
        snapshot: { ...snapshot(), revisions: [ref("goal"), ref("goal")] },
      }),
    inputError("WORK_SCHEMA_INVALID"),
  );
  assert.throws(
    () =>
      buildWorkAcceptanceEvent({
        eventId: "acceptance",
        now,
        payload: { ...acceptance(), binding: { ...acceptance().binding, evidence: [] } },
      }),
    inputError("WORK_SCHEMA_INVALID"),
  );
  assert.throws(
    () =>
      buildWorkOccurrenceEvent({
        eventId: "occurrence",
        now,
        payload: { ...occurrence(), parentExecutionId: occurrence().executionId },
      }),
    inputError("WORK_SCHEMA_INVALID"),
  );
});
    ...
    for (const path of paths) {
      ...
      for (const key of Object.keys(original)) {
        ...
        parseError(
          ...
          path.length === 0 && key === "ledgerVersion" ? "WORK_VERSION_UNSUPPORTED" : "WORK_SCHEMA_INVALID",
        );
      }
...
test("work-v4 non-scope revisions, populated references and builder limits use the same strict foundation", () => {
  for (const kind of ["goal", "node", "obligation", "artifact", "policy"] as const) {
    const record: Omit<WorkRevision, "digest"> = {
      ...revision(),
      kind,
      id: kind,
      permittedEffects: [],
    ...
    const successor = buildWorkRevisionEvent({
      eventId: `${kind}:2`,
      now,
      revision: {
        ...record,
        revision: 2,
        predecessor: { kind, id: kind, revision: 1, digest: first.payload.revision.digest },
      },
    });
      ...
      () =>
        buildWorkRevisionEvent({
          ...
          revision: {
            ...record,
            revision: 2,
            predecessor: { ...ref(kind), revision: 2 },
          },
        }),
      inputError("WORK_SCHEMA_INVALID"),
    );
  ...
  const event = buildWorkSnapshotEvent({
    eventId: "snapshot",
    now,
    snapshot: {
      ...snapshot(),
      bindings: [binding],
      revisions: [ref("policy"), ref("goal"), ref("obligation"), ref("artifact")],
    },
  });
  ...
  assert.deepEqual(
    claim.payload.binding.evidence.map((e) => e.id),
    ["evidence", "z"],
  );
  ...
  assert.throws(
    () =>
      buildWorkRevisionEvent({
        ...
        revision: {
          ...revision(),
          permittedEffects: Array.from({ length: 257 }, (_, i) => `effect:${i}`),
        },
      }),
  ...
  assert.throws(
    () => buildWorkAcceptanceEvent({ eventId: "acceptance", now, payload: large }),
    inputError("WORK_LIMIT_EXCEEDED"),
  );
});
...
test("work-v4 ingestion bounds and diagnostics preserve valid physical deliveries without repair", () => {
  ...
  assert.deepEqual(result, {
    events: [rev(), rev()],
    errors: [
      { line: 3, code: "WORK_JSON_INVALID" },
      { line: 5, code: "WORK_VERSION_UNSUPPORTED" },
    ],
    complete: false,
  });
  ...
  for (const value of [null, undefined, 1, {}])
    assert.deepEqual(parseWorkLedgerText(value as any), {
      events: [],
      errors: [{ line: null, code: "WORK_SCHEMA_INVALID" }],
      complete: false,
    });
  assert.equal(parseWorkLedgerText((line + "\n").repeat(10_000)).events.length, 10_000);
  assert.deepEqual(parseWorkLedgerText((line + "\n").repeat(10_001)), {
    events: [],
    errors: [{ line: null, code: "WORK_LIMIT_EXCEEDED" }],
    complete: false,
  });
...
const loadGenerator = async () =>
  (await import(generatorUrl.href)) as typeof import("../scripts/generate-ledger-v4-contract.ts");
...
function expectedPublishedFixtures() {
  const scope = rev(),
    scopeRef = { ...ref("scope"), digest: scope.payload.revision.digest };
  const binding = {
    intent: ref("goal"),
    obligation: ref("obligation"),
    artifact: ref("artifact"),
  ...
  const selected = buildWorkSnapshotEvent({
    ...
    snapshot: {
      ...snapshot(),
      scope: scopeRef,
      revisions: [ref("goal"), ref("obligation"), ref("artifact"), ref("policy")],
      bindings: [binding],
    },
  });
  const observed = buildWorkOccurrenceEvent({
    eventId: "occurrence",
    now,
    payload: { ...occurrence(), scope: scopeRef, artifact: ref("artifact") },
  });
  const claim = buildWorkAcceptanceEvent({
    eventId: "acceptance",
    now,
    payload: {
      authorityId: "authority",
      binding: {
        ...acceptance().binding,
        scope: scopeRef,
        snapshot: { id: "snapshot", digest: selected.payload.snapshot.digest },
        evidence: [{ id: "evidence", digest, event: { eventId: observed.eventId, digest: observed.digest } }],
      },
    },
  });
  return {
    "work-revision.json": scope,
    "work-snapshot.json": selected,
    "work-occurrence.json": observed,
    "work-acceptance.json": claim,
  };
}
...
test("work-v4 scalar domains reject terminal line breaks before hashing", () => {
  for (const ending of ["\n", "\r", "\r\n", "\u2028", "\u2029"]) {
    assert.throws(
      () => buildWorkRevisionEvent({ eventId: "valid" + ending, now, revision: revision() }),
      inputError("WORK_SCHEMA_INVALID"),
    );
    assert.throws(
      () =>
        buildWorkRevisionEvent({
          eventId: "revision",
          now,
          revision: { ...revision(), contentDigest: digest + ending },
        }),
      inputError("WORK_SCHEMA_INVALID"),
    );
    assert.throws(
      () =>
        buildWorkOccurrenceEvent({
          eventId: "occurrence",
          now,
          payload: { ...occurrence(), executionId: occurrence().executionId + ending },
        }),
      inputError("WORK_SCHEMA_INVALID"),
    );
  }
...
test("published work-v4 fixtures come from real builders and the closed schema", async () => {
  ...
  const schema = await json(join(contractRoot, "ledger-event.schema.json")),
    validator = Compile(schema);
  assert.equal(schema.$schema, "https://json-schema.org/draft/2020-12/schema");
  assert.deepEqual(
    schema.oneOf.map((s: any) => s.$ref).sort(),
    ["workRevisionEvent", "workSnapshotEvent", "workOccurrenceEvent", "workAcceptanceEvent"]
      .map((s) => "#/$defs/" + s)
      .sort(),
  );
  ...
  const dir = await tempDir("work-contract-generation-"),
    before = await collateral();
  await generator.generateLedgerV4Contract(dir);
  assert.deepEqual(await collateral(), before);
  assert.deepEqual(
    await readFile(join(dir, "ledger-event.schema.json")),
    await readFile(join(contractRoot, "ledger-event.schema.json")),
  );
  for (const [name, event] of Object.entries(fixtures)) {
    ...
    const visit = (value: any, path: string[]) => {
      ...
      if (!Array.isArray(value)) {
        for (const key of [...Object.keys(value), "extra", "__proto__", "approved_by", "receipt"]) {
          const altered: any = structuredClone(event),
            target = path.reduce((o, k) => o[k], altered);
          ...
          assert.equal(
            parseWorkLedgerText(JSON.stringify(altered)).errors[0]?.code,
    ...
    visit(event, []);
    for (const discriminator of ["capability_decision", "unknown", null, 4])
  ...
  const readmePath = join(contractRoot, "README.md");
  const readme = await readFile(readmePath, "utf8");
  const links = [...readme.matchAll(/\[[^\]]+\]\(([^)#]+)(?:#[^)]*)?\)/g)].map((m) => m[1]);
  assert.ok(links.length >= 2);
  for (const link of links.filter((link) => !/^https?:/.test(link)))
    assert.ok(existsSync(resolve(dirname(readmePath), link)), `${readmePath}: ${link}`);
});
...
test("work-v4 schema domains agree with runtime without pretending to validate semantic evidence", async () => {
  ...
  const bothInvalid = (event: any, path: string[], value: unknown) => {
    const bad: any = structuredClone(event),
      parent = path.slice(0, -1).reduce((o, k) => o[k], bad);
  ...
  for (const bad of [0, -1, 0.5, Number.MAX_SAFE_INTEGER + 1, "1"])
    bothInvalid(fixtures["work-revision.json"], ["payload", "revision", "revision"], bad);
  for (const ts of [
    "0000-01-01T00:00:00.000Z",
    ...
    bothInvalid(fixtures["work-revision.json"], ["ts"], ts);
  for (const field of ["executionId", "parentExecutionId"])
    for (const bad of ["exec:bad", occurrence().executionId + "\n"]) {
      bothInvalid(fixtures["work-occurrence.json"], ["payload", field], bad);
    }
  for (const provenance of ["declared", "observed"] as const)
    for (const state of ["unknown", "starting", "running", "completed", "failed"] as const) {
      const e = buildWorkOccurrenceEvent({ eventId: "states", now, payload: { ...occurrence(), provenance, state } });
      assert.equal(validator.Check(e), true);
      assert.equal(parseWorkLedgerText(JSON.stringify(e)).complete, true);
    }
  for (const field of ["provenance", "state"])
    bothInvalid(fixtures["work-occurrence.json"], ["payload", field], "approved");
  for (const field of Object.keys(occurrence().labels)) {
    bothInvalid(
      fixtures["work-occurrence.json"],
      ["payload", "labels", field],
      field.endsWith("Digest") ? "not-a-digest" : "bad label",
    );
  }
  for (const kind of ["scope", "goal", "node", "obligation", "artifact", "policy"] as const) {
    const body: Omit<WorkRevision, "digest"> = {
      ...revision(),
      kind,
      id: kind,
      permittedEffects: [],
      parent: kind === "goal" ? ref("scope") : kind === "node" || kind === "obligation" ? ref("goal") : null,
      policy: kind === "obligation" ? ref("policy") : null,
      dependencies: kind === "obligation" ? [ref("obligation", "other")] : [],
    };
    for (const n of [1, 2]) {
      const e = buildWorkRevisionEvent({
        eventId: kind,
        now,
        revision: { ...body, revision: n, predecessor: n === 1 ? null : { kind, id: kind, revision: 1, digest } },
      });
      assert.equal(validator.Check(e), true, kind);
      assert.equal(parseWorkLedgerText(JSON.stringify(e)).complete, true);
    }
    for (const parentKind of (kind === "goal"
      ? ["scope", "goal"]
      : kind === "node" || kind === "obligation"
        ? ["goal", "node"]
        : []) as RevisionRef["kind"][]) {
      const alternative = buildWorkRevisionEvent({
        eventId: kind,
        now,
        revision: { ...body, parent: ref(parentKind) },
      });
      assert.equal(validator.Check(alternative), true);
      assert.equal(parseWorkLedgerText(JSON.stringify(alternative)).complete, true);
    }
    const e = buildWorkRevisionEvent({ eventId: kind, now, revision: body });
    bothInvalid(
      e,
      ["payload", "revision", "parent"],
      kind === "goal" || kind === "node" || kind === "obligation" ? null : ref("goal"),
    );
    ...
    if (kind === "artifact" || kind === "policy")
      bothInvalid(e, ["payload", "revision", "permittedEffects"], ["write"]);
  }
  ...
  for (const [file, path, kind] of [
    ["work-snapshot.json", ["scope"], "goal"],
    ["work-snapshot.json", ["revisions", "0"], "scope"],
    ...
    ...["scope", "intent", "obligation", "artifact", "policy"].map((k) => [
      "work-acceptance.json",
      ["binding", k],
      k === "intent" ? "artifact" : "node",
    ]),
  ] as [keyof typeof fixtures, string[], string][]) {
    bothInvalid(
      fixtures[file],
      ["payload", ...(file === "work-snapshot.json" ? ["snapshot"] : []), ...path, "kind"],
      kind,
    );
  }
  const maximal = buildWorkRevisionEvent({
    eventId: "maximal",
    now,
    revision: {
      ...revision(),
      revision: Number.MAX_SAFE_INTEGER,
      predecessor: { ...ref("scope"), revision: Number.MAX_SAFE_INTEGER - 1 },
      permittedEffects: Array.from({ length: 256 }, (_, i) => `effect:${i}`),
    },
  });
  assert.equal(validator.Check(maximal), true);
  assert.equal(parseWorkLedgerText(JSON.stringify(maximal)).complete, true);
  const upper = buildWorkOccurrenceEvent({
    eventId: "upper",
    now,
    payload: { ...occurrence(), executionId: occurrence().executionId.toUpperCase() },
  });
  assert.equal(validator.Check(upper), true);
  const unselectedArtifact = buildWorkSnapshotEvent({
    eventId: "null-artifact",
    now,
    snapshot: {
      ...snapshot(),
  ...
  const oversized = structuredClone(fixtures["work-revision.json"]) as any;
  oversized.payload.revision.permittedEffects = many;
  assert.equal(validator.Check(oversized), false);
  assert.equal(parseWorkLedgerText(JSON.stringify(oversized)).errors[0]?.code, "WORK_LIMIT_EXCEEDED");
  ...
  assert.equal(validator.Check(forged), true);
  assert.equal(parseWorkLedgerText(JSON.stringify(forged)).errors[0]?.code, "WORK_DIGEST_MISMATCH");
  const duplicate = JSON.stringify(fixtures["work-revision.json"]).replace(
    '"ledgerVersion":4',
    '"ledgerVersion":4,"ledgerVersion":4',
  );
  assert.equal(validator.Check(JSON.parse(duplicate)), true);
  assert.equal(parseWorkLedgerText(duplicate).errors[0]?.code, "WORK_DUPLICATE_MEMBER");
});

test("work-v4 contract generator import is read-only and every write target is explicit", async () => {
  const dir = await tempDir("work-generator-import-"),
    before = await collateral();
  ...
  assert.deepEqual(JSON.parse(child.stdout), fixtureNames);
  ...
  const missing = spawnSync(process.execPath, [fileURLToPath(generatorUrl)], {
    cwd: dir,
    encoding: "utf8",
    timeout: 10000,
    maxBuffer: 100000,
  ...
  assert.match(missing.stderr, /explicit target/);
  assert.deepEqual(await readdir(dir), []);
  assert.deepEqual(await collateral(), before);
  const output = join(dir, "explicit");
  const generated = spawnSync(process.execPath, [fileURLToPath(generatorUrl), output], {
    cwd: dir,
    encoding: "utf8",
    timeout: 10000,
    maxBuffer: 100000,
...
test("layout options fixtures are reproducible without Principal vocabulary", async () => {
  const generator = await loadGenerator();
  assert.equal(
    typeof generator.buildLayoutOptionsFixture,
    "function",
    "pure complete layout wire fixture export is required",
  );
  const layout = generator.buildLayoutOptionsFixture(),
    expected = layoutFixture().events;
  ...
  const published = await readFile(join(contractRoot, "fixtures/layout-options.json"), "utf8");
  assert.equal(published, bytes);
  ...
  for (let i = 0; i < 2; i++) {
    const dir = await tempDir("layout-contract-reproduction-");
    await generator.generateLedgerV4Contract(dir);
    ...
    for (const name of fixtureNames) {
      ...
      assert.deepEqual(
        await readFile(join(dir, "fixtures", name)),
        await readFile(join(contractRoot, "fixtures", name)),
      );
    }
  ...
  const ctx = layoutAuthority(),
    { projectWorkLedger } = await import("../src/governance/work-ledger.ts");
  ...
  assert.deepEqual(
    projectWorkLedger(fixtureText(layout), { selectedSnapshot: ctx.selectedSnapshot, authority: null }).progress,
    { accepted: 0, total: 1 },
  );
});

test("explicit work-v4 inspection does not widen old readers", async () => {
  const events = Object.values(expectedPublishedFixtures()),
    text = events.map((e) => JSON.stringify(e)).join("\n") + "\n";
  const dir = await tempDir("work-reader-compat-"),
    path = join(dir, "work");
  await writeFile(path, text);
  const inspection = await inspectWorkLedger({ version: 4, path });
  assert.equal(inspection.ingestion!.complete, true);
  assert.equal(inspection.projection!.progress, null);
