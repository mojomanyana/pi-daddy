...
test("ordinary public delegation opts into per-execution native session files without model fields", async () => {
  const root = await tempDir("host-native-"),
    bin = join(root, "bin"),
...
test("native target refuses reused occurrences and nonprivate roots", async () => {
  const root = await tempDir("native-owned-");
  await allocateNativeSessionTarget(root, "exec:one");
  await assert.rejects(allocateNativeSessionTarget(root, "exec:one"), /EEXIST/);
  await chmod(root, 0o755);
  await assert.rejects(allocateNativeSessionTarget(root, "exec:two"), /private/);
});
