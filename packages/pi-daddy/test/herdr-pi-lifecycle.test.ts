...
test("the pinned Herdr Pi integration reports native start/settled lifecycle and registers no tools", async () => {
  ...
  try {
    ...
    const module = await import(
      `${new URL("../src/executors/vendor/herdr-pi-lifecycle.ts", import.meta.url).href}?socket=${encodeURIComponent(socket)}`
    );
    module.default(pi);
    ...
    assert.deepEqual(
      reports.filter((report) => report.method === "pane.report_agent").map((report) => report.params.state),
      ["idle", "working", "idle"],
    );
