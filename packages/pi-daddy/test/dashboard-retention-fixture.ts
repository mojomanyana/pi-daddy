...
export async function ordinaryHostRetention(root: string) {
  ...
  try {
    ...
    grantsExtension({
      on: (n: string, f: unknown) => hooks.set(n, f),
    ...
    const result = await tools
      .get("delegate")
      .execute("call:retained", { task: "retained ordinary fixture", tools: [] }, undefined, undefined, ctx);
    if (result.details.exitCode !== 0) throw Error("ordinary source worker did not succeed");
    await drainExecutionRetention(result.details.retention);
    const path = result.details.retention.manifestPath,
      manifest = parseExecutionRetentionManifest(await readFile(path, "utf8"));
    return { path, relativePath: relative(root, path), manifest };
  } finally {
