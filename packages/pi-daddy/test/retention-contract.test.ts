...
import {
  buildRetentionContractFiles,
  writeRetentionContract,
  checkRetentionContract,
} from "../scripts/generate-retention-contract.ts";
import {
  buildExecutionRetentionManifest,
  parseExecutionRetentionManifest,
  RETENTION_SCHEMA,
} from "../src/governance/retention-contract.ts";
import { verifyRetainedBytes } from "../src/governance/execution-retention.ts";
...
test("retention v2 source builder and published strict schema reject unknown or contradictory facts", () => {
  const validator = Compile(RETENTION_SCHEMA),
    m = example();
  assert.equal(validator.Check(m), true);
  ...
  for (const mutate of [
    ...
    const bad = example();
    mutate(bad);
    assert.equal(validator.Check(bad), false);
    assert.throws(() => buildExecutionRetentionManifest(bad));
  }
  for (const mutate of [
...
test("retention JSON and object builders reject duplicate keys, accessors and hidden array state", () => {
  ...
  assert.throws(() =>
    parseExecutionRetentionManifest(source.replace('"version": "2.0"', '"version":"1.0","version":"2.0"')),
  );
  const accessor = example();
  let invoked = false;
  Object.defineProperty(accessor.identity, "toolCallId", {
    enumerable: true,
    get() {
      invoked = true;
      return "call";
  ...
  assert.throws(() => buildExecutionRetentionManifest(accessor));
  assert.equal(invoked, false);
  const array = example();
  array.coverage.losses.hidden = "unserialized";
  assert.throws(() => buildExecutionRetentionManifest(array), "strict JSON builder rejects extra own array fields");
  const numeric = example();
  numeric.native.pid = 1;
  assert.throws(() =>
    parseExecutionRetentionManifest(JSON.stringify(numeric).replace('"pid":1', '"pid":1.000000000000000000001')),
  );
  const m = buildExecutionRetentionManifest(example());
  assert.ok(Object.isFrozen(m.nativeSession));
...
test("public P03 consumer verifies bytes and never substitutes a manifest digest for missing content", () => {
  ...
  assert.equal(live.nativeSession.branchLeafId, "aaaaaaaa");
  assert.equal(unknown.nativeSession.branchLeafId, null);
});
...
test("retention contract generator is explicit and byte-reproducible including complete fixture inventories", async () => {
  ...
  const imported = spawnSync(
    ...
    [
      "--input-type=module",
      "-e",
      `await import(${JSON.stringify(new URL("../scripts/generate-retention-contract.ts", import.meta.url).href)})`,
    ],
    {
      cwd: root,
      env: { HOME: root, TMPDIR: root, PI_CODING_AGENT_DIR: root, PATH: "", PI_OFFLINE: "1" },
      encoding: "utf8",
      timeout: 10000,
    },
  );
  ...
  const a = join(root, "a"),
    b = join(root, "b");
  await writeRetentionContract(a);
  await writeRetentionContract(b);
  await checkRetentionContract(a);
  await checkRetentionContract(b);
  for (const [name, bytes] of Object.entries(fixtures)) {
    assert.equal(await readFile(join(a, name), "utf8"), bytes);
    assert.equal(await readFile(join(b, name), "utf8"), bytes);
  }
  await mkdir(join(a, "fixtures", "unexpected"));
  await assert.rejects(checkRetentionContract(a), /inventory/);
