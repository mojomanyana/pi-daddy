...
test("cancel readback waits for real settlement under the existing control locks", async () => {
  ...
  const path = join(f.budget.directory, "budget.jsonl"),
    storeLock = join(f.binding.directory, "experiment.jsonl.lock");
  ...
  const owners = new WeakSet<object>(),
    settlements = new WeakSet<object>();
  let timer: ReturnType<typeof setTimeout>;
  const bound = new Promise<never>((_, reject) => {
    timer = setTimeout(() => reject(new Error("control readback did not wait for original settlement")), 5000);
  });
  fsPromises.open = (async (...args: any[]) => {
    if (String(args[0]) === storeLock && acquired > 0) await holding;
    if (String(args[0]) === path + ".lock" && settlementsSeen) {
      waited();
      release();
    }
  ...
  proto.writeFile = async function (data: unknown, ...args: any[]) {
    ...
    if (
      owners.has(this) &&
      String(data).includes('"type":"settle"') &&
      String(data).includes('"attemptId":"execution:1"')
    )
      settlements.add(this);
    return result;
  };
  proto.sync = async function (...args: any[]) {
    if (settlements.has(this)) {
      settlementsSeen = true;
      held();
      await proceed;
    }
  ...
  try {
    ...
    release();
    held();
    fsPromises.open = original.open;
    proto.writeFile = original.writeFile;
  ...
  assert.equal(done.variants[1].state, "cancelled");
