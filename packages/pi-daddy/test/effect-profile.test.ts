...
async function fixture() {
  const root = await tempDir("effect-profile-");
  await chmod(root, 0o700);
  const binding = await createResourceBudget({
    directory: join(root, "budget"),
    authorityDigest: hash("fixed-host-authority"),
...
test("profile probes remove only their fresh owned directory after success and probe failure", async () => {
  const { binding } = await fixture(),
    preserved = "profile-probe-preserved";
  await mkdir(join(binding.directory, preserved));
  await prepareDigestProfile(binding);
  assert.deepEqual(
    (await readdir(binding.directory)).filter((name) => name.startsWith("profile-probe-")),
    [preserved],
  );
  const original = childProcess.spawn,
    spy = mock.method(childProcess, "spawn", (command: any, args: any, options: any) =>
      command === "/usr/bin/bwrap" ? original("/usr/bin/false", [], options) : original(command, args, options),
    );
  syncBuiltinESMExports();
  try {
    await assert.rejects(prepareDigestProfile(binding), /fixed native profile failed/);
  } finally {
    spy.mock.restore();
    syncBuiltinESMExports();
  }
...
test("real probed digest profile uses the existing launcher, exact byte input and durable admission", async () => {
  const { binding } = await fixture(),
    profile = await prepareDigestProfile(binding);
  const bytes = Buffer.from("abc");
  const execution = runDigestProfile(profile, { attempt: attempt("one"), bytes });
  bytes.fill(0);
  const result = await execution;
  assert.equal(result.output.code, 0);
  assert.equal(result.digest, hash("abc"));
  const state = await openResourceBudget(binding).inspect();
  assert.equal(state.attempts, 1);
  assert.equal(state.inputBytes, 3);
  assert.equal(state.active, 0);
  assert.equal(state.reservations[0].inputDigest, hash("abc"));
  await assert.rejects(runDigestProfile(profile, { attempt: attempt("one"), bytes: Buffer.from("abc") }), {
    code: "DUPLICATE",
  });
});
...
test("unsupported, receipt-shaped and mutable-destination routes fail before workload launch", async () => {
  ...
  await assert.rejects(
    runDigestProfile({ profile: "shell" } as unknown as DigestProfile, {
      attempt: attempt("no"),
      bytes: Buffer.from("x"),
    }),
    /unsupported or unprobed/,
  );
  const profile = await prepareDigestProfile(binding);
  await assert.rejects(
    runDigestProfile(JSON.parse(JSON.stringify(profile)), { attempt: attempt("forged"), bytes: Buffer.from("x") }),
    /unsupported or unprobed/,
  );
  const marker = join(root, "outside-denied");
  await assert.rejects(
    runDigestProfile(profile, {
      attempt: attempt("destination"),
      bytes: Buffer.from("x"),
      destination: marker,
    } as never),
    /byte-only/,
  ...
  await assert.rejects(
    runDigestProfile(profile, { attempt: attempt("money"), bytes: Buffer.from("x"), moneyCap: 1 } as never),
    /byte-only/,
  );
  await assert.rejects(
    runDigestProfile(profile, { attempt: attempt("large"), bytes: Buffer.alloc(16385) }),
    /byte-only/,
  );
...
test("unsupported routes make zero actual spawn calls, and a post-spawn accounting failure still rejects", async () => {
  ...
  const spy = mock.method(childProcess, "spawn", (...args: any[]) => {
    ...
    if (replace && child.pid) {
      const path = join(binding.directory, "budget.jsonl"),
        bytes = readFileSync(path);
      renameSync(path, path + ".retained-before-replacement");
  ...
  try {
    await assert.rejects(
      runDigestProfile({ ...profile }, { attempt: attempt("unsupported"), bytes: Buffer.from("abc") }),
    );
    await assert.rejects(
      runDigestProfile(profile, {
        attempt: attempt("writable"),
    ...
    replace = true;
    await assert.rejects(runDigestProfile(profile, { attempt: attempt("lost"), bytes: Buffer.from("abc") }), {
    ...
    assert.ok(old.includes('"type":"reserve"'));
    assert.ok(!old.includes('"type":"settle"'));
  } finally {
    spy.mock.restore();
    syncBuiltinESMExports();
  }
...
test("cancellation after durable reservation is charged, settled, and cannot be retried under the same identity", async () => {
  const { binding } = await fixture(),
    profile = await prepareDigestProfile(binding),
    budget = openResourceBudget(binding);
  const early = AbortSignal.abort();
  await assert.rejects(
    runDigestProfile(profile, { attempt: attempt("early"), bytes: Buffer.from("abc") }, early),
    /before reservation/,
  );
  ...
  const observer = (async () => {
    try {
      for await (const _ of watch(join(binding.directory, "budget.jsonl"), { signal: watching.signal })) {
        if ((await readFile(join(binding.directory, "budget.jsonl"), "utf8")).includes('"type":"reserve"')) {
          controller.abort();
          break;
        }
      }
  ...
  try {
    const result = await runDigestProfile(
      profile,
      { attempt: attempt("cancelled"), bytes: Buffer.from("abc") },
      controller.signal,
    );
  ...
  assert.equal(state.attempts, 1);
  assert.equal(state.inputBytes, 3);
  await assert.rejects(runDigestProfile(profile, { attempt: attempt("cancelled"), bytes: Buffer.from("abc") }), {
    code: "DUPLICATE",
  });
});
...
test("actual namespace denies owned outside read/write, read-only mutation, host network and process creation", async (t) => {
  ...
  await writeFile(join(input, "allowed"), "allowed");
  let connections = 0;
    ...
    runtime = await digestRuntime();
  {
    ...
    const output = await runChild({
      ...
      args: digestNamespaceArgs(runtime, code, [], input),
      cwd: root,
      env: {},
      timeoutMs: 3000,
      killGraceMs: 50,
    ...
    assert.deepEqual(JSON.parse(output.text), {
      allowed: "allowed",
      denied: ["outsideRead:ENOENT", "outsideWrite:ENOENT", "readOnlyWrite:EROFS"],
    });
    ...
    const child = await runChild({
      command: "/usr/bin/bwrap",
      args: digestNamespaceArgs(
        runtime,
        `try{require('node:child_process').spawnSync('/runtime/node',['-e','process.exit(90)']);process.exit(91)}catch(e){console.log(e.code)}`,
        [],
      ),
      cwd: root,
      env: {},
    ...
    assert.equal(child.code, 0, child.text);
    assert.equal(child.text.trim(), "ERR_ACCESS_DENIED");
  }
});
...
test("cancelling the actual namespace stops the owned worker, not just its output collector", async () => {
  ...
  const result = await runChild({
    command: "/usr/bin/bwrap",
    args: digestNamespaceArgs(runtime, `console.log('ready');setInterval(()=>{},1000);`, []),
    env: {},
    cwd: "/",
    timeoutMs: 3000,
    killGraceMs: 50,
    maxOutputBytes: NATIVE_DIAGNOSTIC_TEXT_BYTES,
    signal: controller.signal,
    onSpawn: (value) => {
      pid = value;
    },
    onOutput: (text) => {
      if (text.includes("ready") && !ready) ready = collect(pid).then(() => controller.abort());
    },
  });
  await ready;
  assert.equal(result.aborted, true, nativeResultDiagnostic(result));
  assert.ok(owned.size >= 2, "actual owned namespace worker was observed before cancellation");
  const live = async (id: number) => {
    try {
      return !/^State:\s+Z/m.test(await readFile(`/proc/${id}/status`, "utf8"));
    } catch {
      return false;
    }
  };
  for (let i = 0; i < 100 && (await Promise.all([...owned].map(live))).some(Boolean); i++)
    await new Promise((r) => setTimeout(r, 10));
  assert.deepEqual(
    await Promise.all([...owned].map(live)),
    [...owned].map(() => false),
  );
});
