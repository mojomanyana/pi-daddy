...
} from "../src/products/resource-budget.ts";
...
const request = (
  b: GovernedBudgetBinding,
  requestId = "pause",
  expectedRevision = 0,
  action: DispatchRequest["action"] = "pause-dispatch",
): DispatchRequest => ({
  version: "1.0",
  requestId,
  ...
  expectedRevision,
  action,
  targetExecutionId: action === "cancel-execution" ? "attempt" : null,
});
// Independently reconstruct fixed test-world host decisions. Never authorize the received wire object.
const grant = (b: GovernedBudgetBinding) => ({
  authorityDigest,
  requestDigests: [
    request(b),
    request(b, "resume", 1, "resume-dispatch"),
    request(b, "stale", 0, "resume-dispatch"),
    request(b, "second", 1, "resume-dispatch"),
    ...["cancel-execution", "revise-scope", "reprioritize", "select-alternative"].map((a, i) =>
      request(b, `unsupported-${i}`, 0, a as DispatchRequest["action"]),
    ),
  ].map(dispatchRequestDigest),
});
...
test("strict versioned wire contract rejects duplicate members, lossy integer tokens and authority metadata", async () => {
  ...
  const schema = JSON.parse(
    await readFile(new URL(import.meta.resolve("pi-daddy/contracts/dispatch-control/v1/request.schema.json")), "utf8"),
  );
  const check = Compile(schema as TSchema);
  assert.ok(check.Check(r));
  assert.deepEqual(parseDispatchRequest(JSON.stringify(r)), r);
  for (const bad of [
    { ...r, version: "2.0" },
    { ...r, authority: authorityDigest },
    { ...r, targetExecutionId: "pane" },
    { ...r, expectedRevision: 0.1 },
  ]) {
    assert.equal(check.Check(bad), false);
    assert.throws(() => parseDispatchRequest(JSON.stringify(bad)));
  }
  assert.throws(() =>
    parseDispatchRequest(
      JSON.stringify(r).replace('"expectedRevision":0', '"expectedRevision":1.000000000000000000001'),
    ),
  );
  assert.throws(() =>
    parseDispatchRequest(JSON.stringify(r).replace('"version":"1.0"', '"version":"1.0","version":"1.0"')),
  );
});

test("exact host-authorized pause/resume changes the actual reservation gate, not work acceptance", async () => {
  const b = await fixture(),
    budget = openResourceBudget(b),
    pause = request(b),
  ...
  assert.equal(state.records[0].outcome, "paused");
  ...
  assert.deepEqual(await controls.request(pause), state);
  assert.deepEqual(await readFile(join(b.directory, "budget.jsonl")), bytes);
  assert.equal((await controls.request(resume)).paused, false);
  const permit = await budget.reserve(demand("allowed"));
  await permit.settle("completed");
  assert.equal((await budget.inspect()).attempts, 1);
  assert.ok(Object.isFrozen(state.records[0].request));
});
...
test("busy steering stays pending until original permits settle; restart/receipt/idle claims cannot manufacture a boundary", async () => {
  const b = await fixture(),
    budget = openResourceBudget(b),
    permit = await budget.reserve(demand("busy"));
  const pause = request(b),
    control = budget.controls(grant(b));
  const pending = await control.request(pause);
  assert.equal(pending.records[0].application, "pending");
  assert.equal(pending.admission, "blocked-pending");
  ...
  await permit.settle("completed");
  assert.equal(
    (await openResourceBudget(b).controls(null).reconcile(pause.requestId)).records[0].application,
    "pending",
    "reconciliation needs independent authority again",
  );
  assert.equal((await restart.reconcile("another-id")).records[0].application, "pending");
  const applied = await restart.reconcile(pause.requestId);
  assert.equal(applied.paused, true);
  const bytes = await readFile(join(b.directory, "budget.jsonl"));
  assert.deepEqual(await restart.reconcile(pause.requestId), applied);
  assert.deepEqual(await readFile(join(b.directory, "budget.jsonl")), bytes);
});
...
test("stale selections, conflicting IDs and unknown authority refuse without replacing newer decisions", async () => {
  const b = await fixture(),
    pause = request(b),
    stale = request(b, "stale", 0, "resume-dispatch");
  ...
  assert.equal(
    (
      await openResourceBudget(b)
        .controls(null)
        .request(request(b, "no-authority", 1))
    ).records[3].decision,
    "authority-unavailable",
  );
  assert.equal(denied.records[2].decision, "authority-unavailable");
...
test("cancellation is a distinct refused route, never ordinary pending steering or terminal typing", async () => {
  const b = await fixture(),
    requests = ["cancel-execution", "revise-scope", "reprioritize", "select-alternative"].map((a, i) =>
      request(b, `unsupported-${i}`, 0, a as DispatchRequest["action"]),
    );
  const c = openResourceBudget(b).controls(grant(b));
  for (const r of requests) {
    const state = await c.request(r);
    assert.equal(state.records.at(-1)?.decision, "unsupported");
    assert.equal(state.records.at(-1)?.application, "not-applied");
...
test("status reads have no journal, lock, directory or session/control side effects", async () => {
  const b = await fixture(),
    c = openResourceBudget(b).controls(null);
  const session = join(b.directory, "session.jsonl");
  await writeFile(session, '{"message":"owned fixture"}\n');
  const names = await readdir(b.directory),
    before = await Promise.all(names.map((n) => readFile(join(b.directory, n))));
  for (let i = 0; i < 4; i++) {
    await c.inspect();
    await openResourceBudget(b).inspect();
  }
  assert.deepEqual(await readdir(b.directory), names);
  assert.deepEqual(await Promise.all(names.map((n) => readFile(join(b.directory, n)))), before);
});
...
test("real cross-process redelivery persists one request and one application", async () => {
  ...
  await Promise.all(
    Array.from({ length: 4 }, () =>
      promisify(execFile)(
        process.execPath,
        ["--input-type=module", "-e", code, JSON.stringify(b), JSON.stringify(grant(b)), JSON.stringify(r)],
        { env: { PATH: "", HOME: b.directory, TMPDIR: b.directory }, timeout: 10000 },
      ),
    ),
  );
  const lines = (await readFile(join(b.directory, "budget.jsonl"), "utf8")).trim().split("\n");
  assert.equal(lines.length, 3);
  assert.equal((await openResourceBudget(b).controls(null).inspect()).revision, 1);
});

test("missing/partial/replaced mandatory journal fails closed; no receipt-shaped repair", async () => {
  const b = await fixture(),
    c = openResourceBudget(b).controls(grant(b));
  const file = join(b.directory, "budget.jsonl"),
    bytes = await readFile(file);
  await writeFile(file, bytes.subarray(0, bytes.length - 1));
  await assert.rejects(c.request(request(b)), /incomplete/);
  await writeFile(file, bytes);
  await rename(file, file + ".owned-history");
...
test("failed mandatory request sync gives no application acknowledgement; only explicit reconciliation can finish", async (t) => {
  const b = await fixture(),
    r = request(b),
    c = openResourceBudget(b).controls(grant(b));
  const handle = await open(join(b.directory, "budget.jsonl"), "r"),
    prototype = Object.getPrototypeOf(handle);
  await handle.close();
  const sync = t.mock.method(prototype, "sync", async () => {
    throw new Error("owned mandatory sync failure");
  });
  await assert.rejects(c.request(r), /owned mandatory sync failure/);
  sync.mock.restore();
  const state = await c.inspect();
  assert.equal(state.records[0].application, "pending");
  assert.equal(state.paused, false);
  const before = await readFile(join(b.directory, "budget.jsonl"));
  assert.equal((await c.request(r)).records[0].application, "pending");
  assert.deepEqual(await readFile(join(b.directory, "budget.jsonl")), before);
...
test("independent authority is detached; v1 journals cannot silently opt into control semantics", async () => {
  const b = await fixture(),
    r = request(b),
    a = grant(b),
    c = openResourceBudget(b).controls(a);
  a.authorityDigest = "d".repeat(64);
  a.requestDigests.length = 0;
  assert.equal((await c.request(r)).paused, true);
  const root = await tempDir("dispatch-legacy-");
  await chmod(root, 0o700);
