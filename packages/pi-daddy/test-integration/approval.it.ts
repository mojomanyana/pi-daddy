...
describe(
  "the persisted approval store, read back through a real pi process",
  {
 ...
 */
describe(
  "an always-scoped approval, from the dialog to the file and back",
  {
    skip: !piAvailable()
      ? "pi is not on PATH"
      : !modelTestsEnabled
  ...
  () => {
    test("it is offered, written, honoured without re-prompting, and voided by a body edit", async () => {
      ...
      assert.ok(
        granting.toolCalls.some((c) => c.name === "delegate"),
